...
- **Technical Implementation:** See [dih_models/plotting/chart_style.py](../dih_models/plotting/chart_style.py) for all code constants and functions
...
All reusable charts are located in [knowledge/figures/](../knowledge/figures/) as **Quarto Markdown (.qmd) files**. These files contain executable Python code that generates visualizations. See that directory for the complete list of available visualizations.
...
```markdown
{{< include knowledge/figures/disease-vs-war-annual-deaths-pie-chart.qmd >}}
```
...
4. **Save generated images** in the same `knowledge/figures/` directory
...
2. **Save images using dynamic project root** (works regardless of where quarto runs from):
   ...
   # At bottom - save to knowledge/figures/ using dynamic path
   output_dir = project_root / 'knowledge' / 'figures'
   output_dir.mkdir(parents=True, exist_ok=True)
