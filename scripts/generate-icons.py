...
def generate_icons(source_path, output_dirs, base_name='dih-icon', transparent=True, social_source_path=None):
    ...
    print(f"      Use the same files in your HTML <link> tags and manifest.json")
    print(f"{'='*60}\n")
...
def main():
    parser = argparse.ArgumentParser(
        description='Generate minimal essential icon sizes and favicons from source image'
    )
    parser.add_argument(
        '--source',
        type=str,
    ...
    parser.add_argument(
        ...
        type=str,
        default='dih-icon',
        help='Base name for generated files (default: dih-icon)'
    )
    parser.add_argument(
        '--no-transparent',
        action='store_true',
        help='Skip making black background transparent'
    )
    parser.add_argument(
        '--social-source',
        type=str,
    ...
    if args.output is None:
        ...
        output_dirs = [
            r'E:\code\dih-neobrutalist\public\assets\icons',
            str(current_project_icons)
        ]
    else:
