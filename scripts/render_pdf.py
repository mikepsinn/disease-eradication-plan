...
from render_utils import BuildMonitor, create_latex_parser, kill_existing_quarto_processes, run_pre_validation
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Unified PDF render script with validation, logging, and monitoring")
    parser.add_argument(
        "--timeout",
        type=int,
        default=900,
    ...
    parser.add_argument("--log-file", type=str, default="build-pdf.log", help="Log file path (default: build-pdf.log)")
    parser.add_argument(
        "--command",
        type=str,
        default="quarto render . --to pdf",
        help="Build command to run (default: quarto render . --to pdf)",
    )
    parser.add_argument(
        "--kill-existing", action="store_true", help="Kill all existing Quarto/LaTeX processes before starting build"
    )
