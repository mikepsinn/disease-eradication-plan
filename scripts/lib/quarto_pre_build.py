...
def prepare_pdf_build_temp(
    ...
    try:
        ...
        # Always copy critical files that may be gitignored (index.qmd, _quarto.yml)
        # These are generated by prepare_*() functions and needed for rendering
        critical_files = ["index.qmd", "_quarto.yml"]
        for filename in critical_files:
            src_file = project_root / filename
            if src_file.exists():
                dest_file = build_temp / filename
                shutil.copy2(src_file, dest_file)
                if verbose:
                    print(f"[*] Copied critical file: {filename}", flush=True)
