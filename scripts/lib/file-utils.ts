...
/**
 * Gets all QMD files with frontmatter (title or description)
 * Includes:
 * - All files explicitly listed in _quarto-manual.yml (always included)
 * - Additional QMD files with frontmatter that pass exclusion filters
 *
 * Excludes (unless in _quarto-manual.yml):
 * - figures/, includes/, templates/, operations/, test/, draft/, archive/, etc.
 * - Files with test/backup patterns in filename
 * - Auto-generated files
 * - Variant files (-academic.qmd, -foundations.qmd)
 *
 * @param options Configuration options
 * @returns Array of absolute file paths (deduplicated)
 */
export async function getAllQmdFilesWithFrontmatter(options?: {
  /** Base directory to search (default: 'knowledge') */
  basePath?: string;
  /** Additional directories to exclude */
  excludeDirs?: string[];
}): Promise<string[]> {
  const { basePath = 'knowledge', excludeDirs = [] } = options || {};

  // Start with files from _quarto-manual.yml (always included, except auto-generated)
  const bookFiles = await getBookFilesForProcessing();
  const bookFilesSet = new Set(
    bookFiles
      .map(f => path.resolve(ROOT_DIR, f))
      .filter(f => !isAutoGeneratedFile(f))
  );

  // Directories to exclude for non-book files
  const defaultExcludeDirs = [
    'figures', 'includes', 'templates', '_book', 'node_modules',
    'test', 'tests',           // Test directories
    'draft', 'drafts',         // Draft content
    'archive', 'archived', 'old',  // Archived content
    'scratch', 'temp', 'tmp',  // Temporary files
    'example', 'examples', 'sample', 'samples',  // Example content
    'operations',              // Internal operations docs
  ];
  const allExcludeDirs = [...defaultExcludeDirs, ...excludeDirs];

  // Filename patterns to exclude (case-insensitive)
  ...
  // Collect valid files (start with book files)
  const validFilesSet = new Set<string>(bookFilesSet);

  for (const filePath of allFiles) {
    const absolutePath = path.resolve(filePath);

    // Skip if already in book files
    if (bookFilesSet.has(absolutePath)) continue;

    const relativePath = path.relative(ROOT_DIR, filePath).replace(/\\/g, '/');
    const fileName = path.basename(filePath, '.qmd');

    // Skip files in excluded directories
    const inExcludedDir = allExcludeDirs.some(dir =>
      relativePath.includes(`/${dir}/`) || relativePath.startsWith(`${dir}/`)
    );
    if (inExcludedDir) continue;

    // Skip files matching excluded filename patterns
    const matchesExcludePattern = excludeFilePatterns.some(pattern => pattern.test(fileName));
    if (matchesExcludePattern) continue;

    // Skip auto-generated files
    if (isAutoGeneratedFile(filePath)) continue;

    // Skip variant files (-academic.qmd, -foundations.qmd)
    if (relativePath.endsWith('-academic.qmd') || relativePath.endsWith('-foundations.qmd')) continue;

    // Check if file has frontmatter with title or description
    try {
      const content = await fs.readFile(filePath, 'utf-8');
      const { data: frontmatter } = matter(content);

      if (frontmatter.title || frontmatter.description) {
        validFilesSet.add(absolutePath);
      }
    } catch (error) {
      // Skip files that can't be read
      console.warn(`  [WARN] Could not read ${relativePath}: ${error}`);
    }
  }

  return Array.from(validFilesSet);
}
