...
9.  **Preserve ALL Reference Links:** You MUST keep ALL existing reference links from BOTH the existing chapter and the archived file. Links in the format `[text](../references.qmd#anchor-id)` or other citation links are critical for fact-checking and credibility. Do NOT convert these to plain text or remove them.
10. **Preserve ALL External Links:** Keep all links to external sources (URLs) from both files. These provide important citations.
11. **Fix Image Paths:** All images have been moved to the `/assets/` folder at the project root. When you encounter image references like `![alt](../images/foo.png)` or `![alt](images/bar.jpg)`, you MUST update them to use the correct relative path from the target file's location to `/assets/`. Calculate the path based on {{targetFilePath}}:
