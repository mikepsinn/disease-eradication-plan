...
async function main() {
    ...
    // Final step: Compile all assessments into the strategic initiatives report
    await generateStrategicInitiatives(cache);
}

async function generateStrategicInitiatives(assessments: Record<string, z.infer<typeof ArticleAssessment>>) {
    console.log("\n📝 Compiling strategic initiatives report...");

    const refactorPlan: { action: string; path: string; details: string }[] = [];
    const contentImprovements: { path: string; suggestions: string[] }[] = [];
    let totalFilesAnalyzed = 0;
    let filesNeedingAction = 0;

    for (const filePath in assessments) {
        totalFilesAnalyzed++;
        const assessment = assessments[filePath];
        const relativePath = path.relative(ROOT_DIR, filePath);
        let needsAction = false;

        // Populate refactor plan
        if (assessment.recommendations.shouldDelete) {
            refactorPlan.push({ action: 'DELETE', path: relativePath, details: 'Reason: ' + (assessment.recommendations.shouldDelete ? 'Recommended by AI' : 'N/A') });
            needsAction = true;
        }
        if (assessment.recommendations.shouldMoveTo) {
            refactorPlan.push({ action: 'MOVE', path: relativePath, details: 'New location: ' + assessment.recommendations.shouldMoveTo });
            needsAction = true;
        }

        // Populate content improvements
        if (assessment.improvements && assessment.improvements.length > 0) {
            contentImprovements.push({ path: relativePath, suggestions: assessment.improvements });
            needsAction = true;
        }
    ...
    refactorPlan.sort((a, b) => a.path.localeCompare(b.path));
    contentImprovements.sort((a, b) => a.path.localeCompare(b.path));

    const report = {
        summary: {
            report_generated_at: new Date().toISOString(),
            total_files_analyzed: totalFilesAnalyzed,
            files_needing_action: filesNeedingAction,
        },
        refactor_plan: refactorPlan,
        content_improvements: contentImprovements,
    };

    const yamlReport = yaml.dump(report, { indent: 2 });
    const header = `# Strategic Initiatives Report
# Auto-generated by executive-function.ts
# This report outlines the AI's recommended actions to improve the wiki's structure and content.
# The 'refactor_plan' can be executed by the 'apply-initiatives.py' script.

`;
    await fs.promises.writeFile(STRATEGIC_INITIATIVES_FILE, header + yamlReport);
    console.log(`✅ Strategic initiatives report created at: ${STRATEGIC_INITIATIVES_FILE}`);
}
