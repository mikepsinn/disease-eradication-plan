...
from render_utils import BuildMonitor, kill_existing_quarto_processes, run_post_validation, run_pre_validation
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Unified HTML render script with validation, logging, and monitoring")
    parser.add_argument(
        "--timeout", type=int, default=300, help="Seconds with no output before killing build (default: 300)"
    )
    parser.add_argument(
    ...
    parser.add_argument("--skip-post-validation", action="store_true", help="Skip post-render validation")
    parser.add_argument(
        "--output-dir",
        type=str,
        default="_book/warondisease",
        help="Output directory for post-validation (default: _book/warondisease)",
    )
    parser.add_argument(
        "--log-file", type=str, default="build-html.log", help="Log file path (default: build-html.log)"
    )
    parser.add_argument(
        "--command",
        type=str,
        default="quarto render . --to html",
        help="Build command to run (default: quarto render . --to html)",
    )
    parser.add_argument(
        "--kill-existing", action="store_true", help="Kill all existing Quarto processes before starting build"
    )
