...
def _collect_config_variable_names(project_root: Path, config_name: str) -> Set[str]:
    ...
    def collect_qmd_paths(node: Any) -> None:
        ...
        if isinstance(href, str) and href.endswith(".qmd"):
            qmd_paths.add(href)
        for key in ("chapters", "contents", "appendices", "render"):
            if key in node:
    ...
    filtered_parameters = (
        ...
        / "knowledge"
        / "appendix"
        / f"parameters-and-calculations-{config_name}.qmd"
    )
    for relative_path in qmd_paths:
        ...
        if relative_path == "index.qmd" and isinstance(index_source, str):
            ...
            source_path = filtered_parameters

        if source_path.exists():
            variable_names.update(extract_variables_from_qmd(source_path, project_root))
...
def _strip_confidence_intervals(variables_path: Path, verbose: bool = True) -> int:
    ...
    interval_label = r'(?:95% CI|90% model range|modeled range)'

    # Strip intervals from display text, including custom model-range labels.
...
def prepare_build_temp(config_name: str, verbose: bool = True) -> Optional[Path]:
    ...
    # When no checked-in filtered file exists, generate one directly in the isolated build.
    ...
    if config_name != "book":
        if filtered_vars.exists():
            ...
            target_vars = build_temp / "_variables.yml"
            if main_vars.exists():
                variable_names = _collect_config_variable_names(project_root, config_name)
                variable_count = generate_filtered_variables_yml(
                    variable_names=variable_names,
                    full_variables_path=main_vars,
                    output_path=target_vars,
                )
                if verbose:
                    print(
                        f"[*] Generated filtered _variables.yml for {config_name}: "
                        f"{variable_count} entries",
                        flush=True,
                    )
            else:
