...
if average_annual_percentage_increase is not None:
    print(
        f"Average Annual % Increase between {start_year} and {last_provided_year}: {average_annual_percentage_increase:.2f}%"
    )
...
max_spending_value = max(
    max(spending_provided), max(spending_projected_filtered), max(spending_reduction), max(spending_freeze)
)
...
plt.plot(
    years_projected_filtered, spending_projected_filtered, "r-", label="Projected Status Quo", linewidth=2
)  # Projected Status Quo
plt.plot(
    years_reduction, spending_reduction, "g--", label="Projected 1% Reduction", linewidth=2
)  # Projected 1% Reduction
plt.plot(
    years_freeze, spending_freeze, "m-.", label="Projected Spending Freeze", linewidth=2
)  # Projected Spending Freeze
...
plt.plot(
    filtered_years_nih, filtered_funding_nih, "c-", label="NIH Funding (Billions)", linewidth=2
)  # Using cyan color for distinction
...
plt.text(
    midpoint_historical,
    y_historical + max_spending_value * 0.05,
    "Global\nInflation-\nAdjusted\nMilitary\nSpending\nDoubled\nSince 2000",
    fontsize=11,
    ha="left",
...
plt.text(
    midpoint_x,
    y_status_quo + max_spending_value * 0.1,
    f"Status Quo\n{average_annual_percentage_increase:.1f}% Average Annual Increase\nof ${average_absolute_annual_increase_billion:.1f} Billion/Year",
    fontsize=11,
    ha="center",
    bbox=dict(facecolor="white", edgecolor="black", pad=4),
)
plt.text(
    midpoint_x,
    y_reduction - max_spending_value * 0.25,
    "If All Nations Adopted a Treaty\nto Reduce by 1% Each Year\nWe Wouldn't Be Back Down\nto 2000-Level Spending\nUntil 2100",
    fontsize=11,
    ha="center",
    bbox=dict(facecolor="white", edgecolor="black", pad=4),
)
plt.text(
    midpoint_x + 0.1 * (projection_end_year - start_year),
    y_freeze + max_spending_value * 0.025,
    emoji.emojize("If We Froze Current Spending"),
    fontsize=11,
    ha="center",
    bbox=dict(facecolor="white", edgecolor="black", pad=4),
)
...
label_text = emoji.emojize(
    f"Average Annual Increase: ${average_absolute_annual_increase:.2f} Billion or {average_annual_percentage_increase:.2f}%"
)
plt.text(
    midpoint_x,
    max_spending_value * 9 + y_offset,
    label_text,
    fontsize=11,
    ha="center",
