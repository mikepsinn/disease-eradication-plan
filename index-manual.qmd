...
Before I explain how, one question.

Your species spends {{< var military_to_clinical_trials_spending_ratio >}} times more on weapons than clinical trials. Is it physically possible to do better than this?

If no, stop reading. You have worse problems than war.

If yes, the rest of this manual is the how.
...
Count what you just agreed to. Improvement is physically possible (you said so at the top). The benefits compound (healthier workers produce more, which funds more cures, which you just read). Politicians respond to money (that's the Super PAC). Rich people prefer not dying (that's the bonds). And with {{< var chain_global_billionaire_count_nounit >}} billionaires on the planet, at least one of them can do arithmetic. Five premises. You agreed to all of them without being asked, because each one is individually obvious. The conclusion is not a hope. It's a proof.

That's the [Logical Inevitability Theorem](/knowledge/appendix/recruitment-and-propaganda-plan.qmd#logical-inevitability-theorem) with numbers attached. Try disagreeing with any one of those [five premises](/knowledge/appendix/recruitment-and-propaganda-plan.qmd#logical-inevitability-theorem) without sounding like someone who thinks the current allocation is the best of all possible worlds.

Sharing this manual takes 30 seconds. At the global average wage, that costs you {{< var sharing_opportunity_cost >}}. If the treaty path works, your lifetime income goes up by {{< var treaty_path_lifetime_income_gain_per_capita >}}. The [ratio](knowledge/appendix/recruitment-and-propaganda-plan.qmd#how-trivial-exactly) is {{< var sharing_upside_downside_ratio_treaty >}} to one. For not sharing to be rational, the probability of success has to fall below 1 in {{< var sharing_breakeven_one_in_treaty >}}. You just agreed to all five premises. You cannot simultaneously hold those premises and a probability that low. Not forwarding this to the most powerful person you know is not caution. It is the single failure mode the math can't route around.
