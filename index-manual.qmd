...
Your treaty passes, redirecting {{< var treaty_annual_funding >}} in papers annually. Rich humans get {{< var victory_bond_annual_return_pct >}} returns on what they gave you, forever. Politicians get career advancement from voting yes (the same fund pays for both). This is a good deal because forever is a long time. Unless you die from preventable diseases. Which you're fixing, so it works out.
...
At the current discovery rate, discovering treatments will ~{{< var status_quo_queue_clearance_years >}}. Everyone currently alive will be dead by then. Pragmatic trials (testing drugs on real patients in real hospitals instead of 200 people hand-picked by pharmaceutical companies) at scale increase discovery rate {{< var dfda_trial_capacity_multiplier >}} (from {{< var new_disease_first_treatments_per_year >}} to {{< var dfda_first_treatments_per_year >}}), compressing that to ~{{< var dfda_queue_clearance_years >}}.
...
The whole thing runs on a [decentralized framework for drug assessment](/knowledge/solution/dfda.qmd) that cuts trial costs by {{< var dfda_trial_cost_reduction_factor >}} and funds {{< var dih_patients_fundable_annually >}} (versus the current {{< var current_trial_slots_available >}}). Currently, a committee of people who've never had the disease votes on whether you're allowed to try treating it. It's like requiring a building permit to put out a fire. Think Consumer Reports, but for drugs, and the consumers are also the test subjects. Voluntarily. Because they're dying anyway and would like to stop.
...
Your decentralized system collects every side effect automatically from every patient in real time. You know "12% got headaches, 3% were severe" BEFORE you take the pill, not after the class action lawsuit. The FDA doesn't publish these numbers at all. They make you guess. Somehow, having actual data is considered the dangerous option.
...
For every dollar spent on R&D alone, you get {{< var dfda_roi_rd_only >}} back. That's the conservative estimate, counting only what pharma saves on trial costs.

Factor in the lives saved by finding treatments {{< var dfda_trial_capacity_plus_efficacy_lag_years >}} sooner, and the return jumps to {{< var treaty_roi_trial_capacity_plus_efficacy_lag >}} to 1. Total economic value: {{< var dfda_trial_capacity_plus_efficacy_lag_economic_value >}}. That's the {{< var dfda_trial_capacity_plus_efficacy_lag_years >}} timeline shift from above, multiplied by the years of healthy life saved, multiplied by $150,000 (the standard economic valuation of one year of human life). Your calculator will display an error, then catch fire. This is correct.
