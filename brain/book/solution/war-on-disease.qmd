---
title: ⚕️ The War on Disease
subtitle: Designing the First War to Win
format:
  html:
    toc: true
    toc-depth: 3
lastStructureCheckHash: 0079cd0b43eae711945fd8d540fcd7efda00c8e49d09d198f3d8d4085c1b6448
lastLatexCheckHash: e3ac09e53ca13475a8994f55ff8da285a5bafec4652a11b1846cf608d864099c
---

```{python}
#| echo: false
import sys
import os

# Quarto executes from project root (execute-dir: project in _book.yml)
appendix_path = os.path.join(os.getcwd(), 'brain', 'book', 'appendix')
if appendix_path not in sys.path:
    sys.path.insert(0, appendix_path)

from economic_parameters import *
```

## On Why All Previous Wars on Abstract Concepts Failed

Every government "War on X"—Drugs, Poverty, Terror, Christmas—has been a spectacular failure.

This is not a coincidence. It's economics.

When you build a permanent bureaucracy to fight something, that bureaucracy needs the enemy to exist. Otherwise, everyone loses their jobs.

The DEA needs drug dealers. The War on Terror needs terrorists. The War on Poverty needs poor people. It's like hiring an exterminator who breeds rats on the side. Technically profitable, definitely unethical.

## On How to Design a War That Actually Ends

The War on Disease is different in one critical way: it's designed to win and then dissolve into nothingness, like a therapy session that actually works.

Here's what makes it different from every other doomed war on abstract nouns:

**On Not Having a Bureaucracy to Protect:**

The DIH runs on code, not committees. There are no headquarters. No permanent staff. No pensions to protect. No one whose mortgage depends on disease continuing to exist.

If we cure all diseases, the system just... stops. Like a fire extinguisher that doesn't need fires to justify its budget. Revolutionary concept.

**On Paying for Results Instead of Effort:**

The NIH pays scientists to write grant proposals about maybe someday potentially thinking about doing research.

The DIH pays them when sick people stop being sick.

If a researcher's cure works, they get rich. If it fails, they get nothing. This is how every other functional industry works, except government and academia, which explains a lot.

**On Competition Versus Monopoly:**

The FDA creates a single, 17-year-long pipeline to approval where only the richest companies can play.

The dFDA creates a competitive marketplace where thousands of treatments are tested simultaneously and the ones that work survive. Like evolution, except faster and with less dying.

The best ideas win based on whether they actually cure people, not based on which company bought the most congressmen.

This isn't a war of attrition. It's a war of incentives. We're not trying to make people better through moral persuasion or inspirational posters. We're just making it more profitable to cure people than to kill them or let them die slowly while extracting their savings.

It's capitalism's redemption arc, and it only took 400 years.

## On The Part Where Other Problems Accidentally Get Solved

The War on Disease isn't just about curing cancer. It's about systematically dismantling the biological roots of our other self-inflicted disasters.

Turns out, a lot of "unsolvable" social problems are just biology problems nobody bothered to solve because missiles were more exciting.

**On the War on Drugs:**

Addiction is a neurological disorder. Your brain's reward system got hijacked. It's not a moral failing, it's chemistry.

Cure the brain chemistry, and the drug trade collapses. Cartels can't sell heroin if nobody's brain wants heroin. Economics 101: no demand, no market.

We spent $1 trillion fighting drug dealers with guns. We could have spent $1 billion fixing brains with medicine. But that would have made too much sense.

**On the War on Poverty:**

A huge portion of poverty is driven by catastrophic medical costs and lost productivity from chronic illness.

Healthy people can work. Sick people can't. Dead people definitely can't. This shouldn't be controversial, yet here we are having to explain it.

Fix the diseases, slash the medical bills, and suddenly people can afford food and rent. Revolutionary.

**On the War on Terror:**

Radicalization is often a cocktail of mental illness, trauma, social isolation, and untreated PTSD.

It's a public health problem, not a military one. You can't bomb someone into being mentally stable. Trust me, we tried. It didn't work. Cost $8 trillion though.

**On the War on Crime:**

A vast amount of crime is linked to addiction, personality disorders, impulse control issues, and even environmental factors like lead poisoning.

If you fix the brain chemistry that makes people violent and impulsive, crime rates plummet. But that would put a lot of prisons out of business, and we can't have that.

**On The Obvious Truth Nobody Wants to Admit:**

Fix the biology, and you fix the society.

It's not a panacea. You'll still have assholes. But you'll have healthy, functional assholes instead of sick, desperate ones, which is a measurable improvement.

And it's definitely more effective than building more prisons and dropping more bombs, which is what we've been doing for the past century with mixed results at best.

## On The Real Victory (Hint: It's Not Just the Curing Diseases Part)

The War on Disease is a Trojan horse.

The obvious goal is curing disease. That's what it says on the label.

The real goal is proving that there's a better way to solve large-scale human problems than building a government bureaucracy, giving it a budget, and waiting for it to fail spectacularly over several decades.

**On Why Every Previous War Failed:**

Every government "War on X" has been a disaster because it was built on:

- Central planning (people in suits guessing what might work)
- Perverse incentives (getting paid for not solving the problem)
- Monopoly on solutions (one approved way to fail)

This creates a system where success threatens everyone's job, so failure becomes the mission.

**On Why This War Will Win:**

The War on Disease will be the first government "War on X" to actually win because it's built on:

- Competition (thousands of approaches tested simultaneously)
- Aligned incentives (you get rich by actually curing people)
- Ruthless focus on results (treatments that don't work die immediately)

Success makes everyone richer, so success becomes the mission. Revolutionary.

**On What Winning Looks Like:**

**Year 1 Targets:**

- 1,000+ active trials launched (versus current 50/year)
- 100,000+ patients enrolled (versus current 5,000)
- A `{python} format_percentage(TRIAL_COST_REDUCTION_PCT)` reduction in trial costs demonstrated (from $41,000 to $500 per patient)

**Year 10 Targets:**

- 50+ diseases cured (versus current 0)
- Aging reversed in human trials (death becomes optional, subject to terms and conditions)
- Life expectancy growth rate returns to pre-1962 levels (4 years per decade instead of 2)

**On The Part Where Everything Else Gets Fixed:**

When this model succeeds—and it will, because math—the credibility of central planning will be shattered.

The public will look at the War on Disease and say "wait, why don't we do this for education? And energy? And literally everything?"

And bureaucrats will say "because we need to study it first" and people will say "shut up, we're doing it anyway."

The War on Disease isn't just the first war we'll win.

It's the last war we'll need to fight, because it will give us a template for solving everything else.

Or to put it another way: We're not just curing diseases. We're curing government's addiction to failure.

Which, ironically, is the hardest disease to cure. But if we can do it, everything else is easy.

---

::: {.callout-note}

## Next Steps

For detailed implementation, see:

- [The Decentralized FDA](dfda.qmd) - The platform enabling this war
- [Aligning Incentives](aligning-incentives.qmd) - How we get everyone on board
- [The 1% Treaty](1-percent-treaty.qmd) - The funding mechanism
:::
