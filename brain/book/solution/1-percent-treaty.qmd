...
3.  **The Prize:** That 1%—a **`{python} format_billions(TREATY_ANNUAL_FUNDING)`-a-year river of cash**—is redirected to cure all human diseases.

**How It Works:**

1.  **Funding:** Creates `{python} format_billions(TREATY_ANNUAL_FUNDING)`/year revenue stream for the DIH
2.  **Execution:** DIH subsidizes patient participation in the [dFDA](dfda.qmd)—clinical trials that are **[80x cheaper](../references.qmd#recovery-trial-`{python} TRIAL_COST_REDUCTION_FACTOR`x-cost-reduction)** than current model
...
This is how we steal `{python} format_billions(TREATY_ANNUAL_FUNDING)` a year from the military-industrial complex and give it to dying people.
...
4.  **The Prize:** That 1% - that `{python} format_billions(TREATY_ANNUAL_FUNDING)` per year - goes into a global fund to cure the diseases that are actually killing people, unlike the bombs which are mostly just sitting around getting dusty and occasionally almost ending the world by accident.
...
**For Economists**:
"[The ROI is `{python} format_roi(ROI_ALL_DIRECT_BENEFITS)`](../economics.qmd#the-complete-case-1-222-1-roi-all-direct-benefits). This beats every other public health intervention on record."
...
4.  **Invest in the fund** (`{python} format_percentage(VICTORY_BOND_ANNUAL_RETURN_PCT)` annual returns, remember?)
...
- The redirect is at `{python} format_percentage(TRIAL_COST_REDUCTION_PCT)`. The military-industrial complex now builds starships and planetary defense systems against asteroids.
