...
This is how we steal `{python} format_billions(CAPTURED_DIVIDEND)` a year from the military-industrial complex and give it to dying people.
...
4.  **The Prize:** That 1% - that `{python} format_billions(CAPTURED_DIVIDEND)` per year - goes into a global fund to cure the diseases that are actually killing people, unlike the bombs which are mostly just sitting around getting dusty and occasionally almost ending the world by accident.
...
**For Economists**:
"[The ROI is `{python} format_roi(ROI_TIER_2_COMPLETE)`](../economics.qmd#the-complete-case-1-222-1-roi-all-direct-benefits). It's the single greatest economic opportunity in human history."
...
## The Money Flow: Where `{python} format_billions(CAPTURED_DIVIDEND)` Actually Goes
...
**Input**: `{python} format_billions(CAPTURED_DIVIDEND)` annually from military budgets worldwide

**Distribution**:
The `{python} format_billions(CAPTURED_DIVIDEND)` flows into the DIH Treasury, where it is allocated directly by the people of Earth via the Wishocracy platform. There are no committees or central planners. While allocations are dynamic, initial priorities determined by the crowd are projected to focus on:
