...
The key: Coordination prevents waste. Instead of 500 cancer nonprofits all applying to the Gates Foundation for the same thing, your agents coordinate: "Org A applied for X, Org B apply for Y." This is how you raise the $200M for the referendum while cutting fundraising costs `{python} format_percentage(TRIAL_COST_REDUCTION_PCT)`.

Once the treaty passes? Your agents shift to allocating the `{python} format_billions(TREATY_ANNUAL_FUNDING)` from Wishocracy instead of begging foundations. But until then, we still live in the old world where money doesn't magically appear.
