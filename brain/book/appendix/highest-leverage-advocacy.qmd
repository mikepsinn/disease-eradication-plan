...
- **`{python} format_roi(ROI_DFDA_SAVINGS_ONLY)` ROI** ([dFDA cost-benefit analysis](dfda-cost-benefit-analysis.qmd))
...
A: It's not arrogance when you have the data. [`{python} format_roi(ROI_DFDA_SAVINGS_ONLY)` ROI](dfda-cost-benefit-analysis.qmd), [80X efficiency improvement](recovery-trial.qmd), and [capturing 0.16% of `{python} format_billions(TOTAL_ENDGAME_BENEFITS_ANNUAL)` annual waste](peace-dividend-calculations.qmd) is just math.
...
- **[Cost-Benefit Analysis](dfda-cost-benefit-analysis.qmd)** - Why this is `{python} format_roi(ROI_DFDA_SAVINGS_ONLY)` ROI
