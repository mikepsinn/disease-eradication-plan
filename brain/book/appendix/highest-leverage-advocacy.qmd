...
- **Scale:** [\`{python} format_billions(CAPTURED_DIVIDEND)` annually](peace-dividend-analysis.qmd) could fund more climate action than most environmental organizations' combined budgets
...
- **Peace Dividend Math:** Capturing even 0.1% of the [\`{python} format_billions(TOTAL_ENDGAME_BENEFITS)` annual cost of violence](peace-dividend-analysis.qmd) provides more poverty reduction than traditional aid
...
A: It's not arrogance when you have the data. [`{python} format_roi(ROI_TIER_1_CONSERVATIVE)` ROI](dfda-cost-benefit-analysis.qmd), [80X efficiency improvement](recovery-trial.qmd), and [capturing 0.16% of \`{python} format_billions(TOTAL_ENDGAME_BENEFITS)` annual waste](peace-dividend-analysis.qmd) is just math.
...
Every year we delay is [63 million preventable deaths](../problem/cost-of-war.qmd#annual-excess-deaths), [\`{python} format_billions(TOTAL_ENDGAME_BENEFITS)` in waste](peace-dividend-analysis.qmd), and the continued risk of [nuclear winter](../references.qmd#nuke-winter-150tg).
