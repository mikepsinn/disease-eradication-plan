...
**Subject:** Invitation to govern `{python} format_billions(CAPTURED_DIVIDEND)` health treasury (Bitcoin-level impact)

"The initiative builds the world's largest decentralized health treasury (`{python} format_billions(CAPTURED_DIVIDEND)` annually) using proven DAO models from MakerDAO/Uniswap. Early VICTORY bond holders will govern allocation of this treasury across global health research.
...
"The 1% Treaty redirects 1% of global military spending (`{python} format_billions(CAPTURED_DIVIDEND)` annually) into health research using Oxford RECOVERY trial methods (80X lower cost). Early investors get governance control over allocation.
...
- **Return:** `{python} format_billions(CAPTURED_DIVIDEND)`+ annually in perpetuity
