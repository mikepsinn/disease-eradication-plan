...
The capital required for the [1% Treaty](1-percent-treaty.qmd) is not for ongoing operations. It's "activation energy"—the one-time push to overcome political inertia and unlock **[$27+ billion annually](../references.qmd#value-captured-by-1pct-treaty)** for global health.
...
- Capitalize [DIH](../solution/dih.qmd) treasury for first year
...
- **Legal fees:** Success fees paid in [VICTORY bonds](../economics/victory-bonds.qmd) upon ratification
...
**Subject:** Invitation to govern `{python} format_billions(TREATY_ANNUAL_FUNDING)` health treasury (Bitcoin-level impact)

"The initiative builds the world's largest decentralized health treasury (`{python} format_billions(TREATY_ANNUAL_FUNDING)` annually) using proven DAO models from MakerDAO/Uniswap. Early VICTORY bond holders will govern allocation of this treasury across global health research.

This isn't just an investment - it's a chance to direct more capital than the NIH budget toward curing diseases through [80X more efficient](../references.qmd#recovery-trial-`{python} TRIAL_COST_REDUCTION_FACTOR`x-cost-reduction) decentralized trials."

### Template 2: Health Billionaires (Industry Advantage)

**Subject:** 2-5X your R&D budget (backed by government treaties)

"The 1% Treaty redirects 1% of global military spending (`{python} format_billions(TREATY_ANNUAL_FUNDING)` annually) into health research using Oxford RECOVERY trial methods (80X lower cost). Early investors get governance control over allocation.

For [Company], this means 2-5X more R&D funding for the same work you're already doing, plus governance votes on priority areas."
...
| **Platform & Tech MVP**           | $50M        | $75M        | Engineering costs with `{python} format_percentage(TRIAL_COST_REDUCTION_PCT)` cash / `{python} format_percentage(TRIAL_COST_REDUCTION_PCT)` token split     |
| **Legal & Framework (Pilots)**    | `{python} format_millions(DFDA_ANNUAL_OPEX)`        | $60M        | Top-tier law firms with success fees in VICTORY bonds |
...
| **Global Legal Expansion**          | `{python} format_millions(DFDA_ANNUAL_OPEX)`        | $80M        | Success-fee-based retainers in all target nations      |
...
- Average competitive U.S. Senate campaign: [$50-75M](../references.qmd#average-us-senate-campaign-cost)
...
This isn't traditional fundraising, it's political arbitrage. The approach adopts the defense industry's massively profitable playbook: turn [$14 million in lobbying into $70.8 billion in contracts](../references.qmd#military-lobbying-returns).
...
- **Return:** `{python} format_billions(TREATY_ANNUAL_FUNDING)`+ annually in perpetuity
