...
5.  VICTORY Bonds are pure capitalism (`{python} format_percentage(VICTORY_BOND_ANNUAL_RETURN_PCT)` returns).
...
**Objection**: "`{python} format_percentage(VICTORY_BOND_ANNUAL_RETURN_PCT)` returns? Curing disease? This sounds like a scam."

**Response**:

**The returns** are high because:

- We're capturing value from a $109T disease burden. The economic benefits work out to over $7.5 million per person globally over a 50-year period (`{python} format_billions(TOTAL_COMPLETE_BENEFITS_ANNUAL)` annual benefit ÷ 8B people = $152K/person/year)
- 10% of `{python} format_billions(TREATY_ANNUAL_FUNDING)` yearly inflows
...
- `{python} TRIAL_COST_REDUCTION_FACTOR`x cost reduction
...
**Objection**: "You create a `{python} format_billions(TREATY_ANNUAL_FUNDING)` fund, politicians will redirect it to their friends."
...
Here's how you do it: you raise a [`{python} format_billions(TREATY_CAMPAIGN_TOTAL_COST)` war chest through VICTORY Bonds](../economics/victory-bonds.qmd), then allocate [$250 million specifically for lobbying](../economics/campaign-budget.qmd#ai-assisted-lobbying-250m-25) - enough to completely overwhelm the defense industry's $300 million annual spend. Then, you go to the same K-Street lobbying firms the defense contractors use, and you outbid them for their top talent.
...
**Science works through experimentation.** The dFDA lets us experiment `{python} TRIAL_COST_REDUCTION_FACTOR`x faster and cheaper.
...
- Need `{python} format_billions(TREATY_CAMPAIGN_TOTAL_COST)` bonds to fund campaign
...
- Continuing to spend `{python} format_billions(GLOBAL_MILITARY_SPENDING_ANNUAL_2024)` on weapons while sitting on 13,000 nuclear warheads, enough to cause an extinction-level event 130 times over.
...
**Response**: This is a solved problem. We combine multiple proven methods:
...
**Response**: That's old tech. The DIH will be built on modern, energy-efficient **[Proof-of-Stake (PoS) systems, which use over 99.9% less energy](../references.qmd#pos-energy-efficiency)** than the early Proof-of-Work blockchains. Our commitment to global health includes planetary health.
...
**Response**: We're using voting systems designed to prevent this. **Quadratic voting** makes each additional vote progressively more expensive, making it impossible for any single entity to dominate. We can also implement identity-based voting power based on reputation and participation, not just capital. The goal is a system governed by its active stakeholders, not just its wealthiest ones.
...
**Response**: The [decentralized FDA](../solution/dfda.qmd) model is **[80 times more efficient](../references.qmd#recovery-trial-`{python} TRIAL_COST_REDUCTION_FACTOR`x-cost-reduction)**. It uses pragmatic trials costing [$500 per patient](../references.qmd#recovery-cost-500) versus [$41,000 in traditional trials](../references.qmd#trial-cost-41k). The NIH's RECOVER initiative spent $1.6B for zero completed trials in 4 years. With that same budget, our model could run 2,800 trials for 2.8 million patients. The evidence is overwhelming.
...
**Response**: It's about reallocating finite resources, not just printing more money. The world's top scientists, engineers, and manufacturing capacity are a limited resource. A **direct, treaty-bound reallocation** forces a true shift. By cutting 1% from military budgets, we're not just moving dollars; we're freeing up the physicists and data scientists from building weapons to curing Alzheimer's.

## "What About Defense Industry Jobs?"

**Objection**: "This will destroy millions of jobs in the defense industry."

**Response**: This is about redirecting waste into a new engine for job creation. The U.S. contribution is less than 0.5% of the [trillions the Pentagon already can't account for](../references.qmd#pentagon-unaccounted-2-5t). We're taking that wasted money to create jobs in medicine. The same engineers building guidance systems can build next-generation medical imaging devices. This is a net job creator.

## "This Is Disrespectful to Soldiers"

**Objection**: "This plan is anti-military and disrespects the troops who protect us."

**Response**: This plan *supports* soldiers by attacking the budgetary waste that fails them. When trillions are unaccounted for, that's money not buying better equipment or support for troops. We're redirecting a fraction of that waste to protect soldiers and their families from the threats far more likely to harm them, like pandemics and chronic disease.
...
**Response**: We hire good lawyers and keep everything separate.
