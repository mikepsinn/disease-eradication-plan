...
- **Saves ~[\`{python} format_billions(DFDA_GROSS_SAVINGS_ANNUAL)` Annually](#gross-r-d-savings-from-dfda-implementation)**: Reduces global clinical trial costs (~[\`{python} format_billions(GLOBAL_TRIAL_MARKET)`/year market](../references.qmd#clinical-trial-spending-dfda-baseline)) by a baseline of **[`{python} format_percentage(TRIAL_COST_REDUCTION_PCT)`](#gross-r-and-d-savings-from-dfda-implementation)**, with up to **[95%](#gross-r-and-d-savings-from-dfda-implementation)** savings in optimal scenarios.
...
For this analysis, we use a conservative baseline estimate of **[\`{python} format_billions(GLOBAL_TRIAL_MARKET)` per year in global clinical trial spending that is potentially addressable by dFDA](#example-parameterization)** through decentralization, automation, and real-world data integration. This figure accounts for future market growth and the expanding scope of trials that could benefit from dFDA methodologies.
...
- **Range Used in Sensitivity Analysis**:
  - Conservative: 30% (saving $30B annually from a [`{python} format_billions(GLOBAL_TRIAL_MARKET)` addressable spend](#market-size-and-impact))
  - Base Case: [**`{python} format_percentage(TRIAL_COST_REDUCTION_PCT)` (saving `{python} format_billions(DFDA_GROSS_SAVINGS_ANNUAL)` annually)**](#example-parameterization) from a [`{python} format_billions(GLOBAL_TRIAL_MARKET)` addressable spend](#market-size-and-impact)
  - Optimistic: 70% (saving $70B annually) from a [`{python} format_billions(GLOBAL_TRIAL_MARKET)` addressable spend](#market-size-and-impact)
  - Transformative (RECOVERY Trial-like): 95% (saving $95B annually) from a [`{python} format_billions(GLOBAL_TRIAL_MARKET)` addressable spend](#market-size-and-impact)
...
- **Baseline Assumptions for R&D Savings (same as before):**
  - Global Clinical Trial Spending Addressable by dFDA: **`{python} format_billions(GLOBAL_TRIAL_MARKET)` / year**.
...
This table analyzes the ICER for the dFDA Infrastructure by varying key assumptions. Global Clinical Trial Spending Addressable by dFDA is assumed at `{python} format_billions(GLOBAL_TRIAL_MARKET)`/year (leading to `{python} format_billions(DFDA_GROSS_SAVINGS_ANNUAL)` gross savings in the `{python} format_percentage(TRIAL_COST_REDUCTION_PCT)` reduction scenario) unless otherwise specified.
