...
- **Range Used in Sensitivity Analysis**:
  ...
  - Base Case: [**`{python} format_percentage(TRIAL_COST_REDUCTION_PCT)` (saving `{python} format_billions(DFDA_GROSS_SAVINGS)` annually)**](#example-parameterization) from a [`{python} format_billions(GLOBAL_TRIAL_MARKET)` addressable spend](#market-size-and-impact)
...
- **Potential Savings**: `{python} format_percentage(TRIAL_COST_REDUCTION_PCT)` reduction implies [`{python} format_billions(DFDA_GROSS_SAVINGS)`/year saved](#gross-r-and-d-savings-from-dfda-implementation) if the entire industry migrated.
...
- **Net Annual Savings** (assuming full adoption and [`{python} format_percentage(TRIAL_COST_REDUCTION_PCT)` R and D cost reduction](#gross-r-and-d-savings-from-dfda-implementation)): `{python} format_billions(DFDA_GROSS_SAVINGS)`/year.
...
- Net Annual Savings: [`{python} format_billions(DFDA_GROSS_SAVINGS)`/year](#gross-r-and-d-savings-from-dfda-implementation) (from `{python} format_percentage(TRIAL_COST_REDUCTION_PCT)` R&D cost reduction).
...
- **Base Case Scenario:** `{python} format_qalys(DFDA_QALYS_ANNUAL)` QALYs/year
...
This table analyzes the ICER for the dFDA Infrastructure by varying key assumptions. Global Clinical Trial Spending Addressable by dFDA is assumed at `{python} format_billions(GLOBAL_TRIAL_MARKET)`/year (leading to `{python} format_billions(DFDA_GROSS_SAVINGS)` gross savings in the `{python} format_percentage(TRIAL_COST_REDUCTION_PCT)` reduction scenario) unless otherwise specified.
...
| ---------------------------------------------------- | -------------------------- | -------------------------- | ----------------------------- | ------------------------------- | --------------------------- | --------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Base Case: Core Platform Ops Only**                | **`{python} format_percentage(TRIAL_COST_REDUCTION_PCT)`** (`{python} format_billions(DFDA_GROSS_SAVINGS)` Savings)     | **$0.01875B ($18.75M)**    | **-$49.981B**                 | **840,000**                     | **-$59,501**                | **Dominant**                            | Ops cost from the ['Annual Operational Costs'](#annual-operational-costs-illustrative-at-target-scale-of-5m-mau-50tb-ingest-month) ROM midpoint. QALYs from [new base model](#appendix-detailed-qaly-calculation-model).                                                              |
| Core Platform + Medium Broader Initiative            | `{python} format_percentage(TRIAL_COST_REDUCTION_PCT)` (`{python} format_billions(DFDA_GROSS_SAVINGS)` Savings)         | $0.04005B ($40.05M)        | -$49.96B                      | 840,000                         | -$59,476                    | Dominant                                | Ops from ['Annual Operational Costs'](#annual-operational-costs-illustrative-at-target-scale-of-5m-mau-50tb-ingest-month) + ['Scenario Based ROM Estimates for Broader Initiative Costs'](#scenario-based-rom-estimates-for-broader-initiative-costs) (Medium), aligns with ROI calc. |
| Total Ecosystem (Low-Medium Cost)                    | `{python} format_percentage(TRIAL_COST_REDUCTION_PCT)` (`{python} format_billions(DFDA_GROSS_SAVINGS)` Savings)         | $0.5B ($500M)              | -$49.5B                       | 840,000                         | -$58,929                    | Dominant                                | Illustrative total ecosystem cost.                                                                                                                                                                                                                                                    |
| Total Ecosystem (High Cost, e.g. w/ Part. Comp.)     | `{python} format_percentage(TRIAL_COST_REDUCTION_PCT)` (`{python} format_billions(DFDA_GROSS_SAVINGS)` Savings)         | $5B                        | -$45B                         | 840,000                         | -$53,571                    | Dominant                                | Illustrative high total ecosystem cost (as prior base).                                                                                                                                                                                                                               |
