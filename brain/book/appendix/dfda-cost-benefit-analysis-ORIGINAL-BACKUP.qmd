...
- **Saves ~[\`{python} format_billions(DFDA_GROSS_SAVINGS)` Annually](#gross-r-d-savings-from-dfda-implementation)**: Reduces global clinical trial costs (~[\`{python} format_billions(GLOBAL_TRIAL_MARKET)`/year market](../references.qmd#clinical-trial-spending-dfda-baseline)) by a baseline of **[50%](#gross-r-and-d-savings-from-dfda-implementation)**, with up to **[95%](#gross-r-and-d-savings-from-dfda-implementation)** savings in optimal scenarios.
...
- **Range Used in Sensitivity Analysis**:
  ...
  - Base Case: [**50% (saving `{python} format_billions(DFDA_GROSS_SAVINGS)` annually)**](#example-parameterization) from a [`{python} format_billions(GLOBAL_TRIAL_MARKET)` addressable spend](#market-size-and-impact)
...
This table analyzes the ICER for the dFDA Infrastructure by varying key assumptions. Global Clinical Trial Spending Addressable by dFDA is assumed at `{python} format_billions(GLOBAL_TRIAL_MARKET)`/year (leading to `{python} format_billions(DFDA_GROSS_SAVINGS)` gross savings in the 50% reduction scenario) unless otherwise specified.
...
| ---------------------------------------------------- | -------------------------- | -------------------------- | ----------------------------- | ------------------------------- | --------------------------- | --------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Base Case: Core Platform Ops Only**                | **50%** (`{python} format_billions(DFDA_GROSS_SAVINGS)` Savings)     | **$0.01875B ($18.75M)**    | **-$49.981B**                 | **840,000**                     | **-$59,501**                | **Dominant**                            | Ops cost from the ['Annual Operational Costs'](#annual-operational-costs-illustrative-at-target-scale-of-5m-mau-50tb-ingest-month) ROM midpoint. QALYs from [new base model](#appendix-detailed-qaly-calculation-model).                                                              |
| Core Platform + Medium Broader Initiative            | 50% (`{python} format_billions(DFDA_GROSS_SAVINGS)` Savings)         | $0.04005B ($40.05M)        | -$49.96B                      | 840,000                         | -$59,476                    | Dominant                                | Ops from ['Annual Operational Costs'](#annual-operational-costs-illustrative-at-target-scale-of-5m-mau-50tb-ingest-month) + ['Scenario Based ROM Estimates for Broader Initiative Costs'](#scenario-based-rom-estimates-for-broader-initiative-costs) (Medium), aligns with ROI calc. |
| Total Ecosystem (Low-Medium Cost)                    | 50% (`{python} format_billions(DFDA_GROSS_SAVINGS)` Savings)         | $0.5B ($500M)              | -$49.5B                       | 840,000                         | -$58,929                    | Dominant                                | Illustrative total ecosystem cost.                                                                                                                                                                                                                                                    |
| Total Ecosystem (High Cost, e.g. w/ Part. Comp.)     | 50% (`{python} format_billions(DFDA_GROSS_SAVINGS)` Savings)         | $5B                        | -$45B                         | 840,000                         | -$53,571                    | Dominant                                | Illustrative high total ecosystem cost (as prior base).                                                                                                                                                                                                                               |
