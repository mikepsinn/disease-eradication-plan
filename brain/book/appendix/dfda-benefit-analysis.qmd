...
- **Range Used in Sensitivity Analysis**:
  ...
  - Base Case: **`{python} format_percentage(TRIAL_COST_REDUCTION_PCT)` (saving `{python} format_billions(DFDA_GROSS_SAVINGS)` annually)** from a [`{python} format_billions(GLOBAL_TRIAL_MARKET)` addressable spend](../references.qmd#clinical-trial-spending-dfda-baseline)
