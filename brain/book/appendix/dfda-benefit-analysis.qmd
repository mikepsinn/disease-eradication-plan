...
- **Range Used in Sensitivity Analysis**:
  - Conservative: 30% (saving $30B annually from a [`{python} format_billions(GLOBAL_TRIAL_MARKET)` addressable spend](../references.qmd#clinical-trial-spending-dfda-baseline))
  - Base Case: **`{python} format_percentage(TRIAL_COST_REDUCTION_PCT)` (saving `{python} format_billions(DFDA_GROSS_SAVINGS_ANNUAL)` annually)** from a [`{python} format_billions(GLOBAL_TRIAL_MARKET)` addressable spend](../references.qmd#clinical-trial-spending-dfda-baseline)
  - Optimistic: 70% (saving $70B annually) from a [`{python} format_billions(GLOBAL_TRIAL_MARKET)` addressable spend](../references.qmd#clinical-trial-spending-dfda-baseline)
  - Transformative (RECOVERY Trial-like): 95% (saving $95B annually) from a [`{python} format_billions(GLOBAL_TRIAL_MARKET)` addressable spend](../references.qmd#clinical-trial-spending-dfda-baseline)
