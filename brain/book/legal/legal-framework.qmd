...
The 1% Treaty redirects `{python} format_billions(TREATY_ANNUAL_FUNDING)` from bombs to bandages. The people who profit from bombs have lawyers. Good lawyers. Expensive lawyers. Lawyers who've put people in prison for less.
...
- **What It Does**: The financial engine for high-risk, high-return investors:
  - Issues VICTORY Bonds: Sells bonds with `{python} format_percentage(VICTORY_BOND_ANNUAL_RETURN_PCT)` target returns to investors
  - Funds the War Chest: Uses the `{python} format_billions(TREATY_CAMPAIGN_TOTAL_COST)` raised to fund expensive lobbying and media campaigns
...
3. With expectation of profits ✓ (`{python} format_percentage(VICTORY_BOND_ANNUAL_RETURN_PCT)` annually)
