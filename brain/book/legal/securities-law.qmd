...
Here's the irony: The same laws that let defense contractors sell death certificates called "bonds" will let you sell life certificates called "VICTORY bonds." You just need to structure them correctly so the SEC doesn't send you to a country club prison.
...
A detailed breakdown of the `{python} format_billions(TREATY_CAMPAIGN_TOTAL_COST)` Use of Funds is available in the [Campaign Budget](../economics/campaign-budget.qmd) chapter. Be specific. Vague use of funds = SEC investigation.
...
Once the treaty passes and bonds are paying `{python} format_percentage(VICTORY_BOND_ANNUAL_RETURN_PCT)` returns:
...
The best part: Once investors make `{python} format_percentage(VICTORY_BOND_ANNUAL_RETURN_PCT)` returns, they'll defend you more vigorously than any lawyer. Nothing protects you legally like making rich people richer while saving lives.
