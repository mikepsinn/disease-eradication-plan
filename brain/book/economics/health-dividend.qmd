...
## The `{python} format_roi(ROI_TIER_1_CONSERVATIVE)` Return

The dFDA Infrastructure costs **`{python} format_millions(DFDA_ANNUAL_OPEX)` per year** to run.
It saves the world **`{python} format_billions(DFDA_GROSS_SAVINGS_ANNUAL)` per year** in clinical trial costs.
That's a **`{python} format_roi(ROI_TIER_1_CONSERVATIVE)` return on investment**.
...
Global clinical trial spending: **`{python} format_billions(GLOBAL_CLINICAL_TRIAL_MARKET_ANNUAL)` per year** (and growing).
...
**For detailed calculations and sensitivity analysis**, see [Daily Opportunity Cost of Inaction](../appendix/dfda-daily-opportunity-cost.qmd).

## The Platform Cost Breakdown

The dFDA costs approximately **`{python} format_millions(DFDA_ANNUAL_OPEX)` annually** to operate at scale, with a similar one-time build cost. This includes cloud infrastructure, a lean engineering team, compliance, and global integration.

**For detailed cost analysis including ROM estimates, market comparables, and sensitivity scenarios**, see [dFDA Platform Costs](../appendix/dfda-platform-costs.qmd).

## ROI Scenarios

The `{python} format_roi(ROI_TIER_1_CONSERVATIVE)` ROI is our central estimate based on NPV analysis over 10 years. Even in worst-case scenarios (higher costs, lower adoption), the ROI remains exceptional at 66:1.

**For interactive charts, sensitivity analysis, and detailed NPV calculations**, see [Health Dividend Calculations](../appendix/health-dividend-calculations.qmd).
...
`{python} format_millions(DFDA_ANNUAL_OPEX)` to save $50 billion.
`{python} format_roi(ROI_TIER_1_CONSERVATIVE)` ROI.
...
- [Health Dividend Calculations](../appendix/health-dividend-calculations.qmd) - Interactive NPV/ROI calculator
- [dFDA Platform Costs](../appendix/dfda-platform-costs.qmd) - ROM estimates and market comparables
