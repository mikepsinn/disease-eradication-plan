...
## The `{python} format_roi(ROI_DFDA_SAVINGS_ONLY)` Return
...
That's a **`{python} format_roi(ROI_DFDA_SAVINGS_ONLY)` return on investment**.
...
The `{python} format_roi(ROI_DFDA_SAVINGS_ONLY)` ROI is our central estimate based on NPV analysis over 10 years. Even in worst-case scenarios (higher costs, lower adoption), the ROI remains exceptional at 66:1.
...
`{python} format_roi(ROI_DFDA_SAVINGS_ONLY)` ROI.
