...
Create a funding mechanism that raises `{python} format_billions(CAMPAIGN_TOTAL_COST)` to finance the [1% Treaty](../appendix/1-percent-treaty.qmd) campaign through perpetual revenue-sharing bonds.

**The structure to build:**

- Raise `{python} format_billions(CAMPAIGN_TOTAL_COST)` from bond investors
...
### Phase 1: Raise the Initial `{python} format_billions(CAMPAIGN_TOTAL_COST)`
...
Deploy the `{python} format_billions(CAMPAIGN_TOTAL_COST)` raised according to the [Campaign Budget](campaign-budget.qmd):
...
- @sec-campaign-budget - How to allocate the `{python} format_billions(CAMPAIGN_TOTAL_COST)`
...
**Remember the core mechanism:** Raise `{python} format_billions(CAMPAIGN_TOTAL_COST)` → Treaty redirects `{python} format_billions(TREATY_ANNUAL_FUNDING)` annually → Pay bondholders 10% (`{python} format_billions(VICTORY_BOND_ANNUAL_PAYOUT)`) → Fund research with 90% ($24.3B)
