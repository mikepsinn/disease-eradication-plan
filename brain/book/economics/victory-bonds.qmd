...
- Once treaty passes, `{python} format_billions(CAPTURED_DIVIDEND)` annually flows from military budgets to [DIH](../solution/dih.qmd)
...
- `{python} format_billions(CAPTURED_DIVIDEND)` annually flows from military budgets to DIH treasury
...
**What the 1% Treaty redirects**: 1% = `{python} format_billions(CAPTURED_DIVIDEND)`/year to medical research
...
|------|----------|----------------|-------------------|--------------|
| 1-3  | 1%       | `{python} format_billions(CAPTURED_DIVIDEND)`           | `{python} format_billions(VICTORY_BOND_ANNUAL_PAYOUT)`             | `{python} format_percentage(VICTORY_BOND_ANNUAL_RETURN_PCT)`         |
| 4-7  | 2%       | $54B           | $5.4B             | 540%         |
| 8-10 | 5%       | $135B          | $13.5B            | 1,350%       |
| 10+  | 10%+     | $270B+         | `{python} format_billions(CAPTURED_DIVIDEND)`+             | 2,700%+      |
...
1. **`{python} format_billions(CAPTURED_DIVIDEND)`+ annual treaty revenue stream** (starts year 1 if treaty passes)
...
### Scenario A: Full Success (`{python} format_billions(CAPTURED_DIVIDEND)`/year)

Show investors what full treaty adoption looks like:

**Treaty adoption**: 100+ countries (all major military powers)
**Annual DIH revenue**: `{python} format_billions(CAPTURED_DIVIDEND)`
...
"The returns are high because the political risk is high. This is a venture-scale bet on political change. The returns are not generated from new investors, but from a share of a new, legally mandated `{python} format_billions(CAPTURED_DIVIDEND)` annual revenue stream created by the treaty. The model is based on the proven economics of political lobbying, redirected toward a humanitarian goal."
...
**Remember the core mechanism:** Raise `{python} format_billions(CAMPAIGN_TOTAL_COST)` → Treaty redirects `{python} format_billions(CAPTURED_DIVIDEND)` annually → Pay bondholders 10% (`{python} format_billions(VICTORY_BOND_ANNUAL_PAYOUT)`) → Fund research with 90% ($24.3B)
