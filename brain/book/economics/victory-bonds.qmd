...
Create a funding mechanism that raises `{python} format_billions(TREATY_CAMPAIGN_TOTAL_COST)` to finance the [1% Treaty](../solution/1-percent-treaty.qmd) campaign through perpetual revenue-sharing bonds.

**The structure to build:**

- Raise `{python} format_billions(TREATY_CAMPAIGN_TOTAL_COST)` from bond investors
- Use funds to execute the treaty campaign
- Once treaty passes, `{python} format_billions(TREATY_ANNUAL_FUNDING)` annually flows from military budgets to [DIH](../solution/dih.qmd)
- Pay bondholders 10% of treaty inflows perpetually
- Use remaining 90% for medical research

**Investor pitch:** `{python} format_percentage(VICTORY_BOND_ANNUAL_RETURN_PCT)` annual returns if treaty passes at full scale
...
### Phase 1: Raise the Initial `{python} format_billions(TREATY_CAMPAIGN_TOTAL_COST)`
...
Deploy the `{python} format_billions(TREATY_CAMPAIGN_TOTAL_COST)` raised according to the [Campaign Budget](campaign-budget.qmd):
...
- `{python} format_billions(TREATY_ANNUAL_FUNDING)` annually flows from military budgets to DIH treasury
...
- **Year 1**: `{python} format_billions(VICTORY_BOND_ANNUAL_PAYOUT)` distributed to bondholders
- **Year 2**: `{python} format_billions(VICTORY_BOND_ANNUAL_PAYOUT)` distributed to bondholders
- **Year 3**: `{python} format_billions(VICTORY_BOND_ANNUAL_PAYOUT)` distributed to bondholders
...
**What the 1% Treaty redirects**: 1% = `{python} format_billions(TREATY_ANNUAL_FUNDING)`/year to medical research
...
|------|----------|----------------|-------------------|--------------|
| 1-3  | 1%       | `{python} format_billions(TREATY_ANNUAL_FUNDING)`           | `{python} format_billions(VICTORY_BOND_ANNUAL_PAYOUT)`             | `{python} format_percentage(VICTORY_BOND_ANNUAL_RETURN_PCT)`         |
| 4-7  | 2%       | $54B           | $5.4B             | 540%         |
| 8-10 | 5%       | $135B          | $13.5B            | 1,350%       |
| 10+  | 10%+     | $270B+         | `{python} format_billions(TREATY_ANNUAL_FUNDING)`+             | 2,700%+      |
...
1. **`{python} format_billions(TREATY_ANNUAL_FUNDING)`+ annual treaty revenue stream** (starts year 1 if treaty passes)
...
### Scenario A: Full Success (`{python} format_billions(TREATY_ANNUAL_FUNDING)`/year)

Show investors what full treaty adoption looks like:

**Treaty adoption**: 100+ countries (all major military powers)
**Annual DIH revenue**: `{python} format_billions(TREATY_ANNUAL_FUNDING)`
**Bondholder annual payment**: `{python} format_billions(VICTORY_BOND_ANNUAL_PAYOUT)` (10%)
**Research funding**: $24.3B (90%)
**Investor ROI**: `{python} format_percentage(VICTORY_BOND_ANNUAL_RETURN_PCT)` annually
...
1. **Attract sufficient capital**: `{python} format_percentage(VICTORY_BOND_ANNUAL_RETURN_PCT)` projected returns compete with high-risk VC/hedge funds
...
- **Execution Risk:** The campaign may fail to achieve its milestones. This is mitigated by milestone-gated funding, a lean operational model, and hiring an elite, experienced team.
...
|------------|---------------|------------|------|
| **VICTORY Bonds** | **`{python} format_percentage(VICTORY_BOND_ANNUAL_RETURN_PCT)`*** | **High** | **Perpetual** |
...
"The returns are high because the political risk is high. This is a venture-scale bet on political change. The returns are not generated from new investors, but from a share of a new, legally mandated `{python} format_billions(TREATY_ANNUAL_FUNDING)` annual revenue stream created by the treaty. The model is based on the proven economics of political lobbying, redirected toward a humanitarian goal."
...
"Philanthropy is a donation with an expected financial return of zero. VICTORY Bonds are a for-profit investment instrument, structured as debt, with a projected `{python} format_percentage(VICTORY_BOND_ANNUAL_RETURN_PCT)` annual return. While the outcome is profoundly impactful, the vehicle is a financial one designed to attract capital, not donations."
...
- Returns: `{python} format_percentage(VICTORY_BOND_ANNUAL_RETURN_PCT)` annually (if full treaty passes)
...
- @sec-campaign-budget - How to allocate the `{python} format_billions(TREATY_CAMPAIGN_TOTAL_COST)`
...
**Remember the core mechanism:** Raise `{python} format_billions(TREATY_CAMPAIGN_TOTAL_COST)` → Treaty redirects `{python} format_billions(TREATY_ANNUAL_FUNDING)` annually → Pay bondholders 10% (`{python} format_billions(VICTORY_BOND_ANNUAL_PAYOUT)`) → Fund research with 90% ($24.3B)
