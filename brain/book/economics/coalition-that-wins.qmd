...
lastInstructionalVoiceHash: 335685e8b6722a1c65ea5be260bb01a7a3eaa905cec50741960fd73f54b2bea1
---
...
- VICTORY bonds: 40-271.8% returns (better than weapons)
...
| VICTORY Bonds | 40-271.8% | "Saved humanity" |
...
```{python}
#| echo: false
#| fig-cap: "The Cascade Effect - How first movers force others to join"
#| fig-alt: "Three parallel cascades: Insurance (First Company → Lower Premiums → Competitors Match → All Join), Pharma (First Company → Faster Development → Competitors Fall Behind → All Join), Countries (First Country → Citizens Live Longer → Others Look Bad → Global Treaty)"
...
dot = graphviz.Digraph(comment='Cascade Effect', format='png')
dot.attr(rankdir='TD', size='12,12')
setup_graphviz_style(dot)

# Insurance cascade
dot.node('A', 'First Insurance Company Invests')
dot.node('B', 'Lower Premiums')
dot.node('C', 'Competitors Must Match')
dot.node('D', 'All Insurers Join')

# Pharma cascade
dot.node('E', 'First Pharma Invests')
dot.node('F', 'Faster Drug Development')
dot.node('G', 'Competitors Fall Behind')
dot.node('H', 'All Pharma Joins')

# Country cascade
dot.node('I', 'First Country Signs')
dot.node('J', 'Citizens Live Longer')
dot.node('K', 'Other Countries Look Bad')
dot.node('L', 'Global Treaty')
...
png_path = render_graphviz_with_watermark(dot, 'cascade-effect')
