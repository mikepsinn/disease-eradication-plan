# Chart Visualizations

All chart design guidelines, color palettes, typography specifications, and technical standards have been moved to the root-level **[DESIGN_GUIDE.md](../../DESIGN_GUIDE.md)**.

## Quick Links

- **Design Standards:** See [DESIGN_GUIDE.md](../../DESIGN_GUIDE.md) for complete visual design guidelines
- **Writing Style:** See [STYLE_GUIDE.md](../../STYLE_GUIDE.md) for tone and voice
- **Contributing:** See [CONTRIBUTING.md](../../CONTRIBUTING.md) for workflow and standards
- **Technical Implementation:** See [_chart_style.py](_chart_style.py) for Python code constants

## Available Charts

This directory contains all reusable chart visualizations (`.qmd` files) that can be included in presentations and book chapters using:

```markdown
{{< include brain/charts/chart-name.qmd >}}
```

Browse the files in this directory to see all available visualizations.
