...
from brain.charts._chart_style import (
    ...
    COLOR_RED, COLOR_BLUE, COLOR_DARK, COLOR_MID, COLOR_WHITE, COLOR_BG, COLOR_ACCENT,
...
bars1 = ax1.bar(categories1, values1, color=colors1, edgecolor=COLOR_WHITE, linewidth=3, alpha=0.9)
...
ax1.set_ylabel('Annual Spending (Billions)', fontsize=36, weight='bold', color=COLOR_DARK)
ax1.set_title('Direct Spending\n"You think that\'s bad?"', fontsize=48, pad=30, weight='bold', color=COLOR_DARK)
ax1.tick_params(axis='x', labelsize=32, colors=COLOR_DARK)
ax1.tick_params(axis='y', labelsize=28, colors=COLOR_MID)
ax1.yaxis.set_major_formatter(plt.FuncFormatter(lambda x, p: f'{x/1000:.1f}T' if x >= 1000 else f'{x:.0f}B'))
clean_spines(ax1, ['top', 'right'])
ax1.grid(True, alpha=0.3, color=COLOR_MID, axis='y')
...
# Colors for war stack (shades of red/dark)
war_colors = ['#c1272d', '#8b1a1f', '#5a1114', '#2d0a0b']

# Plot stacked bar for war
bottom = 0
war_bars = []
for i, (value, color) in enumerate(zip(war_stack, war_colors)):
    bar = ax2.bar(0, value, bottom=bottom, color=color, edgecolor=COLOR_WHITE, linewidth=2, alpha=0.9, width=0.8)
    war_bars.append(bar)
    ...
    if i == 0:
        ...
        label_text = f'Infrastructure: ${war_infrastructure/1000:.1f}T'
    elif i == 2:
        label_text = f'Lives + Trade: ${(war_lives + war_trade)/1000:.1f}T'
    else:
        label_text = f'Indirect: ${war_indirect/1000:.1f}T'
...
# Plot cure bars
ax2.bar(1, cure_values[0], color=COLOR_BLUE, edgecolor=COLOR_WHITE, linewidth=3, alpha=0.9, width=0.8)
ax2.bar(2, cure_values[1], color=COLOR_BLUE, edgecolor=COLOR_WHITE, linewidth=3, alpha=0.9, width=0.8)
...
# Total label on war stack
ax2.text(0, war_total * 1.02, f'$9.7T TOTAL', ha='center', va='bottom',
        fontsize=48, weight='bold', color=COLOR_RED)

ax2.set_xticks([0, 1, 2])
ax2.set_xticklabels(categories2)
ax2.set_ylabel('Annual Cost (Billions)', fontsize=36, weight='bold', color=COLOR_DARK)
ax2.set_title('True War Costs\n"Here\'s what that $2.7T ACTUALLY buys"', fontsize=48, pad=30, weight='bold', color=COLOR_DARK)
ax2.tick_params(axis='x', labelsize=32, colors=COLOR_DARK)
ax2.tick_params(axis='y', labelsize=28, colors=COLOR_MID)
ax2.yaxis.set_major_formatter(plt.FuncFormatter(lambda x, p: f'{x/1000:.1f}T' if x >= 1000 else f'{x:.0f}B'))
clean_spines(ax2, ['top', 'right'])
ax2.grid(True, alpha=0.3, color=COLOR_MID, axis='y')
...
# Plot war total
ax3.bar(0, war_total, color='#c1272d', edgecolor=COLOR_WHITE, linewidth=3, alpha=0.9, width=0.8)
ax3.text(0, war_total * 0.5, f'$9.7T', ha='center', va='center',
        fontsize=42, weight='bold', color=COLOR_WHITE)

# Plot stacked disease bar
bottom = 0
disease_bars = []
for i, (value, color) in enumerate(zip(disease_stack, disease_colors)):
    bar = ax3.bar(1, value, bottom=bottom, color=color, edgecolor=COLOR_WHITE, linewidth=2, alpha=0.9, width=0.8)
    disease_bars.append(bar)

    # Add labels for each segment
    label_y = bottom + value/2
    if i == 0:
        label_text = f'Medical: ${disease_medical/1000:.1f}T'
    elif i == 1:
        label_text = f'Lost Work: ${disease_productivity/1000:.1f}T'
    else:
...
ax3.text(1, disease_total * 1.02, f'$109T TOTAL', ha='center', va='bottom',
        fontsize=52, weight='bold', color='#8b0000')

# Plot cure bars (tiny compared to disease)
ax3.bar(2, cure_values[0], color=COLOR_BLUE, edgecolor=COLOR_WHITE, linewidth=3, alpha=0.9, width=0.8)
ax3.bar(3, cure_values[1], color=COLOR_BLUE, edgecolor=COLOR_WHITE, linewidth=3, alpha=0.9, width=0.8)
...
ax3.set_ylabel('Annual Cost (Billions)', fontsize=36, weight='bold', color=COLOR_DARK)
ax3.set_title('The Full Picture\n"And here\'s what we\'re NOT curing"', fontsize=48, pad=30, weight='bold', color=COLOR_DARK)
ax3.tick_params(axis='x', labelsize=32, colors=COLOR_DARK)
ax3.tick_params(axis='y', labelsize=28, colors=COLOR_MID)
...
ax3.grid(True, alpha=0.3, color=COLOR_MID, axis='y')
...
plt.savefig('stupidity-budget-progression.png', dpi=150, bbox_inches='tight', facecolor=COLOR_BG)
plt.show()
```
