...
- **ROI Calculation**: `{python} format_roi(ROI_DFDA_SAVINGS_ONLY)` based on `{python} format_millions(DFDA_ANNUAL_OPEX)` annual cost generating `{python} format_billions(DFDA_GROSS_SAVINGS_ANNUAL)` savings
