...
- **ROI Calculation**: `{python} format_roi(ROI_TIER_1_CONSERVATIVE)` based on `{python} format_millions(DFDA_ANNUAL_OPEX)` annual cost generating `{python} format_billions(DFDA_GROSS_SAVINGS)` savings
