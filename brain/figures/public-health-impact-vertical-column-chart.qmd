...
ax.set_xticklabels(interventions, fontsize=11)

# Value labels for traditional interventions (positioned above bars)
for i in range(3):
    ax.text(i, values[i] + 3, f'${values[i]:,.1f}B',
            ha='center', va='bottom', fontsize=13, color=COLOR_BLACK, weight='bold',
...
ax.text(x_pos, peace_dividend/2, f'${peace_dividend:.1f}B\nPeace\nDividend\nfrom 1%\nLess\nConflict',
        ha='center', va='center', fontsize=9, color=COLOR_BLACK, weight='bold',
        bbox=dict(boxstyle='round,pad=0.4', facecolor=COLOR_WHITE, edgecolor=COLOR_BLACK, linewidth=1))
# dFDA label (in middle of top segment)
ax.text(x_pos, peace_dividend + dfda_savings/2, f'${dfda_savings:.0f}B\nHealth and\nEconomic\nBenefit of a\nDecentralized\nFDA',
        ha='center', va='center', fontsize=8, color=COLOR_BLACK, weight='bold',
        bbox=dict(boxstyle='round,pad=0.4', facecolor=COLOR_WHITE, edgecolor=COLOR_BLACK, linewidth=1))
...
# Add title
ax.set_title('Gross Annual Economic Benefit of the 1% Treaty and Decentralized FDA', fontsize=16, color=COLOR_BLACK, weight='bold', pad=20)
...
ax.annotate('Only the 1% Treaty\nis self-funding\n(generates wealth\nwhile saving lives)',
           ...
           ha='center', fontsize=9, color=COLOR_BLACK, weight='bold',
...
plt.savefig(output_dir / 'public-health-impact-vertical-column-chart.png', dpi=200, bbox_inches=None, facecolor=COLOR_WHITE)
