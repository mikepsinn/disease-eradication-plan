...
Everyone cuts 1% at the same time. Your national security actually *increases*, because everyone has 1% fewer missiles pointed at them. And if you still feel like doing war, $2.69T is still enough to murder every man, woman, and child on Earth 20 times, which should be more than sufficient.
...
> A dominant assurance contract structured as a standing prize. Deposit $1; get it back at 4.18x if nothing improves; share a $29.6T prize pool if global median health and income rise above verified thresholds. VOTE claims (earned by recruiting verified referendum participants) determine your share of the pool on success and remain tied to the verified account that earned them, creating a paid referral program with a projected base-case success payout of $12.3K per recruit. PRIZE deposits remain committed until resolution, which keeps the pool from draining the moment success starts to look plausible. The two targets: global median healthy life expectancy reaches 79 years and median income reaches $504K, representing a per-capita lifetime income gain of $14.9M. If everyone recruits two people, 28 rounds reaches 280 million (the Chenoweth passage threshold). The mechanism solves five problems simultaneously: nobody knows solutions exist (VOTE claims pay for educational conversations), everyone assumes they are alone (real-time scoreboard breaks pluralistic ignorance), the public cannot coordinate (committed prize capital provides the coordination mechanism), nobody knows which solutions to implement (prize pays for outcomes, not proposals), and complex reforms have no on-ramp ($1 is the entry point to everything else). The [Earth Optimization Plan v1](https://manual.warondisease.org) is the terrible starting benchmark. The prize exists to discover and cause the implementation of whatever beats it: the Earth Optimization Plan v2.
