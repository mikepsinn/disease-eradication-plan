...
### [Optimocracy: A Practical Evidence Scoreboard for Health and Wealth](https://manual.warondisease.org/knowledge/appendix/optimocracy-paper.html)

[![Optimocracy: A Practical Evidence Scoreboard for Health and Wealth](https://manual.warondisease.org/assets/og/optimocracy-og-1200x630.jpg)](https://manual.warondisease.org/knowledge/appendix/optimocracy-paper.html)

> Jurisdictions have exposed populations to different policies over decades. That variation creates useful comparisons, but it does not automatically create natural experiments or causal estimates. Optimocracy grades the available evidence, forecasts effects on typical income and healthy life years, publishes bounded recommendations, and records whether later outcomes matched the forecast. Politicians still decide. The scoreboard makes the evidence, uncertainty, decision, and result public. Acting on the scoreboard is a separate problem, so this paper compares adoption mechanisms rather than assuming one. The project's Optimal Governance Trajectory is an illustrative best-case scenario, not a forecast of what this scoreboard will recover.
...
> Centuries of public policy variation across thousands of jurisdictions create many opportunities for comparison. They do not constitute one massive natural experiment, and many comparisons cannot identify causal effects. The Optimal Policy Generator (OPG) is a research specification for grading evidence, forecasting effects on real after-tax median income and healthy life years, and producing bounded recommendations. A method such as synthetic control, difference-in-differences, or regression discontinuity supports a causal grade only when the application passes the design's assumptions and diagnostics. OPG can produce ENACT, REPLACE, REPEAL, and MAINTAIN recommendations when the evidence clears a declared threshold. Weak evidence, unsupported extrapolation, or unresolved severe harm instead produces PILOT, INSUFFICIENT EVIDENCE, or DO NOT RECOMMEND. Each output includes raw effects on both metrics, an evidence grade, uncertainty, blocking factors, and a prospective evaluation date. The framework is agnostic to which party enacted each policy. The project's large welfare and GDP figures are illustrative best-case scenarios, not validated OPG forecasts or recoverable totals.
...
> Public budgets may be substantially misallocated, but the exact share and recoverable value are uncertain. Budget processes respond to lobbying and historical precedent as well as evidence of effectiveness. The Optimal Budget Generator (OBG) is a research specification for grading budget evidence and estimating effects on real after-tax median income and healthy life years. It estimates an Optimal Spending Level only when credible marginal-return curves, scaling behavior, interactions, capacity constraints, transition costs, and opportunity costs are available. Otherwise it produces a bounded range, incremental pilot, scenario, or abstention. The Budget Impact Score (BIS) records confidence in each output. It does not repair weak identification or make an extrapolated point target optimal. The project's large welfare and GDP figures are illustrative best-case scenarios, not validated OBG forecasts or recoverable totals.
