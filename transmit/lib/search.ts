/**
 * Server-side port of the chat widget's TF-IDF RAG search.
 * Mirrors the browser logic in assets/js/chat-widget.js so tests
 * exercise the exact same scoring the user will see.
 */
...
function extractSnippets(
  ...
  for (const pos of positions) {
    ...
    if (windows.length > 0 && winStart <= windows[windows.length - 1].end) {
      windows[windows.length - 1].end = Math.max(
  ...
  for (const w of windows) {
    ...
    let start = w.start < 300 ? 300 : w.start;
...
/**
 * TF-IDF search over the index — same logic as chat-widget.js searchContent().
