...
But the politicians are the secondary audience. The primary audience is you. Right now every human who thinks "obviously we should spend less on murder and more on medicine" also thinks they are the only one. They are not the only one. They are almost everyone. The referendum is the moment {{< var treaty_campaign_voting_bloc_target_nounit >}} humans look up and realize they are {{< var treaty_campaign_voting_bloc_target_nounit >}} humans. That is not a petition. That is a species finding out what it actually wants.
...
Target: **{{< var global_population_activism_threshold_pct >}} global participation** ({{< var treaty_campaign_voting_bloc_target_nounit >}}+ verified votes). That number isn't arbitrary. It's the number at which politicians start losing sleep. More on that later.
...
Your species downloaded TikTok over 4 billion times to watch strangers twerk. Getting {{< var treaty_campaign_voting_bloc_target_nounit >}} to click "yes" on not dying should be easier. It isn't, because not dying is less entertaining than twerking, but it should be, and that gap between "should be" and "isn't" is the entire marketing problem.

Each person who signs up gets a small referral bonus for bringing the next, funded by [Incentive Alignment Bonds](../solution/incentive-alignment-bonds.qmd) [@iab-paper-2025]. Target: less than $1.00 per verified participant. For comparison, acquiring a coffee customer costs $5-15. Nike pays $40 to get one person to buy shoes online. You're trying to convince someone to support not dying for less than the cost of a packet of gum. The fact that "not dying" needs a financial incentive to compete with shoes tells you everything about your species that this manual has been trying to tell you. But here we are. Gum money it is.
...
The realistic target for {{< var global_population_activism_threshold_pct >}} participation ({{< var treaty_campaign_voting_bloc_target_nounit >}} verified humans) scales alongside the [full execution plan](earth-optimization-protocol-v1.qmd#the-sequencing).
...
| **Full Campaign** | Months to years | Global push to {{< var global_population_activism_threshold_pct >}} participation, bond-funded growth |
...
The {{< var treaty_campaign_voting_bloc_target_nounit >}} target is based on the "{{< var global_population_activism_threshold_pct >}} Rule": when {{< var global_population_activism_threshold_pct >}} of a population actively participates in sustained, nonviolent action, the movement has historically won. Every time so far. Across all cultures. For centuries. This is the closest thing your species has to a cheat code, and nobody teaches it in school, because teaching it in school would make governments nervous, and governments write the curriculum.
