...
**1. Three instruments, two campaigns.** This is not a charity. It is not a donation. PRIZE tokens and VOTE tokens run the prize. Incentive Alignment Bonds (IABs) run the lobbying campaign. The instruments are complementary but separate: the prize and referendum create awareness; IABs convert awareness into political pressure.

**PRIZE tokens** fund the prize pool. Buyers deposit funds into escrow earning {{< var prize_escrow_yield_rate >}} annually via rolling locked stablecoin staking. If the plan fails to produce measurable results within {{< var prize_escrow_accumulation_years >}}, buyers get their deposit back plus interest at {{< var prize_escrow_100_return_multiple >}} (zero downside). If it succeeds, the entire pool is distributed to VOTE token holders proportional to their holdings. A conditional donation: your money only spends if someone earned it. Philanthropists buy because they want to fund success. Pessimists buy for the risk-free interest. Both grow the prize pool.
...
The **PRIZE pool** is escrowed and distributes on success, proportional to VOTE holdings. Whoever recruited the most coalition gets the most prize money.
...
1. PRIZE tokens are sold, raising the prize pool (escrowed at {{< var prize_escrow_yield_rate >}}, no single keyholder, time-delayed execution so fraud is caught before funds move). Someone discovers the [IAB mechanism](../solution/incentive-alignment-bonds.qmd) and issues bonds, raising the lobbying campaign fund.
...
4. IAB capital goes to the lobbying campaign: scorecards, PACs, legislative drafting, treaty advocacy. Bondholders self-organize much of this activity because every action that raises implementation probability raises their bond value.
