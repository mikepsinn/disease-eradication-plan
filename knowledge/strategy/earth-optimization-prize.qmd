...
The pool divides among everyone who votes, so the per-token value depends on turnout. On most planets this would require a complicated funnel model (awareness, internet access, action rate). On yours, the referral incentive collapses the funnel into a single parameter.
...
At that rate, {{< var vote_expected_participants >}} people vote over the accumulation period, exceeding the Chenoweth passage threshold ({{< var treaty_campaign_voting_bloc_target >}}) by roughly 8x. Treaty passage is implied well before the pool matures.
