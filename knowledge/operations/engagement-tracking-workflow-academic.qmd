...
description: This document describes a CRM system using Airtable for automated lead capture, classification, nurturing, and conversion tracking.
---
{{< include /knowledge/includes/setup-parameters.qmd >}}


**Purpose:** To ensure no lead is lost, no follow-up is missed, and no conversation is forgotten
...
Most movements fail not because the idea is bad, but because they lose track of people. Someone emails interest in helping. Staff intend to respond. Life happens. Two weeks later, the interested party has moved on.

This system prevents that outcome. Every person who interacts with the organization gets:

1. A permanent record
2. Automatic follow-up reminders
3. A clear path from "curious stranger" to "active contributor"

It operates as a conveyor belt for converting interest into impact.
...
**The Rule:** If it is not in Airtable, it did not happen.

#### Step 2: Classify (Automatic Tagging)

When an engagement is created, the system automatically classifies the person:
...
Not all leads are equal. Route to the appropriate person:

| Engagement Type | Routed To | SLA (Response Time) |
|----------------|-----------|-------------------|
| VICTORY Incentive Alignment Bond Interest > $100K | Founder | 4 hours |
| VICTORY Incentive Alignment Bond Interest < $100K | Finance Lead | 24 hours |
...
People need multiple contacts before they act. Build sequences:
...
- **Day 3:** Check if they have referred anyone
  - If YES: Send "You're a Hero" email, upgrade to Tier 2
  - If NO: Send "Share with 3 Friends" reminder email
  - Log engagement: Type = "Email"

- **Day 7:** Check again
  - If they have referred 3+: Upgrade to Tier 2, send congrats
  - If they have referred 1-2: Send "Almost There" email
  - If they have referred 0: Send final reminder with new angle
...
##### Sequence 2: VICTORY Incentive Alignment Bond Prospect → Committed Investor
...
- **Hour 4:** If no response
  - Slack alert: "URGENT: High-value lead becoming inactive"

- **Day 1:** Send pitch deck + FAQ
  - Log engagement: Type = "Email"

- **Day 3:** Follow-up call invite
  - "Questions are welcome. Calendar link here."
...
- **Day 30:** Final touch
  - "We are moving forward. Last chance to get in at this valuation."

##### Notes

- High-touch, rather than automated blasts
...
Conversion rates are tracked at each step:
...
Patterns requiring human intervention are monitored:

##### Alert Triggers

1. **High-Value Lead Becoming Inactive**
...
For sophisticated email sequences, integration is recommended with:
...
**Recommendation for Pre-Seed:** Start with Option A, graduate to Option B at 1,000+ contacts.
...
2. **Outreach Planning**
   - Identify top 10 prospects who have not engaged
...
Actions:
  1. Send email
     ...
     Subject: Your Vote for a 1% treaty is Recorded
     Body:
       Hi {Full Name},

       Your vote has been recorded. You're now part of the {{< var global_population_activism_threshold_pct >}}
...
Name: "High-Value VICTORY Incentive Alignment Bond Lead - Alert Founder"

Trigger: When record created in table "VICTORY Incentive Alignment Bonds"
...
<!--        [Open in Airtable]({Record URL}) -->
  ...
  2. Send email
     ...
     Subject: 🚨 URGENT: $100K+ VICTORY Incentive Alignment Bond Interest
...
Actions:
  1. Send email
     ...
     Body:
       ...
       We're {{< var global_population_activism_threshold_pct >}} of the way to the tipping point. You can push us closer.
...
**Problem:** Sending too many emails, appearing like spam
**Solution:** Cap at 1 email/week per person unless they engage

#### 2. Under-Tracking

**Problem:** Conversations occurring outside Airtable, losing follow-ups
**Solution:** "If it is not logged, it did not happen" rule

#### 3. No Segmentation

**Problem:** Sending the same message to a $10 donor and a $100K investor
**Solution:** Create segments based on engagement value/type

#### 4. Ignoring Unsubscribes

**Problem:** Legal violations, angry people
**Solution:** Honor unsubscribes immediately; it is the law
...
Once 1,000+ engagements are logged, AI can be trained to:
...
2. **Suggest Next Best Action**
   - Based on engagement history, predict the next step
...
It is the difference between a movement that fizzles and one that reaches the {{< var global_population_activism_threshold_pct >}} tipping point.

**Implementation is critical. The first 100 conversations will set the template for the next 100 million.**