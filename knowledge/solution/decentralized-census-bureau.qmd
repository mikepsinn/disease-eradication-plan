...
function citizenCount() external view returns (uint256) {
    return citizenList.length;  // live count, sybil-resistant, costs nothing to read
}
```

One line. Returns in 50 milliseconds, updates the instant someone registers, and never goes stale.
...
The count is also the least important thing this bureau measures, which is a strange thing to say about a Census Bureau, but here we are.
