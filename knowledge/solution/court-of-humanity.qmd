...
| Ledger | Measure | What it establishes |
|---|---|---|
| One. Direct killing through war | {{< var war_deaths_since_1900_nounit >}} deaths since 1900 | Traditional mass-fatality exposure. |
| Two. Regulatory delay | {{< var existing_drugs_efficacy_lag_deaths_total_nounit >}} deaths since 1962 | A forum for deaths hidden inside delay. |
| Count Three, historical | Disease-control dates in the 1900 redirect counterfactual | The drugs and public-health systems that did not arrive because the money went to war. |
| Count Three, forward | {{< var dfda_trial_capacity_plus_efficacy_lag_lives_saved_nounit >}} future deaths prevented by the treaty remedy | The value of settling now instead of continuing the offense. |
| **Aggregate exposure** | Separate ledgers above | Too large for ordinary compensation; useful as settlement leverage. |

The table deliberately does not add every line into one central damages number. VSL deaths, DALYs, lost GDP, and punitive multipliers are different legal theories. Add them all together and the spreadsheet starts lying.
