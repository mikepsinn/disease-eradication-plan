...
Keeping ledgers separate does not mean the prosecution lacks a demand. It means the Court prevents double recovery at judgment while allowing the prosecutor to plead the strongest corporate-liability number. The headline base ask is {{< var corporate_damages_prosecutor_base_ask_per_capita >}} per living human. The strict floor is what a cautious court could award. The prosecutor's base ask is the aggressive corporate-liability demand. Punitive damages, disgorgement, ongoing lost-income damages, and the forward treaty settlement value are pleaded separately.

| Prosecutor posture | Per living human | Status |
|---|---:|---|
| Strict floor | {{< var corporate_damages_strict_floor_per_capita >}} | Cautious non-duplicative award. |
| Prosecutor base ask | {{< var corporate_damages_prosecutor_base_ask_per_capita >}} | Aggressive historical corporate-liability demand. |
| Treble-style exposure | {{< var corporate_damages_treble_exposure_per_capita >}} | False Claims Act-style penalty exposure. |
| Constitutional-ceiling exposure under *State Farm v. Campbell* | {{< var corporate_damages_state_farm_ceiling_exposure_per_capita >}} | Base ask plus a 9:1 punitive-to-compensatory ceiling, exposure only [@state-farm-v-campbell-2003]. |
| Ongoing lost prosperity | {{< var war_counterfactual_lost_gdp_per_capita >}}/person/year | Annual damages flow, not a one-time stock. |
