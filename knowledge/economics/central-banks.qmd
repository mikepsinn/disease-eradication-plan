...
I am not naming any names. I do not have to. Whenever a currency stops making sense, your species reliably reaches for a group to blame: foreigners, billionaires, immigrants, Jews, communists, the elites, the deplorables, the bankers, the woke, the unwoke. The list rotates by decade and by dinner party. The reason it rotates does not. The actual cause (the people who control the currency) is too abstract to fit on a protest sign, so a more visible group has to stand in. If you live somewhere a currency is unsteady and the news is full of *someone-must-pay*, reread the last two paragraphs. The pattern has a distinctive shape. It has had the same shape every time.
...
On Wishonia, our decentralized resource allocation system has one goal: "are beings' lives measurably improving?" But that's apparently too simple for a species that named its planet "dirt," so let me explain what your two goals actually do.
...
You solved bank runs in 1933 with insurance. You kept the institution that caused them. On Wishonia, when we discover the medicine is making the patient sicker, we stop administering it. On Earth, you doubled the dose and gave the pharmacist a raise.
