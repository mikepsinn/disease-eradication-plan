...
::: {.callout-note}
## This is an optional EOS instrument, not the main way to invest in EOS

If you want to own the company and make money when it wins, that is [EOS shares](investment-terms.qmd): money in, more money out. The Prize deposit described here is the opposite shape. You fund the pool; if the treaty fails you are refunded (at a multiple); if it succeeds you forfeit your principal to the people who won it and you get the world instead. It is a conditional bet for people who would rather have the outcome than the money, and a deposit is a security, offered as one (see [the regulatory section](#v1-bootstrap)). One tool in the kit, not the front door.
:::
...
- **Months 0–6.** Ship Boring and Ballast. 80% of the pool is already compounding. Configure the prize contract on Base to accept deposits and route capital to the two live slices. (Earth Optimization Points are earned through verified-voter recruitment, not minted by deposits.)
...
**The 2026 regulatory environment makes a clean, compliant launch cheaper than ever, but the deposit is a security and is offered as one.** A deposit that refunds at a multiple on the failure branch carries an expectation of profit from a pooled, managed investment. Dressing it as a "charitable gift" to place it outside securities law is exactly the kind of label-engineering that does not survive contact with a regulator, so do not. Offer the deposit through a real exemption: Reg CF is the cheapest and earliest on-ramp for ordinary depositors (portal-based, Form C), Reg A+ Tier 2 scales it ($75M a year, non-accredited allowed), Reg D 506(c) covers accredited capital with no cap. Earth Optimization Points are a separate layer and are not a security: they are earned through verified-voter recruitment, cannot be bought, and cannot be traded, which keeps them on the rewards-program side of the line (like frequent-flyer miles) rather than the investment side. FinCEN's AML/CFT program requirement for investment advisers is delayed to January 1, 2028, which lowers the compliance burden during the launch window. State-level money-transmitter rules (NY DFS, CA DFPI) still need a per-state opinion before accepting deposits in those jurisdictions. The goal is a launch that is honest about what each piece is, not one that hopes a wrapper hides it.
...
| **Lockup** | Until {{< var destructive_economy_50pct_year >}} or the [Earth Optimization Prize](../strategy/earth-optimization-prize.qmd) terminal-metric oracle triggers release, whichever comes first. |
| **Allocation during lockup** | Per the published [Prize Fund allocation policy](#what-the-fund-does) (Boring + Ballast at launch; Tokenized and Frontier slices added per timeline). |
...
| **What can go wrong** | (a) The custodian fails (mitigated: regulated custodian + segregated client funds). (b) The Prize Fund allocation underperforms (mitigated: failure-branch refund still beats a conventional retirement account at the modeled rate). (c) The terminal-metric oracle is gamed (mitigated: Optimitron five-source design, public adversarial-challenge window). |
...
The dashboard does the credibility work. An institution evaluating the fund inspects the dashboard, not a marketing deck.
...
Two things defuse it. First, depositors hold no lever to cause failure. Governance is the vote, which belongs to all humanity, one per person, and cannot be bought; the campaign's power comes from the vote count, the redirected lobbying, and [EOS shares](investment-terms.qmd), none of which a depositor controls. The short position is a private preference with no mechanism behind it. Second, the deposit selects for mission-alignment: you put money in precisely because you would rather have the world than the refund, and the refund is the consolation, not the goal.
...
- **SEC Interpretive Release on crypto assets** ([March 17, 2026](https://www.sec.gov/files/rules/interp/2026/33-11412.pdf)). The Atkins-era SEC's five-token taxonomy puts utility-like tokens outside "digital security" status. A proposed Token Safe Harbor would add startup ($5M) and fundraising ($75M) exemptions. Interpretive guidance is faster than legislation and can already be relied on, but is reversible by a future SEC chair. Most relevant to structure G (Securitize tokenized) and to confirming that the earned, non-tradeable Earth Optimization Points layer sits outside "digital security" status.
- **FinCEN AML/CFT delay for investment advisers** (now January 1, 2028). Reduces the compliance burden for structures B, C, F, G, and J during the launch window.

Structures A and I need none of these to launch. Structures B, C, and H get cheaper and broader if the INVEST Act passes. Structures G and J get cheaper and broader on multiple of these landing. Plan for both the pass and the fail case on legislation; the interpretive guidance is already in effect.

## Prize Deposit vs Shares vs IAB vs Retirement

If the prize fails, your deposit comes back at 4 to 25× what you put in, depending on which strategy the fund targets. Your retirement account makes {{< var conventional_retirement_horizon_multiple >}} over the same period. If the prize succeeds, your deposit goes to the voters, and you live in a world {{< var treaty_personal_upside_blend >}} richer per person. The deposit is the one instrument here that pays nothing on success, by design; if you want to make money *when the treaty wins*, that is [EOS shares](investment-terms.qmd), shown in the table below for contrast.
...
|:---|:---|:---|:---|
| **[EOS shares](investment-terms.qmd)** | Share appreciation (you make money when it wins) | Hold defense stocks at about market, fee-free | Yes |
| **Prize deposit** | Pool routes to Earth Optimization Points holders (deposit forfeited) | 4× – 25× depending on strategy | No |
