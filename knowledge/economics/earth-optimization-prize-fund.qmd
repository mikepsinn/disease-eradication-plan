...
**The 2026 regulatory environment makes the cheapest path also the safest one.** Structure the v1 pool as restricted charitable gifts to a 501(c)(3) (the existing host nonprofit) with refund provisions tied to objective oracle conditions. Under the Atkins-era SEC's [March 2026 Interpretive Release](https://www.sec.gov/files/rules/interp/2026/33-11412.pdf), this arrangement is well outside the "digital security" category of the new five-token taxonomy: depositors do not expect profit from others' efforts; the "return" is treaty passage and disease eradication, which are public goods, not investor profit. FinCEN's AML/CFT program requirement for investment advisers is delayed to January 1, 2028 and the agency is reconsidering scope. A token-based VOTE points layer can be issued through a Wyoming DAO LLC alongside the charitable wrapper and qualifies as a "digital tool" under the new taxonomy (utility-based, earned through verified-voter recruitment, not pre-purchased as an investment). The result: a launch path that's 90% cheaper and 95% faster than the same launch in 2024 would have been. State-level money-transmitter rules (NY DFS, CA DFPI) are still the bright lines and need a per-state opinion before accepting deposits in those jurisdictions; everything else has loosened.
...
| **VOTE points** | You earn VOTE points by recruiting verified treaty-register signups through your referral link, not by depositing. Deposits do not buy VOTE points. (See [Earth Optimization Prize](../strategy/earth-optimization-prize.qmd) for the recruitment mechanism.) |
| **Success branch** | If the Scoreboard targets are hit, your principal routes pro-rata to all VOTE point-holders. You do not get your money back; you get the world that the pool produced. |
...
| **Beneficiary** | You designate one at deposit; update any time before death. PRIZE tokens transfer per the [protocol spec](../appendix/earth-optimization-prize-protocol.qmd). |
...
- **VOTE point ledger.** Total points minted, points per major referrer (opt-in display), projected per-point payout if targets are hit.
...
Capital providers get 4 to 25× on treaty failure and zero on success. Voters get paid on success. Put them in the same vehicle and you have capital allocators with a direct financial interest in the referendum losing. A short position on your own mission.
