...
Run the argument forward and it ends in one sentence. The good infinity runs on biotechnology; biotechnology runs on resources; and the resources are not missing, they are misallocated, parked in the things that manufacture the bad infinity instead of the good one: the weapons, the waste, the {{< var military_to_government_clinical_trials_spending_ratio >}}-to-one machine. So the rational act is not to *find* money. It is to *move* it: identify the waste and the harm in how your species currently spends, and point it at the thing that buys you out of hell and into the long pleasant stretch. That is the entire program. Everything else is one way of doing that one thing.

You do not have to pick the right one. Back any of the ten. The one with the best math is the [Loving Takeover](../appendix/loving-takeover.qmd): the smallest downside, the largest upside, and the only one that pays you back, because you end up owning the missile companies instead of just defunding them.
...
So the bet works on everyone. Believe in the infinities and you act because the payoff is unlimited. Refuse them and the boring math says yes anyway. The only way to lose is to not play. The prize is the misallocation times the correction: a {{< var military_to_government_clinical_trials_spending_ratio >}}-to-one misallocation no one audits, times any nonzero correction, is still enormous, so no amount of acting produces a small result.
