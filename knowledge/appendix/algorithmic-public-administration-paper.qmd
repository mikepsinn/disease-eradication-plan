...
Maximum workable UBI does not mean pretending one flat check can replace civilization. It means cashing out everything that is fundamentally a transfer and keeping explicit only the few functions that are not. Catastrophic health risk remains pooled through a universal catastrophic voucher or publicly financed catastrophic plan. Child education remains a per-child voucher or credit rather than adult cash. Severe disability and long-term care remain transparent top-ups rather than being buried across dozens of overlapping programs. Everything else that is basically a transfer becomes cash.

The federal layer should therefore do only four things in distribution: guarantee the universal cash floor, guarantee catastrophic portability, define explicit high-need supplements, and move money. States and localities should handle most service delivery: schools, land use, provider markets, supplementary services, and local top-ups. That is the maximum-cash design that still respects risk pooling, children, and federalism.
...
| Medicaid eligibility determination | Universal catastrophic health voucher / catastrophic public plan | Identity verifies citizenship; federal floor covers catastrophic risk |
...
The rest of the gain is not imaginary or secondary. It simply arrives in a different form. The conservative non-UBI stack, roughly **$14,900 per person per year**, shows up as cheaper housing, cheaper healthcare, lower bank spreads, lower transaction fees, less filing labor, and higher wages from output that is no longer being suppressed. The protocol therefore improves household balance sheets from both sides: more cash in and less extraction out.
...
Government is expensive because it is intermediated. Every layer between citizen preference and public outcome extracts value and introduces distortion. The US Efficiency Audit alone identifies {{< var us_gov_waste_total >}} per year in governance drag. Once recovered tax gap and excess financial intermediation rent are included, the conservative quantified stack in this paper rises to roughly **~$6.62T/year**.
