...
**The Solution:** Wishocracy eliminates the intermediary entirely by using simple pairwise comparisons ('allocate $100 between cancer research and infrastructure') to directly translate citizen preferences into budget allocations, harnessing collective intelligence to aggregate millions of intuitive trade-off judgments into resource distributions that approximate utilitarian social welfare.
...
4. **Principal-Agent Elimination:** By directly translating citizen preferences into resource allocation without intermediaries, RAPPA removes the systematic distortions introduced by representatives who face misaligned incentives.

To our knowledge, no widely deployed mechanism combines all four properties. Traditional voting captures neither intensity nor tractability. The Analytic Hierarchy Process provides tractability but has been deployed primarily for expert decision-making, not large-scale collective governance. Representative democracy introduces principal-agent distortions that systematically misalign policy with citizen preferences. RAPPA synthesizes insights from decision science and collective intelligence into a unified framework for direct democratic resource allocation.
...
Wishocracy offers a structural solution: eliminate the intermediary entirely. Through Randomized Aggregated Pairwise Preference Allocation, the mechanism translates citizen preferences directly into resource allocations without passing through representatives who can be captured, misled, or misaligned. The theoretical foundations combine the Analytic Hierarchy Process (for cognitive tractability) with collective intelligence research (for aggregation), decomposing n-dimensional preference spaces into simple pairwise comparisons that any citizen can complete.
...
Wishocracy addresses the central failure mode of democratic governance: the principal-agent problem that inevitably corrupts representative institutions. The mechanism's key innovation lies in its four-part synthesis: AHP's cognitive tractability, slider-based preference intensity capture, collective intelligence aggregation, and, most critically, the complete elimination of intermediaries between citizen preferences and resource allocation.
