---
title: 'Wishocracy: Randomized Aggregated Pairwise Preference Allocation for Democratic Resource Governance'
description: 'This paper introduces Wishocracy, a novel governance mechanism that employs Randomized Aggregated Pairwise Preference Allocation (RAPPA) to elicit and synthesize collective preferences for public resource allocation. By decomposing n-dimensional preference spaces into cognitively tractable pairwise comparisons and using randomization to ensure strategy-proofness, the mechanism overcomes human cognitive limitations while preserving preference intensity information.'
aliases:
  - /wishocracy.html
author:
  - name: Mike P. Sinn
...
**RAPPA's Novel Contribution:** Wishocracy represents the first governance mechanism to simultaneously achieve three critical properties that existing mechanisms address in isolation:
...
3. **Collective Intelligence Aggregation:** By synthesizing millions of pairwise judgments through eigenvector methods (building on collective intelligence research), the mechanism leverages diversity to cancel individual errors while aggregating true signals into welfare-maximizing allocations.

No existing mechanism combines all three properties. Traditional voting captures neither intensity nor tractability. Quadratic voting captures intensity but imposes cognitive complexity. The Analytic Hierarchy Process provides tractability but has not been deployed for large-scale collective decision-making. RAPPA synthesizes these insights into a unified framework.
...
**Inputs:** Let $N = \{1, ..., n\}$ be the set of citizens and $O = \{o_1, ..., o_m\}$ be the set of policy priorities. Each citizen $i$ provides pairwise allocation ratios $\rho_{i,A,B} \in [0,1]$ for a subset of pairs $(A,B) \in O \times O$, where $\rho_{i,A,B}$ represents the fraction of resources citizen $i$ would allocate to priority $A$ when comparing $A$ and $B$. Note that $\rho_{i,A,B} + \rho_{i,B,A} = 1$ by construction.

**Aggregation Function:** For each pair $(o_j, o_k)$, we compute the aggregate pairwise comparison using the **Geometric Mean** of individual ratios. The geometric mean is necessary to satisfy the reciprocity axiom (Aczaél & Saaty, 1983) and ensure that reciprocal judgments allow for consistent aggregation:

$$
a_{jk} = \left( \prod_{i \in S_{jk}} \rho_{i,j,k} \right)^{\frac{1}{|S_{jk}|}}
$$

where $S_{jk} \subseteq N$ is the set of citizens who evaluated the pair $(o_j, o_k)$. This produces an $m \times m$ aggregate comparison matrix $\mathbf{A} = [a_{jk}]$.

**Priority Synthesis:** Following the Analytic Hierarchy Process, we compute the principal eigenvector of $\mathbf{A}$ to derive the global priority vector $\mathbf{w} = (w_1, ..., w_m)^T$, where $w_j$ represents the relative priority weight for outcome $o_j$:

$$
\mathbf{A} \mathbf{w} = \lambda_{max} \mathbf{w}
$$

where $\lambda_{max}$ is the largest eigenvalue of $\mathbf{A}$. The weights are normalized such that $\sum_{j=1}^{m} w_j = 1$.
...
**Convergence Properties:** With random pair assignment and large $n$, the law of large numbers ensures that $a_{jk} \to E[\rho_{i,j,k}]$, the population mean preference ratio. The sample size required for $\epsilon$-accurate estimation of each $a_{jk}$ is $O(1/\epsilon^2)$ by standard concentration inequalities.

## **3.4 Solution Layer: GenieDAOs**

**Important:** GenieDAOs represent a *distinct, separate layer* from the core RAPPA mechanism. While RAPPA (Sections 3.1-3.3) handles democratic preference aggregation to determine *what* society should prioritize, GenieDAOs address the orthogonal question of *how* to achieve those priorities.

Once priorities are established through RAPPA, Wishocracy introduces this second layer for solution generation and evaluation. GenieDAOs (Decentralized Autonomous Organizations of problem-solvers) form around specific priorities to propose implementation strategies. These proposals are subject to pairwise comparison using different evaluation criteria: cost-effectiveness, feasibility, and probability of success.
...
- **Layer 2 (GenieDAOs):** Technocratic, evidence-based. "How should we do it?" Experts design and evaluate implementation strategies based on causal knowledge and empirical evidence.
...
$$
This extreme lossy compression effectively destroys all information about preference intensity and specific trade-offs (The "Bundle Problem").
...
**Theorem 1 (Divergence Minimization):** Let $D_{KL}(P || Q)$ be the Kullback-Leibler divergence between true welfare preferences $P$ and enacted policy $Q$. As $N \to \infty$:
$$
...
**Theorem 2 (Utility Dominance):** For any utility distribution $U$ with skewness $\gamma > 0$ (typical of health/wealth distributions), the Social Welfare $W$ of the RAPPA allocation exceeds that of the Median Voter allocation.
$$
W(RAPPA) \approx W(\text{Mean}) > W(\text{Median})
$$
*Proof:* The RAPPA eigenvector centrality $w_j$ corresponds to $\frac{\sum v_{ij}}{\sum \sum v_{ik}}$, which approximates the population arithmetic mean. For convex loss functions (where distinct needs exist), minimizing the sum of squared errors leads to the mean, not the median. Wishocracy thus passes the "Veil of Ignorance" test (Rawls) whereby a citizen typically prefers the Mean allocation to the Median allocation to hedge against being in the tail risk category.
...
*   **Constraint:** Wishocracy introduces $L_{Coordination}$ (the difficulty of designing solutions), which is why the **GenieDAO** layer (Section 3.4) re-introduces agents *only* for implementation, not for priority setting, minimizing the scope of potential agency loss to technical execution rather than value judgment.
...
Academic research on voting interfaces provides direct evidence for RAPPA's design choices. Wellings et al. (2024) compared cumulative voting, quadratic voting, and traditional ranking methods on Stanford's Participatory Budgeting platform. Their findings support several Wishocracy design principles.
...
Yang et al. (2024) found that voting input formats using rankings or point distribution provided a 'stronger sense of engagement in the participatory process.' These findings validate RAPPA's slider-based allocation over binary choice mechanisms.
...
Wishocracy addresses a fundamental challenge in democratic theory: how to aggregate citizen preferences over complex, multidimensional policy spaces in ways that are cognitively tractable, preference-revealing, and welfare-maximizing. The mechanism's key innovation lies in its three-part synthesis: combining AHP's cognitive tractability, quadratic voting's preference intensity capture, and collective intelligence aggregation into a unified framework. No previous mechanism has achieved all three properties simultaneously.
