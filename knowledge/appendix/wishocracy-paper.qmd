...
abstract: |
  ...
  *intensity*, not just what people want, but how much they care. Compare aggregated preferences to each
  ...
  approximate strategy-proofness. At system scale, the Wishonian Optimal Governance Trajectory reaches
...
4. **Accountability Integration:** By producing clear, quantifiable preference signals that can be compared against politician voting records, RAPPA enables accountability mechanisms (Citizen Alignment Scores, [Incentive Alignment Bonds](../solution/incentive-alignment-bonds.qmd)) that align representative behavior with citizen welfare.
...
**Scale note.** If that accountability layer works at system scale, the upside is civilization-scale rather than procedural. Under the project's best-case governance ceiling, the recoverable upside is {{< var political_dysfunction_global_opportunity_cost_total >}} per year. Over 20 years, the [Wishonian](../futures/wishonia.qmd) [Optimal Governance Trajectory](../economics/gdp-trajectories.qmd) reaches {{< var wishonia_trajectory_gdp_vs_current_trajectory_multiplier_year_20 >}} the Earth baseline, raises average income to {{< var wishonia_trajectory_avg_income_year_20 >}} versus {{< var current_trajectory_avg_income_year_20 >}} on the status-quo path, and reaches {{< var wishonia_trajectory_gdp_year_20 >}} in total output. This paper focuses on the citizen-preference and accountability layer of that plan; the full derivation lives in [The Political Dysfunction Tax](political-dysfunction-tax.qmd) [@political-dysfunction-tax-paper-2025].
...
**Scope Constraint (Domain Restriction):** RAPPA operates over a pre-defined question domain $\mathcal{D} \subseteq O$. Questions outside $\mathcal{D}$ cannot appear as comparison items. This is a mechanism property, not a political choice: the system does not accept inputs outside its domain, analogous to type safety in programming languages. When $\mathcal{D}$ is restricted to (e.g.) medical research allocation, fundamental rights questions are structurally excluded from the aggregation. This prevents the tyranny-of-majority failure mode observed in unbounded direct democracy systems such as California ballot propositions [@california-ballot-proposition-overload] and Swiss referendums on minority rights [@swiss-minaret-ban-2009].
...
### **Phase 3: Incentive Alignment (Integration with [Incentive Alignment Bonds](../solution/incentive-alignment-bonds.qmd))**
...
## **Connection to [Incentive Alignment Bonds](../solution/incentive-alignment-bonds.qmd)**

Wishocracy and [Incentive Alignment Bonds](../solution/incentive-alignment-bonds.qmd) are complementary mechanisms addressing different parts of the democratic failure:
...
- **[Incentive Alignment Bonds](../solution/incentive-alignment-bonds.qmd)** solve the *incentive alignment* problem: How do we make politicians act on citizen preferences rather than concentrated interests?
...
## **Connection to [Optimocracy](../solution/optimocracy.qmd)**

Wishocracy and [Optimocracy](https://optimocracy.warondisease.org) [@optimocracy-paper-2025] represent two distinct approaches to governance reform that can function independently or in combination:
...
- **[Optimocracy](../solution/optimocracy.qmd)** is *algorithmic*: a pre-agreed metric is optimized automatically, with minimal political discretion. The output is an allocation that executes regardless of political preferences.

These approaches address different failure modes:

| Failure Mode | Wishocracy Solution | [Optimocracy](../solution/optimocracy.qmd) Solution |
|--------------|--------------------|--------------------|
...
A hybrid architecture might use Wishocracy for *metric selection* (citizens choose what to optimize) and [Optimocracy](../solution/optimocracy.qmd) for *metric optimization* (algorithms allocate to maximize the chosen metric). This combines democratic legitimacy at the constitutional level with capture resistance at the execution level.
...
Wishocracy offers a different approach: rather than replacing representatives, align their incentives with citizen preferences. Through Randomized Aggregated Pairwise Preference Allocation, the mechanism aggregates citizen preferences into a clear signal of what voters actually want. Combined with Citizen Alignment Scores and [Incentive Alignment Bonds](../solution/incentive-alignment-bonds.qmd), this creates accountability pressure that makes welfare-improving votes politically and financially rewarding. The theoretical foundations combine the Analytic Hierarchy Process (for cognitive tractability) with collective intelligence research (for aggregation), decomposing n-dimensional preference spaces into simple pairwise comparisons that any citizen can complete.
