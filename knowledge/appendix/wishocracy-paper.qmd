...
RAPPA achieves the cognitive tractability of traditional voting while capturing preference intensity like quadratic voting, a combination no prior mechanism provides.
...
**Convergence Properties:** With random pair assignment and large $n$, the law of large numbers ensures that $a_{jk} \to E[\rho_{i,j,k}]$, the population mean preference ratio. By Hoeffding's inequality for bounded random variables $\rho_{i,j,k} \in [0,1]$, the sample size required for $\epsilon$-accurate estimation satisfies:

$$
P\left(|a_{jk} - E[\rho_{i,j,k}]| > \epsilon\right) \leq \frac{1}{4|S_{jk}|\epsilon^2}
$$

Thus achieving $\epsilon$-accuracy with confidence $1-\delta$ requires $|S_{jk}| = O(1/\epsilon^2\delta)$ samples per pair, demonstrating that aggregate preference estimates converge rapidly even with sparse sampling.
