---
title: "Optimal Spending Level Framework: A Specification for Evidence-Based Budget Allocation"
subtitle: "Determining Optimal Investment Levels Using Reference Benchmarking, Diminishing Returns Modeling, and Evidence Quality Scoring"
description: "The Optimal Spending Level (OSL) framework answers: 'How much should we spend on each category to maximize welfare?' Like the Recommended Daily Allowance for nutrients, each spending category has an optimal level - too little means underinvestment, too much means waste. The Budget Impact Score (BIS) measures our confidence in each category's OSL estimate based on the quality of causal evidence."
aliases:
...
This specification describes the **Optimal Spending Level (OSL)** framework, a systematic approach to determining how much should be allocated to each budget category to maximize welfare outcomes. Unlike marginal-return frameworks that ask "where should we invest the next dollar?", OSL asks "what is the target spending level for each category?" - analogous to the Recommended Daily Allowance (RDA) for nutrients. Too little spending means underinvestment and missed welfare gains; too much means diminishing returns or outright waste.
...
|  |                     |    |                             |  |
|  |  Answers:           |    |  Answers:                   |  |
|  |  "How much should   |    |  "Which policy reforms      |  |
|  |  we spend on each   |    |  would most improve         |  |
|  |  category?"         |    |  welfare outcomes?"         |  |
|  |                     |    |                             |  |
|  |  Primary output:    |    |  Primary output:            |  |
|  |  Optimal spending   |    |  Ranked reforms by          |  |
|  |  levels + gaps      |    |  welfare impact             |  |
|  +---------------------+    +-----------------------------+  |
|                                                              |
...
The OSL/BIS framework answers: "Given what we know about returns to spending, what are the optimal allocation levels?"
...
The OSL framework uses a structured representation of budget categories:
...
### Three Methods for OSL Estimation
...
Reference benchmarking provides a **starting point** for OSL estimation, not a definitive answer. It should be combined with diminishing returns modeling where dose-response data exists.
...
**OSL estimation**: At $16,000/pupil, the marginal return (~0.30) roughly equals the social discount rate. This suggests:
...
The Budget Impact Score measures our **confidence** in each category's OSL estimate based on the quality and quantity of causal evidence. Unlike earlier formulations that used BIS to directly allocate budgets, OSL determines the target level, and BIS tells us how confident we are in that target.
...
### Minimum Thresholds for OSL Estimation
...
| Factor | OSL Captures | OSL Does Not Capture |
|--------|-------------|---------------------|
...
OSL provides evidence-based targets. Political judgment is still required for implementation strategy.
...
OSL provides evidence-optimal targets, not politically achievable ones. A $445B military cut may be optimal but infeasible.

**Mitigation**: OSL is a north star, not immediate policy. Transition paths must account for political constraints.
...
**Question**: Do OSL-guided reallocations improve outcomes going forward?

**Method**:
1. Pre-register OSL predictions publicly
2. Monitor jurisdictions that adopt OSL guidance vs. those that don't
...
| **Outcome improvement** | Did welfare metrics improve more in OSL-following jurisdictions? | > 10% relative improvement |
...
3. **API access**: Enable researchers to query OSL data programmatically

### Governance Integration

1. **Dashboard for policymakers**: Real-time gap analysis
2. **Budget proposal scoring**: Automatically assess proposed budgets vs. OSL
3. **Incentive Alignment Bonds**: Tie politician compensation to moving toward OSL

## Conclusion

The Optimal Spending Level framework provides a systematic, evidence-based approach to budget allocation. Unlike marginal-return frameworks that can justify infinite spending on high-return categories, OSL recognizes that every category has an optimal level - like the Recommended Daily Allowance for nutrients.

The framework answers three questions:

1. **What is the target?** OSL provides evidence-based spending levels for each category
...
## Appendix A: Worked Example - Complete OSL Calculation {.appendix}

### Example: US Military Discretionary Spending

This worked example demonstrates the complete OSL calculation for a category where reference benchmarking is the primary method.
...
### Complete OSL Analysis Pipeline

```
+-------------------------------------------------------------+
|                    OSL ANALYSIS WORKFLOW                      |
+-------------------------------------------------------------+
...
- **Optimal Spending Level (OSL)**: The evidence-based target spending level for a budget category, analogous to the Recommended Daily Allowance for nutrients. Below OSL indicates underinvestment; above OSL indicates overinvestment.

- **Budget Impact Score (BIS)**: A 0-1 score measuring confidence in the OSL estimate based on the quality and quantity of causal evidence. Higher BIS indicates more reliable OSL estimates.

- **Spending Gap**: The difference between current spending and OSL. Positive gaps indicate underinvestment; negative gaps indicate overinvestment.

- **Reference Country Benchmarking**: Estimating OSL by observing spending levels in comparable high-performing countries and adjusting for context.

- **Diminishing Returns**: The economic principle that marginal returns to spending decrease as spending increases. OSL is where marginal return equals opportunity cost.
...
- **Cost-Effectiveness Threshold**: The maximum acceptable cost per QALY (or other health outcome) for including an intervention in OSL calculations. Typically $50K-$150K per QALY.

- **Dose-Response Curve**: The relationship between spending level (dose) and outcome (response). Used to identify diminishing returns and estimate OSL.
...
- **Evidence Grade**: Letter grade (A-F) summarizing confidence in OSL estimate. A = strong evidence; F = insufficient evidence.
...
### Current US Discretionary Budget vs. OSL-Optimal Allocation
...
**Estimated welfare gain from OSL alignment**: Moving from current allocation to OSL would increase welfare-equivalent output by an estimated 3-5% of GDP ($750B-$1.25T annually), based on the differential returns between over- and under-invested categories.
