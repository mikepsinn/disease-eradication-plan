...
OPG measures policy impact on two fundamental dimensions that capture the vast majority of welfare effects:

### 1. Real After-Tax Median Income Growth

- **Definition**: Year-over-year percentage change in inflation-adjusted, post-tax median household income
- **Sources**: Census Bureau/BLS (US), national statistical offices (other countries)
- **Why median**: Captures typical citizen experience, not skewed by billionaires
- **Why after-tax**: Reflects actual purchasing power after policy interventions
- **Units**: Percentage points (pp) of annual growth rate
...
- **Why healthy years**: Quality-adjusted lifespan matters more than raw longevity
- **Units**: Years of healthy life expectancy

### Why Only Two Metrics?

**Simplicity**: These two metrics are designed to capture the primary welfare dimensions affected by most policies while remaining directly interpretable. No complex conversion factors (VSL, QALY→$) are needed.

**Coverage gap**: Freedom and autonomy concerns---the main dimension not captured---are handled as blocking factors rather than adding metric complexity. A policy that improves income and health but restricts freedom is flagged, not silently scored. Environmental impacts and distributional effects (equity) are tracked as supplementary indicators where data permits, but are not included in the core welfare function to maintain interpretability.
...
The key insight is that centuries of natural policy experiments have already been conducted---we simply need to harvest the evidence systematically. With 50 US states varying policies over 70 years, 200 countries over two centuries, and thousands of cities and counties making independent policy choices, the data to know what works already exists. OPG transforms this scattered evidence into actionable, jurisdiction-specific guidance.
...
- **Policy Impact Score (PIS)**: Intermediate metric quantifying the strength of evidence that a policy affects the two welfare metrics. Computed separately for income and health effects, then combined. Integrates effect size, causal confidence (Bradford Hill criteria), and analysis quality.

- **Real After-Tax Median Income Growth**: Primary economic welfare metric. Year-over-year percentage change in inflation-adjusted, post-tax median household income. Units: percentage points (pp) per year.

- **Median Healthy Life Years**: Primary health welfare metric. Expected years of life in good health at the population median. Units: years.
...
- **Blocking Factor**: Constraint that may impede policy adoption: constitutional (requires amendment), federal preemption (superseded by higher law), political (strong opposition), implementation cost (high fixed costs), autonomy (restricts individual freedom---the primary welfare dimension not captured by the two core metrics).
