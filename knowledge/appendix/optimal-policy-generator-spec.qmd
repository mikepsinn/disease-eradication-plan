...
This specification describes the **Optimal Policy Generator (OPG)**, a framework for producing jurisdiction-specific policy recommendations based on quasi-experimental evidence. OPG answers the question: "Given the evidence, what policies should jurisdiction X adopt or repeal?" The framework operates at any jurisdiction level (country, state, county, city) and produces three outputs: (1) **Enact List** - high-impact policies the jurisdiction lacks or should set at optimal levels, (2) **Repeal List** - harmful policies the jurisdiction has, and (3) **Maintain List** - policies aligned with evidence. Each recommendation includes expected effect estimates, confidence grades, monetized impact, blocking factors, recommended jurisdictional level (subsidiarity), and tracking guidance for continuous improvement. This specification complements the Optimocracy framework's Budget Impact Score (BIS): where BIS produces jurisdiction-specific spending recommendations ("Texas underspends on education by $X"), OPG produces jurisdiction-specific policy recommendations ("Texas should enact primary seat belt laws, expected effect: -1.8 deaths/100K").

## System Overview

### What Policymakers See

A jurisdiction-specific dashboard showing which policies to enact or repeal, ranked by expected welfare impact:
...
policy_recommendations (
    jurisdiction_id, policy_type_id,
    recommendation_type, -- 'enact', 'repeal', 'maintain'
    current_status, -- what they have now
    recommended_target, -- what evidence suggests (for continuous policies, the target level)
...
| Type | Trigger | Output |
|------|---------|--------|
| **Enact** | Jurisdiction lacks policy OR has suboptimal level | "Adopt [policy]" or "Set [policy] at [level]" |
| **Repeal** | Jurisdiction has policy with negative evidence | "Remove [policy]" |
| **Maintain** | Jurisdiction has policy aligned with evidence | "Continue [policy]" |

For continuous policies (taxes, spending levels), "Enact" specifies the target level. Example: "Enact tobacco tax at $2.50/pack" covers both new adoption AND replacing an existing lower rate.
...
::: {.callout-warning}
## Policies to ENACT (Continuous - Set at Optimal Level)

**3. Tobacco Tax at $2.50/pack** (Current: $1.41/pack)
...
14. Priority scoring
    ...
    ├── Generate enact/repeal/maintain lists
    └── Calculate total expected welfare gain

Phase 5: OUTPUT GENERATION
──────────────────────────
15. Recommendation dashboard
    ├── Enact list with expected effects (includes continuous policy levels)
...
- **Optimal Policy Generator (OPG)**: System for producing jurisdiction-specific policy recommendations based on quasi-experimental evidence. Outputs enact/repeal/maintain lists ranked by expected welfare impact, with recommended jurisdictional level (subsidiarity) and tracking guidance for continuous improvement.
...
- **Enact List**: Policies the jurisdiction should adopt (has strong evidence of benefit, jurisdiction lacks them or has suboptimal level). For continuous policies, specifies target level.

- **Repeal List**: Policies the jurisdiction should remove (has evidence of harm, jurisdiction currently has them).

- **Maintain List**: Policies the jurisdiction should keep unchanged (aligned with evidence).
