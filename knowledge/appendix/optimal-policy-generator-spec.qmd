...
abstract: |
  ...
  (90% CI: 2-25%). At system scale, the model's Optimal-Governance Path reaches
...
**Scale note.** If recommendation quality is the bottleneck, the upside from fixing it is enormous. Under the project's best-case governance ceiling, the recoverable upside is {{< var political_dysfunction_global_opportunity_cost_total >}} per year. The 20-year Optimal-Governance Path reaches {{< var wishonia_trajectory_gdp_vs_current_trajectory_multiplier_year_20 >}} the Earth baseline, raises average income to {{< var wishonia_trajectory_avg_income_year_20 >}} versus {{< var current_trajectory_avg_income_year_20 >}} on the status-quo path, and reaches {{< var wishonia_trajectory_gdp_year_20 >}} in total output. This specification focuses on the policy-selection layer of that plan; the full derivation lives in [The Political Dysfunction Tax](political-dysfunction-tax.qmd).
