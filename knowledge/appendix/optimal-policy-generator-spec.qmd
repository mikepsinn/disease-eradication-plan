...
abstract: |
  ...
  The Optimal Policy Generator: For any jurisdiction, four outputs. ENACT (policies proven elsewhere
  that you lack). REPLACE (policies at the wrong level). REPEAL (policies with net harm). MAINTAIN
  (policies aligned with evidence). Each recommendation includes effect size, confidence interval,
  monetized impact, evidence grade.

  Bradford Hill criteria (the gold standard for establishing causality in epidemiology) are operationalized
  into nine quantitative scoring functions: strength of association, consistency across contexts,
  specificity, temporality, dose-response, plausibility, coherence, experimental evidence, and analogy.
  Synthetic control constructs counterfactual outcomes; difference-in-differences isolates policy effects
  from confounders. Agnostic to which party enacted each policy, only whether it worked. Estimated
  welfare gains: 5-15% of GDP for typical US states.
aliases:
...
format:
  ...
  pdf:
    ...
    fontsize: 11pt
---
![The Optimal Policy Generator (OPG) produces systematic policy recommendations for jurisdictions at any level (country, state, city), generating prioritized enact/replace/repeal/maintain recommendations based on quasi-experimental evidence from centuries of policy variation data.](/assets/infographics/knowledge/appendix/optimal-policy-generator-spec-infographic-academic.jpg)
...
Bradford Hill's criteria for causality [@hill1965], originally developed for epidemiology, are operationalized here as explicit scoring functions. Each criterion maps to a saturation function that produces a score in $[0, 1]$.
...
Where $w_{\text{method}}$ is the base method weight (see @tbl-method-weights) and $v_{\text{violations}} \in [0, 1]$ is the proportion of validity checks failed (parallel trends, pre-treatment fit, placebo tests).
...
$$
\text{PIS}_{jpo} = |\hat{\beta}_{jpo}| \times \text{CCS}_{jpo} \times Q_{jpo}
$$

Where:

- $|\hat{\beta}_{jpo}|$ = Absolute value of **standardized** effect estimate. We take absolute value because PIS measures *strength of evidence*, not direction. Direction is reported separately; negative effects on desirable outcomes are flagged as potential harms.
- $\text{CCS}_{jpo}$ = Causal Confidence Score computed from Bradford Hill criteria (see @sec-bradford-hill):

$$
\text{CCS} = S_{\text{temporality}} \times \sum_{k=1}^{9} w_k \cdot S_k
$$
...
**2. Universal Motorcycle Helmet Requirement** (Current: None for adults - partial coverage under 21 only)
