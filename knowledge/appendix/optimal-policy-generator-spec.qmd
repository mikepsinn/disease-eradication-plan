...
description: The Optimal Policy Generator (OPG) is a research-stage extension of the Optimocracy evidence scoreboard. It grades policy evidence, forecasts effects on typical income and healthy life years, and produces enact, replace, repeal, maintain, pilot, or abstain outputs. Cross-jurisdictional variation supports causal claims only when a suitable design passes its diagnostics.
abstract: |
  Centuries of public policy variation across thousands of jurisdictions create many opportunities for
  comparison. They do not constitute one massive natural experiment, and many comparisons cannot identify
  causal effects.

  The Optimal Policy Generator (OPG) is a research specification for grading evidence, forecasting effects on
  real after-tax median income and healthy life years, and producing bounded recommendations. A method such as
  synthetic control, difference-in-differences, or regression discontinuity supports a causal grade only when
  the application passes the design's assumptions and diagnostics.

  OPG can produce ENACT, REPLACE, REPEAL, and MAINTAIN recommendations when the evidence clears a declared
  threshold. Weak evidence, unsupported extrapolation, or unresolved severe harm instead produces PILOT,
  INSUFFICIENT EVIDENCE, or DO NOT RECOMMEND. Each output includes raw effects on both metrics, an evidence
  grade, uncertainty, blocking factors, and a prospective evaluation date.

  The framework is agnostic to which party enacted each policy. The project's large welfare and GDP figures
  are illustrative best-case scenarios, not validated OPG forecasts or recoverable totals.
aliases:
...
> **Status: research-stage extension, not the Optimocracy minimum viable product.** The name describes the intended destination, not a claim that current data can generate an optimal answer for every policy. The operational starting point is a public evidence card with a grade, separate health and income forecasts, a safety screen, a bounded recommendation, and a locked evaluation date. See [Optimocracy](optimocracy-paper.qmd) [@optimocracy-paper-2025] for that smaller protocol.

## Abstract {.unnumbered}

Thousands of jurisdictions have been running different policies for centuries. Some of them got richer. Some got healthier. Some did both. Those comparisons can generate hypotheses and, in suitable designs, causal evidence. They are not automatically clinical trials, and more rows do not remove confounding.

This specification describes the **Optimal Policy Generator (OPG)**, a research framework for producing jurisdiction-specific policy evidence cards. OPG reports effects on **real after-tax median income growth** and **median healthy life years** separately, then assigns an evidence grade and bounded recommendation. A combined score is optional and uses preregistered dimensionless normalization rather than adding percentage points to years.

> **OPG output classes:** ENACT, REPLACE, REPEAL, or MAINTAIN when evidence and safety screens pass. Otherwise: PILOT, INSUFFICIENT EVIDENCE, or DO NOT RECOMMEND.
...
Each recommendation includes expected effects on both metrics, an evidence grade, the principal uncertainties, a safety screen, blocking factors (see @sec-blocking-factors), and a prospective evaluation date.

**Scale note.** Better recommendations could have large benefits. The project's [Optimal Governance Trajectory](../economics/gdp-trajectories.qmd) is an illustrative best-case scenario, not an OPG forecast. Its overlapping policy-domain estimates must not be treated as a recoverable total. This specification focuses on testing whether the policy-selection layer can add value one decision at a time.
...
## The Two Terminal General-Welfare Metrics {#sec-two-metrics}

OPG reports policy impact using the [two-metric dashboard](optimocracy-paper.qmd#sec-two-metric-welfare) defined in [Optimocracy](https://optimocracy.warondisease.org) [@optimocracy-paper-2025]:

1. **Real after-tax median income growth** (pp/year) - economic welfare
2. **Median healthy life years** (years) - health welfare

See [Optimocracy](optimocracy-paper.qmd) for the metric definitions, normalization rule, safety screen, and limitations.

![The two terminal general-welfare metrics: having money and being alive to spend it. You'd think this would be obvious, but governments often forget the second bit.](/assets/images/optimal-policy-generator-spec/optimal-policy-generator-spec-section-the-two-welfare-metrics-sec-two-metrics-bw-academic.jpg)
...
## The Evidence Base: Centuries of Policy Variation {#sec-evidence-base}

Every jurisdiction that enacted a policy created an observation. Some policy changes also create credible quasi-experimental opportunities. OPG harvests the broader evidence base, then grades each comparison according to what its design can actually support.

### Scale of Available Comparisons
...
Each policy change creates a before/after comparison. A jurisdiction that did not adopt is a valid control only if the design supports that role, including credible trends, timing, spillover, and comparability assumptions. The table measures available observations, not independent experiments or effective sample size.
...
- **Potential quasi-experimental designs identified** for validation
...
The evidence base comprises millions of policy-years across jurisdictional levels (see @sec-evidence-base for counts). Systematic analysis can improve the evidence available for policy adoption, but the number of rows does not remove confounding or prove that the complex system beats a simpler evidence review.
...
2. **Taxonomic**: We formalize recommendation types and introduce the Policy Impact Score (PIS) as a structured evidence record. Effect magnitude, causal grade, methodological quality, and safety status remain visible rather than allowing a large speculative effect to conceal weak identification.
...
Let $\mathcal{P}$ denote the set of available policies. For jurisdiction $j$, let $P_j \subseteq \mathcal{P}$ denote the current policy bundle. The operational system forecasts raw income and health effects separately. If a public ranking requires one score, it uses preregistered normalized effects:

$$
S_j(P) = \alpha I_j(P) + (1-\alpha)H_j(P), \qquad I_j,H_j \in [0,100]
$$

Where:

- $I_j(P)$ = Normalized effect on real after-tax median income over the declared horizon
- $H_j(P)$ = Normalized effect on healthy life years over the same horizon
- $\alpha$ = Publicly selected weight, tested for sensitivity

The long-term research target is:
$$
P_j^* = \arg\max_{P \subseteq \mathcal{P}} S_j(P) \quad \text{subject to feasibility and safety constraints}
$$

This target is not an operational claim that the complete optimum is identified. A collection of average policy effects does not define the value of a policy bundle. Complementarities, substitution, saturation, implementation capacity, timing, legal constraints, and transition costs can change the result.
...
The minimum viable system therefore evaluates one bounded decision at a time and models only interactions that are plausibly material. If a recommendation depends on an unidentified interaction or lies outside observed support, the output is Pilot or Insufficient Evidence rather than a claimed optimum.
...
### Evidence Aggregation Notes

When studies estimate comparable populations, policy doses, outcomes, and follow-up horizons, a random-effects meta-analysis may estimate a pooled raw effect:
...
This is a standard synthesis tool, not an identification strategy. High heterogeneity is a warning to investigate context and transportability. It does not have a universal cutoff that converts a pooled estimate into a jurisdiction-specific recommendation.

### Information Value

When a policy is promising but uncertain, value-of-information analysis can help decide whether a pilot is worth its cost:

$$
\text{VOI} = E[\max_{a \in \{adopt, reject\}} U(a | \text{new data})] - \max_{a} E[U(a | \text{current data})]
$$

Use it only when the additional model is likely to change a real decision. Otherwise the simpler rule is enough: prioritize affordable pilots where uncertainty and stakes are both high. See @sec-trial-prioritization.
...
#### Evidence Grade by Design Validation

The method name does not supply a fixed confidence weight. Grade the application after its identifying assumptions and diagnostics are reviewed.

| Grade | Basis | Permitted claim |
|-------|-------|-----------------|
| A | Randomization or validated discontinuity | Strong causal estimate |
| B | Quasi-experimental design that passes diagnostics | Qualified causal estimate |
| C | Adjusted longitudinal design | Predictive estimate |
| D | Cross-sectional comparison or expert benchmark | Prior or hypothesis |
| X | Failed critical diagnostic or unsupported extrapolation | No recommendation |

### Optional Research: Bradford Hill Heuristics {#sec-bradford-hill}

Bradford Hill's considerations [@bradford-hill-1965] can organize a robustness review, but they do not replace identification. The numeric functions in this section are an unvalidated research sketch, not part of the minimum viable product and not a source of the operational causal grade. A failed design-specific diagnostic remains grade X regardless of the heuristic score.
...
### Experimental Causal Confidence Score (CCS) Calculation

For research comparison only, the proposed CCS combines the eight non-temporality heuristics with explicit weights, gated by temporality. These weights require prospective calibration before use and do not authorize causal language:
...
Evidence grade and safety status are gates, not multipliers. Within the same passing tier, recommendations may be ordered by expected raw health and income benefit per dollar, feasibility, uncertainty, and robustness to public weights. A large modeled benefit with weak identification becomes a pilot rather than a high-priority instruction.

| Tier | Interpretation |
|------|----------------|
| **Recommend** | Evidence and safety screens pass; bounded action is supported |
| **Promising: Pilot First** | Potentially valuable, but scaling, transportability, or identification remains uncertain |
| **Insufficient Evidence** | No defensible action ranking yet |
| **Do Not Recommend** | Expected harm, failed diagnostics, or a failed safety screen |

Recommend-tier decisions have:

1. Evidence grade A or B with critical diagnostics passed
2. Safety screens passed and implementation feasible within the stated scope
3. Material expected raw health or income benefit relative to cost
4. A conclusion robust to reasonable weights and uncertainty assumptions
...
For continuous policies (taxes, spending levels), **Replace** specifies a bounded change supported by the evidence. **Enact** is reserved for truly new policies that do not exist in the jurisdiction.

### Blocking and Safety Factors {#sec-blocking-factors}

Recommendations flag constraints that may impede adoption:

| Factor | Treatment | Description | Example |
|--------|-----------|-------------|---------|
| Constitutional constraint | **Gate** | Current proposal is legally unavailable | Takings Clause limits on land use regulations |
| Federal preemption | **Gate** | Current jurisdiction cannot enact the policy | Federal law preempts a local rule |
| Treaty obligation | **Gate** | Current proposal conflicts with an international obligation | WTO rules on tariffs |
| Rights or autonomy concern | **Safety gate** | Policy restricts protected freedom or choice | Mandatory medical intervention |
| Credible subgroup harm | **Safety gate** | Median gains conceal severe harm to a group | Benefit excludes or harms disabled people |
| Catastrophic risk | **Safety gate** | Low-probability harm exceeds the declared threshold | Irreversible environmental release |
| Implementation capacity | **Pilot gate** | Delivery capacity cannot support the proposed scale | New program lacks trained staff |
| Political feasibility | **Metadata** | Organized opposition affects adoption, not the evidence grade | Industry lobbying |

**Why some factors gate the recommendation**

Political feasibility remains metadata because it can change without changing the evidence. Legal impossibility, credible severe harm, catastrophic risk, and unsupported implementation scale affect what the system can responsibly recommend now. A gated proposal may remain visible for research or democratic deliberation, but it receives no Recommend label or evidence-alignment reward until the gate is resolved.
...
1. **Increase learning opportunities**: More jurisdictions can provide more adoption events, comparisons, and opportunities for randomized or credible quasi-experimental designs. They do not automatically create independent experiments.
...
## Policy Impact Score (Evidence Record) {#sec-pis}

### Overview

The Policy Impact Score (PIS) is a structured record used to generate recommendations. It keeps effect magnitude, uncertainty, causal grade, analysis quality, and safety status separate. Multiplying them into one number would allow a large speculative effect to outrank a smaller well-identified effect without showing the trade-off.
...
For each jurisdiction $j$ and policy $p$, store:

$$
\text{PIS}_{jp} = \left(
\hat{\beta}^{\text{inc}}_{jp},
\hat{\beta}^{\text{hlth}}_{jp},
G_{jp},
Q_{jp},
R_{jp}
\right)
...
- $\hat{\beta}^{\text{inc}}_{jp}$ = Raw effect estimate and interval for typical income over the declared horizon
- $\hat{\beta}^{\text{hlth}}_{jp}$ = Raw effect estimate and interval for healthy life years over the same horizon
- $G_{jp}$ = Evidence grade A, B, C, D, or X
- $Q_{jp}$ = Design-specific diagnostics and analysis-quality record
- $R_{jp}$ = Rights, subgroup-harm, catastrophic-risk, feasibility, and extrapolation status

Evidence grade and safety status gate the recommendation. Within the same passing tier, an optional normalized public score may rank expected effects. The raw estimates and weight sensitivity remain visible.

### Always Report Both Metrics Separately

Recommendations display effects on both metrics and the evidence gate:

| Policy | Income Effect | Health Effect | Critical Diagnostics | Grade |
|--------|---------------|---------------|----------------------|-------|
| Primary seat belt | +0.02 pp/yr | +0.15 years | Pass | A |
| Speed limit reduction | +0.01 pp/yr | +0.06 years | Pass with limitations | B |
...
### Optional Effect Normalization

If a combined ranking is required, each metric is converted to a dimensionless percentile using a preregistered reference distribution:

$$
I_{jp},H_{jp} \in [0,100], \qquad
S_{jp} = \alpha I_{jp} + (1-\alpha)H_{jp}
$$

The reference distributions, horizon, and $\alpha$ are fixed before scoring. A 50/50 weight is a public preference, not a neutral scientific conversion. Rankings must show sensitivity to reasonable alternative weights.

### Design Diagnostics Are Gates

A failed identifying assumption is not repaired by multiplying the estimate by a smaller quality weight. Failed or unresolved critical diagnostics produce grade X. Non-critical limitations lower the evidence grade and remain visible in the decision card.
...
## Cross-Jurisdictional Evidence Synthesis {#sec-global-pis}

Comparable jurisdiction-level estimates may be combined via random-effects meta-analysis. Pool raw income and health estimands separately. Do not pool incomparable outcomes, designs, treatment doses, or follow-up horizons merely because each produced a number.
...
### Evidence Grade Across Jurisdictions

Evidence grades, design diagnostics, and safety screens are not averaged. The synthesis reports the pooled raw estimate when appropriate, the distribution of evidence grades, heterogeneity, transportability concerns, and any failed critical diagnostic. A precise pooled association does not become causal if its component designs do not identify a causal estimand.
...
Evidence grades are assigned from the identification design and its diagnostics, not from arbitrary cutoffs on a composite score, heterogeneity statistic, or jurisdiction count.

| Grade | Basis | Permitted Claim |
|-------|-------|-----------------|
| **A** | Randomization or a validated discontinuity with relevant follow-up | Strong causal estimate |
| **B** | Quasi-experimental design that passes critical diagnostics | Qualified causal estimate |
| **C** | Adjusted longitudinal evidence without credible causal identification | Predictive estimate |
| **D** | Cross-sectional comparison or expert benchmark | Prior or hypothesis |
| **X** | Failed critical diagnostic or unsupported causal extrapolation | No evidentiary recommendation |

Safety status is recorded separately from evidentiary grade. Credible causal evidence can still produce **DO NOT RECOMMEND** when a rights, subgroup-harm, catastrophic-risk, or implementation gate fails.

Heterogeneity, sample size, precision, replication, publication bias, and transportability remain visible and may lower a grade. None raises a design above what its identification strategy supports. Grade thresholds should be prospectively calibrated against realized forecasts; they should not be tuned until retrospective examples look persuasive.
...
There is no universal minimum number of periods, observations, or control jurisdictions that validates every design. Each decision card instead reports:

- the number of independent adoption or assignment events;
- pre-treatment and post-treatment coverage;
- the policy-assignment and clustering level;
- design-specific diagnostics;
- spillover, anticipation, and interference assumptions; and
- whether the target decision lies within the evidence's population, dose, and implementation support.

For synthetic-control studies, the card reports pre-treatment fit and compares the treated unit's error with the placebo distribution. No universal RMSE cutoff substitutes for that design-specific assessment.
...
### Quasi-Experimental Opportunity Identification

The system flags policy changes that may support a quasi-experimental design. A flag is not a causal grade; the design-specific diagnostics still have to pass.
...
Flagged opportunities are stored in the `natural_experiments` table for design review and validation. The schema name is historical and does not certify the record as a valid natural experiment.

### Recommended Pilot Jurisdictions

For policies classified **Promising: Pilot First**, candidate jurisdictions are compared using:
...
4. **Jurisdiction selection**: Jurisdictions with cleaner identification opportunities are overrepresented
...
3. **Evidence grade thresholds**: Do A, B, C, D, and X classifications predict prospective reliability?
...
The Optimal Policy Generator is a research framework for translating policy-outcome evidence into jurisdiction-specific decision cards. When evidence and safety screens pass, it can produce enact, replace, repeal, or maintain recommendations. Otherwise it produces a pilot, insufficient-evidence, or do-not-recommend output. Its value must be demonstrated through locked forecasts and later outcomes, not by the fact that it maximizes its own projected score.
...
### Step 1: Build the Policy Evidence Record
...
**Experimental CCS** = 0.81. This heuristic is shown only to illustrate the research appendix. It does not determine the operational evidence grade, which depends on the identification design and its diagnostics.
...
9. Evidence synthesis
   ...
   └── Preserve incompatible estimates separately

10. Design validation
    ├── Run method-specific critical diagnostics
    ├── Document confounding and transportability limits
    ├── Apply the safety and extrapolation screens
    └── Assign evidence grade A, B, C, D, or X

11. PIS evidence record
    ├── Preserve raw income and health estimates
    ├── Store intervals and critical diagnostics
    ├── Store evidence grade and safety status
    └── Normalize only if a public ranking requires it
...
14. Priority rules
    ...
    ├── Rank within passing tiers by bounded benefit, cost, feasibility, and uncertainty
    ├── Generate recommend/pilot/insufficient-evidence/do-not-recommend lists
...
15. Recommendation dashboard
    ...
    ├── Replace list (existing policies with a supported bounded change)
...
- [ ] Pre-treatment and post-treatment coverage is adequate for the selected design
- [ ] Independent adoption or assignment events and the effective sample size are reported
- [ ] Clustering, spillovers, interference, and anticipation are addressed
- [ ] Design-specific critical diagnostics pass
...
- [ ] Target population, dose, horizon, and implementation context are within evidence support
...
| **PIS** | Policy Impact Score: structured effect, evidence, quality, and safety record | @sec-pis |
| **CCS** | Experimental Bradford Hill heuristic; not a source of the operational causal grade | @sec-bradford-hill |
| **Evidence Grade** | A, B, C, D, or X classification based on identification and diagnostics | @sec-evidence-grading |
