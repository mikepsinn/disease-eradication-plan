...
abstract: |
  ...
  **real after-tax median income growth** (economic welfare) and **median healthy life years**
  (health welfare). These two metrics capture ~85-90% of policy effects while remaining directly
  ...
  enacted each policy, only whether it worked. Estimated welfare gains: 5-15% of GDP for typical
  US states.
aliases:
...
**Simplicity**: These two metrics capture ~85-90% of policy welfare effects while remaining directly interpretable. No complex conversion factors (VSL, QALY→$) are needed.

**Coverage gap**: Freedom and autonomy concerns---the main dimension not captured---are handled as blocking factors rather than adding metric complexity. A policy that improves income and health but restricts freedom is flagged, not silently scored.
...
The result: trillions of dollars in welfare losses from documented policy failures (see Optimocracy paper, Section 2). Evidence-based policy movements have attempted to address these failures [@cartwright2012; @haskins2009], but lack the systematic, jurisdiction-specific recommendation generation that OPG provides.
...
where $\beta^{\text{inc}}_{jp}$ and $\beta^{\text{hlth}}_{jp}$ are the marginal effects of policy $p$ on each metric in jurisdiction $j$, and interaction terms are second-order.
...
**Proposition 2 (PIS as Precision-Weighted Evidence)**: Under random-effects meta-analysis with between-jurisdiction variance $\tau^2$, the pooled effect estimate $\hat{\beta}_{\text{pooled}}$ is:
$$
...
These weights sum to 1.0 and can be adjusted based on domain-specific considerations.
...
## Two-Metric Reporting Framework

### Why Two Metrics?

Policy effects ultimately manifest through two fundamental welfare dimensions that capture ~85-90% of impact:

1. **Real After-Tax Median Income Growth** - economic welfare
2. **Median Healthy Life Years** - health welfare

This simplification eliminates the need for complex conversion factors (VSL, QALY→$, etc.) while maintaining interpretability. Freedom/autonomy concerns---the main gap---are flagged as blocking factors rather than adding metric complexity.

### Reporting Format

Report effects on both metrics directly in natural units:

| Metric | Units | Measurement | Example |
|--------|-------|-------------|---------|
| **Income Effect** | pp growth/year | Year-over-year % change in real after-tax median household income | +0.15 pp |
| **Health Effect** | healthy years | Change in median healthy life expectancy | +0.3 years |

### Single Welfare Score (Optional)

For ranking purposes, compute an equal-weighted combination:

$$
\text{WelfareScore} = 0.5 \times \text{Income}_{\text{std}} + 0.5 \times \text{Health}_{\text{std}}
$$

Where standardized scores use cross-jurisdictional standard deviations as denominators.

### Example: Tobacco Tax +$1/pack

| Metric | Effect | Interpretation |
|--------|--------|----------------|
| **Income** | +0.12 pp/year | Reduced healthcare spending, fewer sick days |
| **Health** | +0.25 years | Lower smoking rates → longer healthy life |
| **Grade** | A | Strong quasi-experimental evidence from 23 states |

**Interpretation**: "A $1/pack tobacco tax increase is expected to boost median income growth by 0.12 percentage points annually and add 0.25 healthy life years at the median. This estimate is based on Grade A evidence from 23 US states."
...
### Two-Metric Effect Size Benchmarks
...
These benchmarks are calibrated to cross-jurisdictional variation:

- **Income**: US states vary by ~1.5 pp/year in median income growth
- **Health**: US states vary by ~3-5 years in healthy life expectancy

A "medium" effect represents ~10% of the cross-state variation; a "large" effect represents ~20%.
...
| Moderate (25-50% of effect) | Reasonable precision; moderate confidence |
| Wide (> 50% of effect) | Imprecise; low confidence in point estimate |
