...
- **Sensitivity analysis** showing how OBG changes with different assumptions
...
For categories where comparable cross-country data exists, OBG can be estimated from high-performing reference countries:
...
### Reframing BIS: Confidence in OBG, Not Allocation Driver

The Budget Impact Score measures our **confidence** in each category's OSL estimate based on the quality and quantity of causal evidence. Unlike earlier formulations that used BIS to directly allocate budgets, OBG determines the target level, and BIS tells us how confident we are in that target.
...
| BIS Range | Grade | Interpretation | OBG Confidence |
|-----------|-------|----------------|----------------|
...
| 0.00 - 0.19 | F | Insufficient evidence | Unknown - cannot estimate OBG |
...
|-------------------------------|--------------------------------|
| Allocate proportionally to BIS | Allocate to reach OBG |
| High BIS = more spending | High BIS = confident in OBG |
| Ignores diminishing returns | Explicitly models optimal level |
| Infinite spending possible | Bounded by OBG |
...
1. **Leave-one-country-out**: Does excluding any single reference country change OBG by >20%?
2. **Method comparison**: Do reference benchmarking, diminishing returns, and cost-effectiveness methods agree?
3. **Time stability**: Has OBG changed substantially over past 5 years?
4. **Sensitivity to assumptions**: How does OBG change with ±20% parameter variation?
...
For categories where OBG is uncertain:
...
| Parameter | Default | Test Range | Impact on OBG |
|-----------|---------|------------|---------------|
...
Report OBG range across scenarios for policy guidance.
...
Even with imperfect evidence, systematically moving from severe misallocation (military 100% above OBG, vaccinations 75% below OBG) toward evidence-based targets will produce welfare gains orders of magnitude larger than current discretionary allocation achieves.
...
4. Reference benchmarking
   ...
   +-- Estimate OBG with confidence intervals
