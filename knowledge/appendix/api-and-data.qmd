...
Wishonia Love, CEO of Universe Optimization Services, teaches your visitors how to help end war and disease, starting with the [1% Treaty](../solution/1-percent-treaty.qmd) [@1-percent-treaty-paper-2025] [@1-pct-treaty-impact-paper-2025] (redirecting 1% of global military spending to clinical trials). She answers questions, explains the math, cites sources, and shows relevant charts. Every conversation is one more person who understands how to end the largest unnecessary cause of death on the planet.
...
The script loads its own stylesheet, derives the manual's address from its own URL, fetches live parameter values, and points every number's source link back here. If your visitor's browser blocks the cross-origin fetch, the counters fall back to values baked into the script at deploy time; they tick either way.

Pick a different bar design with one more line (options and previews in the [configurator](https://manual.warondisease.org/assets/scoreboard-embed.html)):

```html
<meta name="dih-scoreboard-variant" content="instrument-panel">
```

Every interaction fires a `dih-scoreboard` CustomEvent on `document` (`bar-shown`, `bar-dismissed`, `overlay-opened`, `overlay-closed`, `cta-clicked`, each tagged with the active variant), so you can measure which design converts with whatever analytics you already run.

### Counters Without the Bar

If you want live numbers inside your own prose without our scoreboard eating the bottom of your screen, add one more meta tag and write spans wherever you want numbers:
