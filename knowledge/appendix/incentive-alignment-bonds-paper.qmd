...
abstract: |
  This paper introduces Incentive Alignment Bonds (IABs), a novel class of financial instruments defined by three primitive properties: (1) investor alignment with public good production, (2) politician alignment with public good production, and (3) funding from government programs with negative net social value. Drawing on mechanism design theory, public choice economics, and campaign finance research, we formalize a three-layer architecture (scoring, electoral support, and post-office benefits) that makes supporting public-good policies incentive-compatible for rational political actors. Unlike Social Impact Bonds, which reward service providers for program outcomes, IABs target politicians directly at the policy level where large-scale budget decisions are made. We prove conditions under which supporting public goods becomes a Nash equilibrium for utility-maximizing politicians, analyze legal constraints under U.S. anti-bribery law, and provide a welfare-economic criterion for identifying appropriate funding sources. By redirecting resources from programs that persist due to Olsonian concentrated-benefit/diffuse-cost dynamics rather than social value, IABs are Pareto-improving by construction. We illustrate the mechanism with a hypothetical global health treaty and discuss applications to climate change, nuclear disarmament, and pandemic preparedness. We conclude with a discussion of limitations, governance challenges, and directions for empirical research.
...
1. **Investor alignment with public good production:** Investors receive returns proportional to verified public-good funding achieved. If the policy succeeds, investors profit; if it fails, they do not. This creates a class of actors with concentrated financial interest in policy success.
...
3. **Funding from harmful or wasteful sources:** The public good is funded by redirecting resources from government activities that either (a) produce measurable harm to society, or (b) persist despite producing no measurable value due to Olsonian concentrated-benefit/diffuse-cost dynamics. This constraint ensures IABs do not crowd out other beneficial programs; they are Pareto-improving by construction.
...
The third primitive property requires identifying government expenditures with negative or zero net social value. This is an empirical question that can be addressed using standard welfare economics:

$$
NSV_i = B_i - C_i - O_i
$$ {#eq-nsv}

where $NSV_i$ is net social value of program $i$, $B_i$ is measurable social benefits, $C_i$ is direct costs, and $O_i$ is opportunity cost (value if resources were allocated to highest-value alternative use).

A program is an appropriate IAB funding source if $NSV_i \leq 0$. That is, if society would be better off without it, even ignoring what the redirected funds could produce.
...
The net social value calculation for military spending beyond deterrence is unambiguously negative: the \$2.4 trillion in direct expenditure produces \$10+ trillion in societal costs. Redirecting even 1% to medical research, which has positive externalities and high multipliers, is Pareto-improving.
...
The empirical criterion is straightforward: if a program's beneficiaries are concentrated, its costs are diffuse, and rigorous cost-benefit analysis shows $NSV \leq 0$, it is an appropriate IAB funding source. The IAB mechanism does not require moral judgment about the program, only measurement.
...
4. **A Pareto-Improvement Criterion**: We formalize how to identify appropriate IAB funding sources using net social value analysis. By redirecting resources from programs with $NSV \leq 0$, those that persist due to Olsonian dynamics rather than social value, IABs are welfare-improving by construction, avoiding the trade-offs inherent in new taxation or deficit spending.
...
**Independent expenditures affect elections.** Following *Citizens United v. FEC* (2010), independent expenditures increased dramatically, from $52 million in 2006 to $290 million in 2010 in congressional races alone [@fowler2016]. Research using transaction-level disbursement data finds that "spending on messages to voters has a statistically significant effect on voter support for candidates" and is "especially effective in changing the composition of voters" [@spenkuch2018].
...
**The revolving door affects politician incentives.** Research documents significant movement between government and private sector. A 2023 study found that 32% of HHS appointees exited to industry employment [@jha2023]. Among four-star military retirees, 80% are employed in the defense industry [@defiancepress2023]. Shepherd and You [-@shepherd2019] find evidence that "career concerns" about post-office employment influence legislative behavior.
...
where $\epsilon$ is the small diffuse benefit and $c_i > 0$ is the concentrated cost. Since $c_i > \epsilon$ for most policies with diffuse benefits, the equilibrium is $a_i^* = 0$. This is Olson's result in formal terms.
...
- $\delta > 0$ captures direct score effect
- $I_i(\theta_i)$ is independent expenditure support, a step function:
...
*Proof:* Under IABs, $a_i = 1$ is a dominant strategy when @eq-ic holds. If $a_i = 1$ is individually optimal regardless of others' actions, then $(1, \ldots, 1)$ is the unique Nash equilibrium. $\square$
...
2. **Instrumental:** We define IABs through three primitive properties: investor alignment, politician alignment, and funding from net-negative-value sources. Together, these create a self-sustaining mechanism for public good production.
...
4. **Welfare-Economic:** We provide a rigorous criterion ($NSV \leq 0$) for identifying appropriate funding sources, ensuring IABs are Pareto-improving by construction rather than competing with beneficial programs.
