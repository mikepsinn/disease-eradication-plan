...
description: Analysis and Return on Investment - How to slash per-patient trial costs by up to 80×, generate approximately $50 billion in annual gross R&D savings, and deliver ROI ranging from {{< var dfda_roi_rd_only >}} (R&D only) to {{< var dfda_roi_rd_plus_delay >}} (with regulatory delay avoidance).
published: true
tags:
  - dfda
  - roi
...
lastNonprofitComplianceHash: 5fa200aba6029572b9a2b31c1f6914c41a623f296f2f308f4c32fca0715372a8
---
...
- **Saves ~{{< var dfda_rd_gross_savings_annual >}} Annually**: Reduces global clinical trial costs (~{{< var global_clinical_trials_spending_annual >}}/year market) by a baseline of **[50.0%](#gross-r-and-d-savings-from-dfda-implementation)**, with up to **[95%](#gross-r-and-d-savings-from-dfda-implementation)** savings in optimal scenarios.
- **{{< var dfda_roi_rd_only >}} Return on Investment (R&D Only)**: A modest annual operational cost of ~{{< var dfda_upfront_build >}} generates ~{{< var dfda_rd_gross_savings_annual >}} in industry-wide savings, yielding an NPV of ~[\$249B over 10 years](#final-roi-and-net-benefit). **[Comprehensive ROI](#roi-with-regulatory-delay-avoidance)** including regulatory delay avoidance: **{{< var dfda_roi_rd_plus_delay >}}** (conservative) to **{{< var dfda_roi_rd_plus_delay_plus_innovation >}}** (full impact).
- **[Eliminates Regulatory Delay](#regulatory-delay-avoidance)**: By providing post-Phase I access via trial participation, the dFDA eliminates the {{< var efficacy_lag_years >}}-year [efficacy lag](regulatory-mortality-analysis.qmd), saving **{{< var dfda_avoided_regulatory_delay_dalys_annual >}} [DALYs/year](#regulatory-delay-avoidance)** (valued at **{{< var dfda_avoided_regulatory_delay_cost_annual >}}/year**).
- **[Two QALY Estimates](#regulatory-delay-avoidance)**: Conservative bottom-up estimate of **{{< var dfda_qalys_rd_plus_delay_annual >}} [QALYs/year](#appendix-detailed-qaly-calculation-model)** from specific mechanisms; comprehensive top-down estimate of **{{< var dfda_qalys_rd_plus_delay_annual >}}/year** including [regulatory delay avoidance](#regulatory-delay-avoidance).
...
**The Problem:** Medical research costs billions and takes a decade per drug. Most humans die before the drugs that could save them finish paperwork.

**The Solution:** Build a global platform that lets anyone join clinical trials instantly. Real-world data replaces expensive bureaucracy. Treatments get ranked by what actually works, not what companies claim works.

### What You Get

- **Cost Cuts:** Clinical trials cost [50-95% less](#gross-r-and-d-savings-from-dfda-implementation). The UK's [RECOVERY trial](recovery-trial.qmd) proved you can cut costs [80-100x](#decentralized-trial-costs-modeled-on-oxford-recovery) without killing anyone extra. Apply that globally to the {{< var global_clinical_trials_spending_annual >}} spent annually on trials, save [tens of billions](#roi-analysis).
...
- **Fewer Dead People:** The platform generates {{< var dfda_qalys_rd_plus_delay_annual >}} extra life-years annually through faster access, better prevention data, and drugs for diseases companies currently ignore.

#### Exceptional Economic Value

- **Return on Investment (ROI):** The dFDA Infrastructure has an extremely high ROI. Based on core platform operational costs (ROM estimate [40 million USD per year](#simplified-roi-scenario) including medium broader initiative costs) against [**$50.0B in annual R and D savings**](#gross-r-and-d-savings-from-dfda-implementation) ([**50.0% reduction scenario**](#gross-r-and-d-savings-from-dfda-implementation), [see Market Size and Impact](#market-size-and-impact)), the NPV analysis yields ROI estimates ranging from **[66:1 to 2,577:1](#full-range-roi-sensitivity-analysis)** depending on total ecosystem costs ([see Full Range ROI Sensitivity Analysis](#full-range-roi-sensitivity-analysis)), with a central estimate of approximately **{{< var dfda_roi_rd_only >}}:1** over 10 years. A modest investment in global software infrastructure generates vast savings across the entire pharmaceutical R&D industry.
...
- **Cost of Trials**: Using existing healthcare encounters, telemedicine, and EHR data to drastically cut per-patient costs (modeled on the Oxford RECOVERY trial success).
...
- Decentralized trial costs drop closer to the [Oxford RECOVERY model](../references.qmd#recovery-trial-cost-reduction): from an average of [\$40,000 - \$120,000 per patient](../references.qmd#clinical-trial-cost-per-patient) in traditional Phase III trials to roughly **{{< var recovery_trial_cost_per_patient >}} - {{< var dfda_target_cost_per_patient_usd >}} per patient**.
...
## dFDA Platform Costs - ROM Estimates (Partnership Model)
...
> - **Annual full platform operations:** [\$11–\$26.5 million](#annual-operational-costs-illustrative-at-target-scale-of-5m-mau-50tb-ingest-month)
...
This section provides a **Rough Order of Magnitude (ROM)** cost estimate based on a **partnership-first strategy** where the dFDA provides open protocol infrastructure rather than competing with existing platforms.
...
Based on these market comparables, the total investment required to fund a global dFDA Infrastructure initiative, from initial build to widespread adoption, can be estimated to be in the range of **\$50 million to {{< var dct_platform_funding_medium >}}**.
...
This top-down estimate matches the bottom-up analysis. While a core, open-source platform can start for tens of millions (upfront build ROM), a fully-realized, globally adopted dFDA represents a **multi-hundred-million-dollar undertaking**, consistent with "Medium Case" and "Worst Case" scenarios.
...
_This estimate excludes costs for running the **DAO governance structure** and developing/maintaining **plugins** (though plugin development could be incentivized via bounties as per the Act)._

### Enhanced ROM Estimates and Cost Optimization

**Disclaimer:** This subsection presents ROM estimates incorporating the full technical requirements from the "Right to Trial & FDA Upgrade Act" using cost-saving strategies. The estimates assume successful open-source development, bounty programs, and AI automation.
...
- **Initial Core Platform Build (~2.5 years): ~\$37.5 - {{< var dfda_upfront_build_max >}}**
...
- **Extrapolation to New System**:
  - A well-integrated global platform could approach {{< var recovery_trial_cost_per_patient >}} - {{< var dfda_target_cost_per_patient_usd >}} per patient in many cases, especially for pragmatic or observational designs.
  - Up to **~80-100x+ cost reduction** cited for RECOVERY vs. typical Phase III trials is an aspirational benchmark, derived by comparing per-patient costs of ~{{< var recovery_trial_cost_per_patient >}}-\$1,000 against traditional costs of [~\$40,000 - \$120,000+](#market-size-and-impact).
...
1. **By Reducing Per-Patient Costs**
   ...
   - If a trial with 5,000 participants costs {{< var recovery_trial_cost_per_patient >}} - {{< var dfda_target_cost_per_patient_usd >}}/patient, total cost is \$2.5 - \$5 million, versus \$200 - \$600 million under traditional models.
   ...
   - $c_d$ is the decentralized cost per patient ({{< var recovery_trial_cost_per_patient >}} - \$1,000)

   For a trial with $x = 5,000$ participants, the savings range from:

   - Minimum savings: $(\$40,000 - \$1,000) \times 5,000 = \$195$ million
  - Maximum savings: $(\$120,000 - \$500) \times 5,000 = \$597.5$ million (using the RECOVERY per-patient baseline of {{< var recovery_trial_cost_per_patient >}})
...
- [Chronic diseases account for ~90% of U.S. healthcare spending](../references.qmd#us-chronic-disease-spending) .
...
### Regulatory Delay Avoidance {#regulatory-delay-avoidance}

**The dFDA's primary health benefit comes from eliminating the "efficacy lag"** — the {{< var efficacy_lag_years >}}-year delay between Phase I safety verification and final approval while drugs undergo Phase II/III efficacy testing.

#### The Efficacy Lag Problem

A [comprehensive quantitative analysis](regulatory-mortality-analysis.qmd) of regulatory delay costs (1962-2024) found:

- **Total Deaths**: {{< var regulatory_delay_deaths_mean >}}M preventable deaths (95% CI: {{< var regulatory_delay_deaths_lower_bound >}}M – {{< var regulatory_delay_deaths_upper_bound >}}M)
- **Total DALYs**: {{< var regulatory_delay_dalys >}}B Disability-Adjusted Life Years lost
- **Ongoing Annual Cost**: ~{{< var regulatory_delay_annual_dalys >}} DALYs/year, valued at {{< var dfda_avoided_regulatory_delay_cost_annual >}}/year
...
The dFDA provides **provisional access post-Phase I** via trial participation:

1. **Phase I Safety Testing**: Maintained at {{< var phase_1_safety_duration_years >}} years (no change)
...
This eliminates approximately **{{< var dfda_regulatory_delay_avoidance_fraction >}} of regulatory delay** (the efficacy lag portion).

#### Quantified Benefits

The dFDA's regulatory delay avoidance creates:

- **Annual DALYs Saved**: {{< var dfda_avoided_regulatory_delay_dalys_annual >}} ({{< var regulatory_delay_dalys >}}B ÷ 62 years × {{< var dfda_regulatory_delay_avoidance_fraction >}})
- **Annual Economic Value**: {{< var dfda_avoided_regulatory_delay_cost_annual >}} ({{< var dfda_avoided_regulatory_delay_dalys_annual >}} × {{< var standard_economic_qaly_value_usd >}})
- **Comprehensive Annual QALYs**: {{< var dfda_qalys_rd_plus_delay_annual >}} (regulatory delay avoidance + prevention benefits)

This represents the **top-down comprehensive estimate** of dFDA health benefits, compared to the **{{< var dfda_qalys_rd_plus_delay_annual >}} conservative bottom-up estimate** from specific mechanism modeling.

For detailed methodology and assumptions, see [The Human Capital Cost of Regulatory Latency](regulatory-mortality-analysis.qmd).
...
- **Range Used in Sensitivity Analysis**:
  - **Conservative**: $\alpha = 0.30$
    $$
    S_{\text{annual}} = 0.30 \times \$100.0\text{B} = \$30.0\text{B} \text{ annually}
    $$
  - **Base Case**: $\alpha = 0.50$
    {{< var dfda_rd_gross_savings_annual_latex >}}
...
The return on investment for the dFDA Infrastructure is exceptionally high because it's global software infrastructure with massive reach. Unlike investments in single drugs or therapies, an investment in the dFDA Infrastructure creates systemic efficiencies that benefit all R&D. The primary economic benefit is the drastic reduction in clinical trial operational costs, which can be redeployed to fund more research.
...
2. **Model Inputs**
   ...
   - **Upfront Cost (Core Platform Build):** Based on the ROM estimate of **~\$37.5 - {{< var dfda_upfront_build_max >}}** detailed in [Upfront (Capital) Expenditure](#upfront-capital-expenditure-initial-build-illustrative-30-months). ROI calculations use a representative figure (e.g., {{< var dfda_upfront_build >}}).
...
- **Platform Cost Scenarios (Derived from ROM Estimates):**
  - **Upfront Cost (Core Platform Build):** ~{{< var dfda_upfront_build >}} (representative figure from [Upfront (Capital) Expenditure](#upfront-capital-expenditure-initial-build-illustrative-30-months)).
...
- **Net Annual Savings** (assuming full adoption and [50.0% R and D cost reduction](#gross-r-and-d-savings-from-dfda-implementation)): {{< var dfda_rd_gross_savings_annual >}}/year.
...
- **Upfront Cost** (Core Platform Build from [Upfront (Capital) Expenditure](#upfront-capital-expenditure-initial-build-illustrative-30-months)): {{< var dfda_upfront_build >}}
...
_The NPV analysis provides the final estimated ROI of approximately **{{< var dfda_roi_rd_only >}}:1**, which is the figure cited throughout this document._
...
- **Upfront Costs:**
  - Core Platform Build (High end from [Upfront (Capital) Expenditure](#upfront-capital-expenditure-initial-build-illustrative-30-months)): ~{{< var dfda_upfront_build_max >}}
  ...
  - _Total Highest Upfront Cost:_ {{< var dfda_upfront_build_max >}} + $2.231B = **~\$2.277 Billion**
...
1. **Cost of Current Drug Development**:
   ...
   - Oxford RECOVERY trial press releases and scientific papers indicating {{< var recovery_trial_cost_per_patient >}} - {{< var dfda_target_cost_per_patient_usd >}}/patient cost.
...
- **Daily QALYs Lost:**
  - The analysis ([Regulatory Mortality Analysis](regulatory-mortality-analysis.qmd)) projects a baseline of **{{< var dfda_qalys_rd_plus_delay_annual >}} Quality-Adjusted Life Years (QALYs) gained per year** from eliminating the regulatory efficacy lag.
  - The daily opportunity cost in lost health is therefore:
    $$
    \frac{74{,}148{,}065\ \text{QALYs}}{365\ \text{days}} \approx \mathbf{203,145\ \text{QALYs lost per day}}
    $$
...
| :--------------------------------------- | :----------------------: | :------------------: | :-----------------: | :------------------------: | :------------------------: | :------------------------------------------------------------------------------------------------------------------------------------------------ |
| **Conservative**                         |           30%            |     {{< var sensitivity_dfda_savings_conservative >}}     |       {{< var dfda_qalys_rd_plus_delay_annual >}}       |      **\$82 Million**      |          **521**           | Assumes lower efficiency gains and moderate health impact, using [conservative QALY model](#summary-of-total-annual-qaly-gains).                  |
| **Base Case**                            |         **50%**          |   **\$50 Billion**   |     **{{< var dfda_qalys_rd_plus_delay_annual >}}**     |     **\$137 Million**      |         **2,301**          | **The central estimate used in this analysis, based on the [median QALY model](#summary-of-total-annual-qaly-gains).**                            |
| **Optimistic**                           |           70%            |     \$70 Billion     |      {{< var dfda_qalys_rd_plus_delay_annual >}}      |     **\$192 Million**      |         **{{< var dfda_large_trial_size >}}**         | Assumes high efficiency and significant improvements in health outcomes, using [optimistic QALY model](#summary-of-total-annual-qaly-gains).      |
| **Transformative (RECOVERY Trial-like)** |         **95%**          |   **\$95 Billion**   |    **{{< var dfda_qalys_rd_plus_delay_annual >}}**    |     **\$260 Million**      |         **{{< var dfda_large_trial_size >}}**         | Reflects exceptional, RECOVERY-like efficiency and broad health benefits, using [new optimistic QALY model](#summary-of-total-annual-qaly-gains). |
...
# Use parameters from dih_models/parameters.py

global_clinical_trial_market = GLOBAL_CLINICAL_TRIALS_SPENDING_ANNUAL  # {{< var global_clinical_trials_spending_annual >}}
dfda_rd_cost_reduction_pct = TRIAL_COST_REDUCTION_PCT  # 50%
dfda_annual_operational_cost = DFDA_ANNUAL_OPEX  # $0.040B ({{< var dfda_upfront_build >}})

# Baseline QALYs generated annually

qaly_gains_annual_baseline = DFDA_QALYS_RD_PLUS_DELAY_ANNUAL  # {{< var dfda_qalys_rd_plus_delay_annual >}} QALYs
value_per_qaly = STANDARD_ECONOMIC_QALY_VALUE_USD  # $**[Primary QALY estimate used]**{{< var dfda_qalys_rd_plus_delay_annual >}} per QALY

# --- Calculations for dFDA Benefits ---

# Gross R&D savings

gross_rd_savings = DFDA_RD_GROSS_SAVINGS_ANNUAL  # Pre-calculated: {{< var sensitivity_peace_dividend_conservative >}}
...
dfda_roi_simple = DFDA_ROI_SIMPLE  # {{< var dfda_roi_simple >}}:1

# NPV-adjusted ROI (canonical figure)

dfda_roi_npv = DFDA_ROI_RD_ONLY  # Conservative R&D-only (from 10-year NPV analysis)

# Monetized value of QALY gains

monetized_qaly_value = DFDA_QALYS_RD_PLUS_DELAY_MONETIZED  # {{< var dfda_qalys_rd_plus_delay_monetized >}}
...
funding_coverage_ratio = DIVIDEND_COVERAGE_FACTOR  # Pre-calculated coverage ratio (~679x)
...
**Note**: The conservative R&D-only ROI ({{< var dfda_roi_rd_only >}}:1) is the most defensible baseline figure, accounting for time value of money and gradual adoption.

**Full ROI range**: {{< var dfda_roi_rd_only >}}:1 (conservative) to {{< var dfda_roi_rd_plus_delay_plus_innovation >}}:1 (full impact), with **{{< var dfda_roi_rd_plus_delay >}}:1 recommended** for balanced presentations.

The simple ROI ({{< var dfda_roi_simple >}}:1) is useful for comparison but doesn't discount future cash flows.
...
2. **Annual Operational Cost**: The platform could be cheaper or more expensive to run than the estimated $40M. You model a range from {{< var lobbyist_bond_investment_max >}} to $80M.
...
# Source: dih_models/parameters.py

# Costs (in billions USD) - from dih_models/parameters.py
...
# Savings (in billions USD) - from dih_models/parameters.py

Rd = GLOBAL_CLINICAL_TRIALS_SPENDING_ANNUAL  # {{< var global_clinical_trials_spending_annual >}} annual global R&D spend on clinical trials
alpha = TRIAL_COST_REDUCTION_PCT  # 0.50 (50% reduction)

# Financial Parameters - from dih_models/parameters.py
...
annual_savings_stream = [(adoption_rate(t) * alpha * Rd) for t in range(1, int(T) + 1)]

print("NPV Model Parameters Loaded from dih_models/parameters.py")
...
operational_costs_stream = [Cop] * int(T)
...
1. **Exceptional ROI**: The platform generates a {{< var dfda_roi_simple >}}:1 return on investment ($50.0B savings from {{< var dfda_upfront_build >}} cost)
2. **Massive Health Benefits**: {{< var dfda_qalys_rd_plus_delay_annual >}} QALYs gained annually, worth $126 billion in monetized value
3. **Robust Financial Model**: NPV analysis confirms strong returns even with conservative assumptions
4. **Self-Funding**: The captured peace dividend ($27.2B) easily covers operational costs ({{< var dfda_upfront_build >}})
...
- **US Willingness-to-Pay Threshold**: Typically $100,000–$**[Primary QALY estimate used]**{{< var dfda_qalys_rd_plus_delay_annual >}} per QALY for interventions that _add_ costs. Dominant interventions are favorable regardless of this threshold.
...
- **Sources for Context:**
  ...
  - [Value per QALY](../references.qmd#qaly-value): "ICER's health benefit price benchmark (HBPB) will continue to be reported using the standard range from $100,000 to $**[Primary QALY estimate used]**{{< var dfda_qalys_rd_plus_delay_annual >}} per QALY..."
...
- **Conservative Scenario:** {{< var dfda_qalys_rd_plus_delay_annual >}} QALYs/year
- **Base Case Scenario:** {{< var dfda_qalys_rd_plus_delay_annual >}} QALYs/year
- **Optimistic/Transformative Scenario:** {{< var dfda_qalys_rd_plus_delay_annual >}} QALYs/year
...
- **Core Platform + Medium Broader Initiative:** $0.04B ({{< var dfda_upfront_build >}}) / year
- **Illustrative Total Ecosystem (Low-Medium):** $0.5B ({{< var recovery_trial_cost_per_patient >}}M) / year
...
| ---------------------------------------------------- | -------------------------- | -------------------------- | ----------------------------- | ------------------------------- | --------------------------- | --------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Base Case: Core Platform Ops Only**                | **50.0%** ($50.0B Savings)     | **$0.01875B ($18.75M)**    | **-$49.981B**                 | **{{< var dfda_qalys_rd_plus_delay_annual >}}**                     | **-$59,501**                | **Dominant**                            | Ops cost from the ['Annual Operational Costs'](#annual-operational-costs-illustrative-at-target-scale-of-5m-mau-50tb-ingest-month) ROM midpoint. QALYs from [new base model](#appendix-detailed-qaly-calculation-model).                                                              |
| Core Platform + Medium Broader Initiative            | 50.0% ($50.0B Savings)         | $0.04005B ($40.05M)        | -$49.96B                      | {{< var dfda_qalys_rd_plus_delay_annual >}}                         | -$59,476                    | Dominant                                | Ops from ['Annual Operational Costs'](#annual-operational-costs-illustrative-at-target-scale-of-5m-mau-50tb-ingest-month) + ['Scenario Based ROM Estimates for Broader Initiative Costs'](#scenario-based-rom-estimates-for-broader-initiative-costs) (Medium), aligns with ROI calc. |
| Total Ecosystem (Low-Medium Cost)                    | 50.0% ($50.0B Savings)         | $0.5B ({{< var recovery_trial_cost_per_patient >}}M)              | -$49.5B                       | {{< var dfda_qalys_rd_plus_delay_annual >}}                         | -$58,929                    | Dominant                                | Illustrative total ecosystem cost.                                                                                                                                                                                                                                                    |
| Total Ecosystem (High Cost, e.g. w/ Part. Comp.)     | 50.0% ($50.0B Savings)         | $5B                        | -$45B                         | {{< var dfda_qalys_rd_plus_delay_annual >}}                         | -$53,571                    | Dominant                                | Illustrative high total ecosystem cost (as prior base).                                                                                                                                                                                                                               |
| Conservative R&D Savings & QALYs                     | 30% ({{< var global_education_for_all_cost >}} Savings)         | $0.5B ({{< var recovery_trial_cost_per_patient >}}M)              | -$29.5B                       | {{< var dfda_qalys_rd_plus_delay_annual >}}                         | -$155,263                   | Dominant                                | Using Low-Med Ecosystem Cost & [Conservative QALY model](#summary-of-total-annual-qaly-gains).                                                                                                                                                                                        |
| Optimistic R&D Savings & QALYs                       | 70% ($70B Savings)         | $0.5B ({{< var recovery_trial_cost_per_patient >}}M)              | -$69.5B                       | {{< var dfda_qalys_rd_plus_delay_annual >}}                       | -$19,041                    | Dominant                                | Using Low-Med Ecosystem Cost & [Optimistic QALY model](#summary-of-total-annual-qaly-gains).                                                                                                                                                                                          |
| **Transformative R&D Savings (RECOVERY Trial-like)** | **95%** ({{< var sensitivity_dfda_savings_optimistic >}} Savings)     | **$0.5B ({{< var recovery_trial_cost_per_patient >}}M)**          | **-$94.5B**                   | **{{< var dfda_qalys_rd_plus_delay_annual >}}**                   | **-{{< var caregiver_value_per_hour_simple >}},890**                | **Dominant**                            | Using Low-Med Ecosystem Cost & [Optimistic QALY model](#summary-of-total-annual-qaly-gains).                                                                                                                                                                                          |
| Platform Breaks Even (R&D Savings = Ops Cost)        | e.g., {{< var trial_cost_reduction_pct >}} ($0.5B Savings) | $0.5B ({{< var recovery_trial_cost_per_patient >}}M)              | $0                            | {{< var dfda_qalys_rd_plus_delay_annual >}}                         | $0                          | Dominant (Cost-Neutral, Health Gaining) | Using Low-Med Ecosystem Cost & Base QALYs.                                                                                                                                                                                                                                            |
...
- **Massive Cost Savings & Extremely Favorable Core ICER**: The core dFDA Infrastructure (with operational costs of ~{{< var dfda_npv_annual_opex >}}-{{< var dfda_upfront_build >}}/year as per the ['Costs of Building and Operating the Global Decentralized FDA ROM Estimate'](#costs-of-building-and-operating-the-global-decentralized-fda-rom-estimate) ROM) generates tens of billions in net annual R and D savings. This results in extremely negative ICERs (e.g., ~-$59,501 per QALY), indicating exceptional value.
...
| :---------------------------- | :----------------------------------------------------------: | :------------------------------------------------------ | :------------------------------- | :------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| **dFDA Infrastructure**             |                     **4,744 - 194,667**                      | **-$260,000 to -$19,000**<a href="#ref_dfda_icer">²</a> | **Dominant**                     | [This analysis's Sensitivity Analysis](#sensitivity-analysis-overall-dfda-platform-cost-effectiveness). Based on $18.75M-$40.05M annual costs generating {{< var dfda_qalys_rd_plus_delay_annual >}}-3.65M QALYs annually.                                                                              |
| **Smallpox Eradication**      |      **{{< var dfda_large_trial_size >}} - 100,000+**<a href="#ref_dominant">³</a>      | **Dominant** (Cost-Saving)                              | **Dominant**                     | The $300M program (1967-1980) prevents 5M annual deaths. Benefit-cost ratio exceeds 100:1. Standard ICER calculation is impractical due to its uncommon scale. ([WHO, 2010](../references.qmd#who-smallpox-eradication-commemoration)) |
| **Childhood Vaccinations**    |        **22 - {{< var dfda_large_trial_size >}}+**<a href="#ref_dominant">³</a>         | Often **Dominant** to **~$100,000**                     | Dominant / Highly Cost-Effective | CDC estimates routine childhood vaccinations prevent 32M hospitalizations and 1.1M deaths among 1994-2023 US birth cohorts, with $2.9T in societal cost savings. ([CDC, 2023](../references.qmd#childhood-vaccinations-prevent-deaths))                                 |
| **Clean Water Programs**      |                       **100 - {{< var traditional_large_trial_size >}}**                        | **~$1,000 - $10,000**                                   | Highly Cost-Effective            | WHO estimates household water treatment costs {{< var lobbyist_bond_investment_max >}}-{{< var recovery_trial_cost_per_patient >}}/DALY averted. Community water supply improvements cost $200-${{< var dfda_drug_approvals_per_year_high >}}/DALY. ([WHO, 2004](../references.qmd#who-clean-water-daly-costs))                                              |
| **Hypertension Screening**    |                         **30 - 50**                          | **~{{< var lobbyist_bond_investment_max >}},000 - $33,000**                                  | Highly Cost-Effective            | Recent US studies show pharmacist-led hypertension management has ICERs under $**[Primary QALY estimate used]**{{< var dfda_qalys_rd_plus_delay_annual >}}/QALY, with most interventions falling in the $20,000-$33,000 range. ([JAMA Netw Open, 2023](../references.qmd#jama-hypertension-pharmacist-led))            |
| **Generic Drug Substitution** |       **{{< var traditional_large_trial_size >}} - {{< var dfda_large_trial_size >}}+**<a href="#ref_dominant">³</a>       | **Dominant** (Cost-Saving)                              | **Dominant**                     | By definition cost-saving when therapeutic equivalence is maintained, with typical savings of 30-80% versus brand-name drugs. ([WHO, 2015](../references.qmd#who-generic-drug-policy))                                                        |
| **Statins / Polypill**        |         **67 - {{< var traditional_large_trial_size >}}+**<a href="#ref_dominant">³</a>         | Cost-Saving to **~$15,000**                             | Dominant / Highly Cost-Effective | Cost-saving in high-risk populations. ICERs range from dominant to $15k/QALY in lower-risk groups. ([eClinicalMedicine, 2022](../references.qmd#eclinicalmedicine-statins-polypill))                                               |

### Methodology Notes

<a id="ref_chart_calc"></a>¹ **QALYs per $1M Calculation**:

- For interventions with positive ICERs: `{{< var traditional_large_trial_size >}},000 / ICER`
...
To provide context for the dFDA's estimated costs, it is useful to compare them to other significant U.S. government investments in health and technology. The dFDA's projected "Lean Ecosystem" cost of approximately **{{< var dfda_upfront_build >}} per year** (covering core platform operations plus medium-scope broader initiatives) is modest in comparison to other major federal projects.

| Initiative / Project                 | Approximate Cost / Budget (Annualized)                                                             | Comparison to dFDA Annual Cost           | Source / Note                                                              |
| :----------------------------------- | :------------------------------------------------------------------------------------------------- | :--------------------------------------- | :------------------------------------------------------------------------- |
| **dFDA Infrastructure (Lean Ecosystem)**   | **~{{< var dfda_upfront_build >}} / year**                                                                           | **1x (Baseline)**                        | [This analysis](#simplified-roi-scenario)                                  |
...
| **dFDA Infrastructure (Lean Ecosystem)**   | **~{{< var dfda_upfront_build >}} / year**                                                                           | **1x (Baseline)**                        | [This analysis](#simplified-roi-scenario)                                  |
| **Cancer Moonshot Initiative**       | **[~\$257 Million / year](https://www.cancer.gov/about-nci/budget)** ($1.8B over 7 years)          | **~6.4x**                                | [21st Century Cures Act](../references.qmd#21st-century-cures-act-2016)          |
| **NIH "All of Us" Research Program** | \*\*~{{< var recovery_trial_cost_per_patient >}} Million / year (FY23 Approx. Budget) | **~12.5x**                               | [NIH Budget](https://www.nih.gov/about-nih/what-we-do/budget)              |
