...
abstract: |
  ...
  per-drug FDA label); (2) precision dosing recommendations derived from optimal daily values
...
scores:
  ...
  voice: 5
  importance: 7
audience: "A technical reader (researcher, regulator, or builder) who wants to know exactly how the dFDA scores treatments and whether the cost and method claims hold up."
feel: "Convinced the method is real and rigorous: watching a billion people take medicine, then testing the interesting signals, is cheaper and more honest than today's gold standard."
do: "Trust the two-stage method and its cost numbers, then return to the chapter that sent you here (or build it)."
---
...
Current pharmacovigilance systems rely primarily on spontaneous adverse event reporting, which suffers from significant underreporting, lack of denominator data, and inability to quantify effect sizes. Meanwhile, the proliferation of wearable devices, health apps, and patient-reported outcomes has generated massive volumes of longitudinal real-world data (RWD) that remain largely untapped for safety and efficacy signal detection.
...
For the complete user-facing vision, see [A Decentralized Framework for Drug Assessment](https://manual.warondisease.org/knowledge/solution/dfda.html).
...
We present a framework that transforms real-world health data into drug safety signals you can act on:
...
This design combines the complementary strengths of each approach:
...
The processing protocol defines seven sequential steps:
...
2. **Privacy and Consent Layer**: Keeps raw records local when possible, enforces scoped consent, and shares only signed summaries or aggregates
3. **Measurement Normalization**: Standardizes units, timestamps, deduplicates records, and attributes data provenance
4. **Variable Ontology**: Assigns semantic categories, default temporal parameters ($\delta$, $\tau$), and filling value logic
5. **Relationship Analysis Engine**: Generates predictor-outcome pairs, performs temporal alignment, computes correlations, and optimizes hyperparameters
6. **Population Aggregation**: Combines individual N-of-1 analyses, computes confidence intervals, detects heterogeneity and subgroups
7. **Report Generation**: Produces outcome labels, treatment rankings, and safety/efficacy signals

The current public implementation is a prototype and reference implementation, not a regulated production medical device, clinical-trial network, or final privacy stack.
...
The protocol is designed for a federated network, not a central warehouse of everyone's medical records.

Raw records should stay with the source system whenever possible: the patient's device or vault, an EHR/FHIR server, a lab system, a study platform, or another approved data holder. Local nodes compute the pieces the network needs, such as variable IDs, time windows, effect sizes, confidence intervals, sample counts, and quality flags. The shared outputs are signed relationship summaries and cohort aggregates, not raw time series.

A coordinator can ask the same question across many nodes and combine the answers. It does not need to copy everyone's measurements into one database first, which would be the traditional "please put all the eggs in the hacking basket" architecture.

Consent must be explicit, scoped, revocable, and logged. A user or institution should be able to allow one study, one data category, one treatment/outcome class, or one time window without granting everything forever. Public outcome labels and treatment rankings should use cohort aggregation, minimum-count thresholds, and no row-level trajectories.

The exact privacy primitive can vary by deployment. Secure aggregation, differential privacy, trusted execution environments, multiparty computation, homomorphic encryption, and zero-knowledge proofs can all fit this design. The spec requires the privacy boundary; a production network should pick the simplest primitive that satisfies the data holder, regulator, and threat model.
...
- **v1 Code Repository**: <https://github.com/decentralized-fda>
- **Documentation**: <https://docs.dfda.earth>
- **v2 Code Repository**: <https://github.com/mikepsinn/optimitron>
- **Causal inference engine**: [`packages/optimizer`](https://github.com/mikepsinn/optimitron/tree/main/packages/optimizer) implements temporal alignment, correlations, Bradford Hill scoring, Predictor Impact Score, response curves, and outcome ranking
- **Data adapters and generated datasets**: [`packages/data`](https://github.com/mikepsinn/optimitron/tree/main/packages/data) includes local-first importers for Apple Health, Fitbit, Oura, MyFitnessPal, Withings, Google Fit, Cronometer, Strava, generic CSV, public-data fetchers, and the static dFDA medical-data snapshot
- **Web surfaces**: [`packages/web/src/app/agencies/dfda`](https://github.com/mikepsinn/optimitron/tree/main/packages/web/src/app/agencies/dfda), [`packages/web/src/app/conditions`](https://github.com/mikepsinn/optimitron/tree/main/packages/web/src/app/conditions), and [`packages/web/src/app/treatments`](https://github.com/mikepsinn/optimitron/tree/main/packages/web/src/app/treatments) expose condition pages, treatment pages, and dFDA agency routes
- **Live evidence site**: <https://studies.dfda.earth> runs this exact PIS/Bradford Hill pipeline today: 160,698 N-of-1 trials across 36,955 health variables from over 10,000 participants, covering 162 conditions and 300+ treatments, published as ranked Outcome Labels
