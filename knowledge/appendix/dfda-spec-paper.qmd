...
abstract: |
  Only 1-10% of adverse drug events get reported to the FDA. Billions of people generate continuous
  health data through wearables (heart rate, sleep, activity, symptoms daily) while starting and stopping
  treatments. This data could detect treatment effects but remains unharvested.

  A two-stage system solves the observational data problem. Stage 1 ({{< var dfda_observational_cost_per_patient >}}/patient):
  aggregate millions of natural experiments, score causal confidence using Bradford Hill criteria via
  the Predictor Impact Score. Observational data finds signals cheaply but can't prove causation.
  Stage 2 ({{< var dfda_pragmatic_trial_cost_per_patient >}}/patient): confirm top signals through pragmatic
  trials embedded in routine care. These are {{< var dfda_trial_cost_reduction_factor >}} cheaper than traditional
  Phase III because they use existing clinical infrastructure, not specialized trial sites. This cost estimate
  is based on a meta-analysis of 108 pragmatic trials plus large-scale implementations like RECOVERY and ADAPTABLE.

  Output: treatment rankings by effectiveness with validated effect sizes. Companies register treatments
  in minutes at zero cost. Patients generate outcome data worth {{< var traditional_phase3_cost_per_patient >}}/patient.
  The Predictor Impact Score operationalizes six Bradford Hill criteria using saturation functions,
  enabling automated causal signal detection from observational data at population scale.

  **Important limitation:** The Predictor Impact Score is a prioritization metric, not proof of causation.
  High PIS signals warrant experimental investigation; low PIS does not rule out true effects. This framework
  complements traditional RCTs. Stage 2 pragmatic trials are required to establish validated causal claims.
  Until PIS thresholds are validated against RCT outcomes, treat them as theoretically-motivated heuristics.
author:
  - name: Mike P. Sinn
    affiliations:
...
![A conceptual diagram showing the flow from raw data and statistical methodology to a user-facing searchable database that ranks drug effectiveness based on real-world outcomes.](/assets/images/dfda-spec-paper/dfda-spec-paper-section-system-overview-from-methodology-to-implementation-system-overview-bw-academic.jpg)
...
![Treatment Rankings Example - Patients see all treatments ranked by real-world effectiveness for their condition](../../assets/images/dfda-comparative-effectiveness-ranking-search.png){#fig-treatment-rankings}

### What Companies See

![A workflow diagram showing the streamlined process for companies, from instant registration to the generation of high-value clinical data through patient-funded treatments.](/assets/images/dfda-spec-paper/dfda-spec-paper-section-what-companies-see-bw-academic.jpg)
...
![The two-stage Predictor Impact Score pipeline illustrating the flow from low-cost real-world data analysis to high-priority pragmatic trials.](/assets/images/dfda-spec-paper/dfda-spec-paper-section-where-this-methodology-fits-bw-academic.jpg)
...
Every year, {{< var global_annual_deaths_curable_diseases >}} people die from diseases for which treatments exist or could exist. The tragedy is not that we lack medical knowledge. It's that our system for generating and validating that knowledge operates at a fraction of its potential capacity.

![A visualization of the 8.2-year clinical trial delay and its cumulative human cost, comparing the 102 million preventable deaths to the scale of 20th-century war casualties.](/assets/images/dfda-spec-paper/dfda-spec-paper-section-the-human-cost-of-the-current-system-bw-academic.jpg)

Consider: a treatment that could save lives today takes an average of {{< var efficacy_lag_years >}} to complete Phase 2-4 clinical trials after initial discovery. During this delay, people die waiting. Since 1962, regulatory testing delays for drugs that were eventually approved have contributed to an estimated {{< var existing_drugs_efficacy_lag_deaths_total >}} preventable deaths, more than all wars and conflicts of the 20th century combined.
...
![A comparison of the three primary pharmacovigilance methods highlighting their respective limitations in scale, speed, cost, and data quality.](/assets/images/dfda-spec-paper/dfda-spec-paper-section-the-pharmacovigilance-gap-bw-academic.jpg)
...
![A visual summary of the patient-generated health data ecosystem, showcasing various sources like wearables and apps alongside key data characteristics such as scale and longitudinal structure.](/assets/images/dfda-spec-paper/dfda-spec-paper-section-the-real-world-data-opportunity-bw-academic.jpg)
...
![Effect sizes from observational studies vs. randomized trials show strong concordance for mortality outcomes](../../assets/images/observational-vs-randomized-effect-sizes.png){#fig-rwe-validation-mortality}

![Across multiple clinical domains, observational and randomized effect sizes track closely](../../assets/images/observational-vs-randomized-trial-effect-sizes.png){#fig-rwe-validation-domains}
...
![A relational diagram showing how a Measurement object acts as a central hub connecting participant, variable, unit, and source identifiers.](/assets/images/dfda-spec-paper/dfda-spec-paper-section-measurement-structure-bw-academic.jpg)
...
![A conceptual diagram showing the relationship between a treatment dose and its impact on symptoms over time, highlighting the onset delay and duration of action.](/assets/images/dfda-spec-paper/dfda-spec-paper-section-mathematical-framework-bw-academic.jpg)
...
![Temporal alignment diagram illustrating how a single predictor measurement at time t creates an outcome window W(t) defined by onset delay (δ) and duration of action (τ).](/assets/images/dfda-spec-paper/dfda-spec-paper-section-mathematical-framework-bw-academic.jpg)
...
![A timeline diagram showing the relationship between an outcome measurement, the buffer period delta, and the preceding sampling window tau used to calculate predictor values.](/assets/images/dfda-spec-paper/dfda-spec-paper-section-outcome-based-pairing-predictor-has-filling-value-bw-academic.jpg)
...
![A timeline diagram showing a predictor measurement at time t_p, followed by a time delay delta, leading to an outcome window of length tau where measurements are averaged.](/assets/images/dfda-spec-paper/dfda-spec-paper-section-predictor-based-pairing-no-filling-value-bw-academic.jpg)
...
![A timeline visualization showing the data inclusion window between the earliest and latest recorded measurements, with data points outside these boundaries marked for exclusion.](/assets/images/dfda-spec-paper/dfda-spec-paper-section-temporal-boundaries-bw-academic.jpg)

#### Conservative Bias

![A conceptual chart showing how zero-filling missing data creates a conservative bias by pulling the measured correlation line toward the null baseline compared to the true effect.](/assets/images/dfda-spec-paper/dfda-spec-paper-section-conservative-bias-bw-academic.jpg)
...
![A conceptual diagram illustrating the difference between forward correlation (Predictor to Outcome) and reverse correlation (Outcome to Predictor), highlighting implications like reverse causality and confounding.](/assets/images/dfda-spec-paper/dfda-spec-paper-section-forward-and-reverse-correlations-bw-academic.jpg)
...
![A diagram illustrating the four components (strength of relationship, statistical significance, temporal order, and dose-response) feeding into the Predictor Impact Score to determine if a drug effect is noise or a candidate for clinical trials.](/assets/images/dfda-spec-paper/dfda-spec-paper-section-predictor-impact-score-bw-academic.jpg)
...
The PIS thresholds below are **theoretically motivated heuristics**, not empirically validated cutoffs. Until retrospective validation against RCT outcomes is performed (see @sec-validation-framework), these thresholds should be treated as provisional guidelines for prioritization, not evidence standards.
:::
...
![A conceptual flow showing how historical data analysis identifies an optimal daily value to achieve personalized precision dosing and superior outcomes.](/assets/images/dfda-spec-paper/dfda-spec-paper-section-optimal-daily-value-for-precision-dosing-bw-academic.jpg)
...
![A diagram showing the mapping of precise calculated values to standard, rounded dosing intervals for practical use.](/assets/images/dfda-spec-paper/dfda-spec-paper-section-grouped-optimal-values-bw-academic.jpg)
...
![A comparison diagram showing the different logic structures for generating personalized dosing recommendations based on whether the health outcome is positive or negative.](/assets/images/dfda-spec-paper/dfda-spec-paper-section-precision-dosing-recommendations-bw-academic.jpg)
...
![A conceptual diagram showing how population-level 'optimal values' pass through filters like individual variation and context to become personalized starting points for experimentation.](/assets/images/dfda-spec-paper/dfda-spec-paper-section-limitations-bw-academic.jpg)
...
![A flowchart depicting the decision logic for flagging a data relationship as 'boring' based on five specific rejection criteria.](/assets/images/dfda-spec-paper/dfda-spec-paper-section-boring-relationship-definition-bw-academic.jpg)
...
![A heatmap grid visualization showing Onset Delays on one axis and Durations on the other, illustrating the search space used to identify the strongest correlation coefficient.](/assets/images/dfda-spec-paper/dfda-spec-paper-section-optimization-grid-bw-academic.jpg)

The parameters yielding the strongest |r| are selected, subject to category-specific physiological constraints.

#### Overfitting Protection

![A visual checklist of the four safeguards used to prevent overfitting, highlighting data thresholds, domain constraints, result benchmarking, and directional consistency checks.](/assets/images/dfda-spec-paper/dfda-spec-paper-section-overfitting-protection-bw-academic.jpg)
...
![A sample Predictor Analysis Report visualization showing a horizontal bar chart of factors ranked by their positive or negative impact on a health outcome, including effect sizes and confidence intervals.](/assets/images/dfda-spec-paper/dfda-spec-paper-section-predictor-analysis-reports-bw-academic.jpg)
...
![A visual breakdown of the five predictor categories used to organize the analysis: treatments, foods, lifestyle, environment, and comorbid conditions.](/assets/images/dfda-spec-paper/dfda-spec-paper-section-category-specific-analysis-bw-academic.jpg)
...
![A hierarchical diagram illustrating the three-tier ranking system used for treatments, prioritized by effect size, then confidence level, and finally sample size.](/assets/images/dfda-spec-paper/dfda-spec-paper-section-within-category-rankings-bw-academic.jpg)
...
![A conceptual mapping showing how the proposed framework addresses each of the nine Bradford Hill criteria for causal inference.](/assets/images/dfda-spec-paper/dfda-spec-paper-section-addressing-the-bradford-hill-criteria-addressing-bradford-hill-bw-academic.jpg)
...
![A conceptual diagram illustrating how the proposed framework maps to and addresses each of the nine Bradford Hill criteria for causal inference.](/assets/images/dfda-spec-paper/dfda-spec-paper-section-addressing-the-bradford-hill-criteria-addressing-bradford-hill-bw-academic.jpg)
...
![A sequential flowchart illustrating the five automated quality check gates that data must pass through before being included in reports.](/assets/images/dfda-spec-paper/dfda-spec-paper-section-automated-quality-checks-bw-academic.jpg)
...
![A flowchart showing the lifecycle of a study from initial flagging criteria to exclusion and the potential for reinstatement after data correction.](/assets/images/dfda-spec-paper/dfda-spec-paper-section-flagged-study-handling-bw-academic.jpg)
...
![A workflow diagram showing the two-stage process: Stage 1 filters millions of observational data signals into top candidates, which Stage 2 then validates through low-cost pragmatic trials to establish causation.](/assets/images/dfda-spec-paper/dfda-spec-paper-section-stage-2-pragmatic-trial-confirmation-bw-academic.jpg)
...
![FDA vs dFDA: The two-stage pipeline dramatically reduces cost and time compared to traditional drug development](../../assets/infographics/FDA vs dFDA Infographic.png){#fig-fda-vs-dfda}
...
![A circular 'Learning Health System' diagram showing the continuous flow between observational Stage 1 models and experimental Stage 2 trials, with specific feedback channels for calibration and validation.](/assets/images/dfda-spec-paper/dfda-spec-paper-section-feedback-loop-trial-results-improve-observational-models-bw-academic.jpg)
...
![A conceptual diagram illustrating the iterative framework where observational signals funnel into experimental validation, creating a continuous learning loop for precision medicine.](/assets/images/dfda-spec-paper/dfda-spec-paper-section-what-this-framework-can-now-do-bw-academic.jpg)
...
The processing protocol defines five sequential layers:
...
![Data processing pipeline from raw ingestion to aggregated population reports.](/assets/images/dfda-spec-paper/dfda-spec-paper-section-implementation-guide-bw-academic.jpg)
...
![A comparison diagram illustrating the complementary relationship between the framework and Randomized Controlled Trials (RCTs), highlighting their distinct roles in drug safety and evidence generation.](/assets/images/dfda-spec-paper/dfda-spec-paper-section-positioning-relative-to-rcts-bw-academic.jpg)
...
![A visual mapping of the FDA Real-World Evidence Framework showing its support for the Sentinel System, post-market commitments, label updates, and comparative effectiveness.](/assets/images/dfda-spec-paper/dfda-spec-paper-section-fda-real-world-evidence-framework-alignment-bw-academic.jpg)
...
![A comparison bar chart showing the hypothesized higher replication rates in RCTs for high-PIS relationships versus low-PIS relationships to demonstrate validity.](/assets/images/dfda-spec-paper/dfda-spec-paper-section-the-critical-question-bw-academic.jpg)

### Proposed Validation Study

**Design**: Retrospective comparison of PIS predictions against published RCT results.

![Workflow of the retrospective validation study comparing PIS predictions against historical RCT results to assess discrimination and calibration.](/assets/images/dfda-spec-paper/dfda-spec-paper-section-proposed-validation-study-bw-academic.jpg)
...
![A visual summary of the three validation hurdles: causal directionality in treatment, the impact of parameter sensitivity on rankings, and the gap between tracker users and the general population.](/assets/images/dfda-spec-paper/dfda-spec-paper-section-known-limitations-requiring-validation-bw-academic.jpg)
...
![A four-stage validation roadmap for PIS predictions, illustrating the sequence from retrospective comparisons and prospective studies to expert review and sensitivity benchmarking.](/assets/images/dfda-spec-paper/dfda-spec-paper-section-validation-priorities-bw-academic.jpg)
...
![A flowchart mapping the 14-step sequential analysis workflow, starting from data ingestion and normalization through statistical processing and culminating in population aggregation and report generation.](/assets/images/dfda-spec-paper/dfda-spec-paper-section-appendix-e-analysis-workflow-bw-academic.jpg)
