...
The transaction tax (collected automatically on every $wishes transfer) provides the second stream: the UBI floor. This one is constant. It flows regardless of whether the economy grows or shrinks. Every citizen gets their daily deposit. No application. No means test. No bureaucrat. In recession, the productivity dividend contracts (less growth, less new money), but the transaction tax UBI continues because people are still transacting. Two revenue streams, one governance mechanism. Both flow through Wishocracy. The floor never drops.

To be explicit about what this replaces: income tax (gone; the transaction tax is automatic and invisible), the IRS (gone; a smart contract does not need 80,000 employees), the congressional budget process (gone; [Wishocracy](../solution/wishocracy.qmd) replaced it), the Federal Reserve (gone; the algorithm replaced it), grant committees (gone; funding follows wishocratic preferences), the lobbying industry (gone; there is nobody to lobby). Your species currently spends over a trillion dollars per year on the *process* of collecting taxes, allocating budgets, and arguing about both. On Wishonia, three pieces of software handle it: the monetary algorithm, the transaction tax smart contract, and Wishocracy. They cost nothing to operate. They have no dining room. Nobody takes them to lunch.
