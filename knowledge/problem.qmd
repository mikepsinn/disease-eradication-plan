...
Every single day, {{< var qalys_from_prevention_optimistic >}} humans stop existing because of diseases you could probably cure if anyone was paying attention (nobody is).

I tried to imagine what {{< var qalys_from_prevention_optimistic >}} people looks like. It's like if you took everyone in a decent-sized stadium and deleted them. Then did it again tomorrow. And the day after that. Forever. You'd run out of stadiums before you'd run out of corpses, which seems like poor urban planning.
...
**Killing each other:** ${{< var global_annual_conflict_deaths_state_violence >}} billion per year  
...
For comparison, imagine if you spent ${{< var global_annual_conflict_deaths_state_violence >}} on guns and $68 on food. Your neighbors would stage an intervention (or call the police, or possibly just move). But when humanity does it, you call it "defense spending" and give everyone medals.
...
When normal people lose {{< var lobbyist_bond_investment_max_millions >}}, they search the couch cushions. When the Pentagon loses $2,{{< var qalys_from_new_therapies >}},000,000, they ask for more money and Congress says yes.
...
During the Oxford RECOVERY trial, they tested treatments on 40,000 patients for {{< var recovery_trial_cost_per_patient >}} per person. The FDA's process costs $41,000 per patient. That's 82 times more expensive for the same result, except slower. It's like if you could get a haircut for {{< var vsl_epa_millions >}} or $820, but the $820 haircut takes seventeen years and you might be bald by then anyway.
...
Humanity spends $2.7 trillion every year on war. That works out to {{< var global_military_spending_per_capita_annual >}} per human on Earth for death tools.
...
- Submarines (one submarine = {{< var traditional_large_trial_size >}} cancer research labs)
...
Out of 2.4 billion people suffering from chronic disease right now, only [5 million get to participate in clinical trials annually](appendix/clinical-trial-participants.qmd). That's 0.2%. The other 99.8% get to suffer and wait (and fill out insurance paperwork, and argue with their insurance company about whether dying is covered under their plan).

Half of humanity would volunteer if asked. You're turning away 99.6% of willing participants. It's like running a restaurant but only serving {{< var disease_related_caregiver_pct >}} of the people who want to eat there, then wondering why everyone is so hungry.
...
This is because of something called "concentrated benefits versus diffuse costs." When defense contractors want $100 billion, they spend $55 million lobbying for it. That's a {{< var lobbying_roi_defense >}}% return on investment.
...
The brain drain is real: Raytheon pays ${{< var qalys_from_prevention_optimistic >}} to design bombs, the NIH pays $55,000 to cure diseases. Guess where the geniuses go.

Switzerland spends 0.7% of GDP on defense and has {{< var switzerland_gdp_per_capita_k >}},000 GDP per capita. America spends 3.5% on defense and has $76,000 GDP per capita. 
...
- {{< var qalys_from_prevention_optimistic >}} humans are deleted daily by curable diseases
...
They say every problem is an opportunity in disguise. In this case, the opportunity is disguised as {{< var qalys_from_prevention_optimistic >}} corpses per day, which is admittedly not the most encouraging disguise, but you work with what you have.
