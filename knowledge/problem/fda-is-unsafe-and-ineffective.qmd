...
In practice, they took a subtler approach: make drugs so expensive ({{< var pharma_drug_development_cost_current >}}) and slow ({{< var treatment_acceleration_years_current >}} years) that you die before the cure arrives. The regulatory barrier isn't just high - it's like setting the bar very high, then adding landmines, then putting the bar on top of Mount Everest.
...
The FDA's system costs **{{< var trial_cost_reduction_factor >}}× more** and takes **{{< var fda_to_oxford_recovery_trial_time_multiplier >}}x longer**. Not a small difference - the difference between functional and designed-to-fail.
...
The total cost? **{{< var pharma_drug_development_cost_current >}}** per new drug (see [source](../references.qmd#drug-development-costs)).
...
Why? Math. If your disease only affects 10,000 people, and it costs {{< var pharma_drug_development_cost_current >}} to develop a drug, each patient would need to pay $260,000 just to break even.
...
**{{< var regulatory_delay_deaths_mean >}} million preventable deaths** (1962-2024)

95% confidence interval: {{< var regulatory_delay_deaths_lower_bound >}}M – {{< var regulatory_delay_deaths_upper_bound >}}M
...
**{{< var regulatory_delay_dalys >}} billion years of healthy human life** deleted by the regulatory framework.

- **Economic cost:** {{< var regulatory_delay_economic_loss >}} (2024 USD)
...
- **Cost of Type II errors:** {{< var regulatory_delay_dalys >}}B DALYs lost (1962-2024)
- **Benefit of Type I prevention:** ~{{< var type_i_error_benefit_dalys >}}B DALYs saved (extreme overestimate)
...
- **Slow:** {{< var treatment_acceleration_years_current >}} years from discovery to patient (you'll be dead)
...
- **{{< var regulatory_delay_deaths_mean >}}M deaths** from delayed access (1962-2024)
- **{{< var regulatory_delay_dalys >}}B DALYs** destroyed by paperwork
- **{{< var regulatory_delay_economic_loss >}}** in economic carnage
