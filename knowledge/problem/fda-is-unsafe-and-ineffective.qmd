...
Meanwhile, regulatory delays prevent millions from accessing treatments that could save their lives. The system doesn't just fail to ensure safety. It actively undermines it while simultaneously killing people through delay.


## A Note on Blame

The FDA employees executing this system didn't design it. Congress did, in 1962, via the Kefauver-Harris Amendments. The FDA implements what Congress mandated. The problem isn't bad people at the FDA. The problem is a bad law that makes good outcomes structurally impossible.

When this chapter says "the FDA does X," read it as "the 1962 law requires the FDA to do X." The humans at the agency follow the rules they were given. The rules are the problem.
...
- {{< var recovery_trial_cost_reduction_factor >}} cost gap: Oxford trial ({{< var recovery_trial_cost_per_patient >}}) vs. standard Phase III ({{< var traditional_phase3_cost_per_patient >}})
- {{< var post_1962_drug_approval_reduction_pct >}} drop in drug approvals: fell from 46/year (1960) to 13/year (1963) [@post-1962-drug-approval-drop]
- Beta-blocker deaths: the decade-long delay killed 100,000 Americans [@beta-blocker-drug-lag-deaths]
- COVID test fiasco: the FDA blocked WHO tests while CDC tests sat contaminated
...
Then in 1962, Congress responded to this success by adding massive **pre-approval efficacy requirements** via the Kefauver Harris Amendment [@kefauver-harris-amendment-1962]. This blocked patient access for years during testing. The new system:
...
The {{< var drug_cost_increase_pre1962_to_current_multiplier >}} cost increase is rigorously documented with multiple data sources, CPI adjustments, and sensitivity analysis. See [Drug Development Cost Analysis](https://drug-cost.warondisease.org) for:

- Congressional testimony and alternative data sources
- Exact inflation calculations (1962 to 2024)
...
However, validating that large-scale real-world evidence produces better health outcomes requires further validation. This is why we need a decentralized framework for drug assessment.
...
A [comprehensive quantitative analysis](https://invisible-graveyard.warondisease.org) uses WHO mortality data and Monte Carlo modeling. It estimates the *total* cost of blocking patient access during the {{< var efficacy_lag_years >}}-year post-safety efficacy delay:
...
For comparison: The entire global pharmaceutical R&D budget is ~{{< var global_pharma_rd_spending_annual >}}/year. The post-safety efficacy lag costs **38x more** than all drug development spending combined.

**Even if you think these estimates are off by a factor of 10**, the core problem remains. The system costs {{< var recovery_trial_cost_reduction_factor >}} more per patient. It takes 36x longer than proven alternatives. It produces less safety data from smaller, less representative populations.
...
This means regulators logically reject drug applications by default. The personal risks of approving a drug with any newsworthy side effect far outweigh the personal risk of preventing access to life-saving treatment.
...
The most infamous case is beta-blockers. Europe used them to prevent heart attacks. The US regulatory process delayed. And delayed. The decade-long lag in approving them killed an estimated **100,000 Americans [@beta-blocker-drug-lag-deaths]**.
...
After 1962, US regulations increased. You can see a divergence from Switzerland's growth in life expectancy. Switzerland did not introduce the same delays to availability.
...
- This assumes ALL drug withdrawals were prevented by the 1962 changes. Those changes blocked patient access during lengthy efficacy trials. They weren't all prevented; Phase I safety testing already existed.
...
The FDA's post-1962 system requires small, artificial efficacy trials before allowing patient access. This was supposed to prove drugs work. But by forcing small artificial trials and excluding real-world populations, it created a system more dangerous than the real-world evidence approach it replaced.
...
The result: Drugs are approved as "safe" based on tiny, artificial samples. Then they're given to millions of real patients where the actual safety problems emerge.
...
Post-approval, the FDA relies on the **FDA Adverse Event Reporting System (FAERS)**. This is a passive, voluntary system. Doctors report problems they happen to notice and connect to a specific drug.
...
**The safety ratio is backwards.** Testing 3,000 cherry-picked patients and hoping voluntary reporting catches problems is more dangerous than active monitoring of adverse events with real patients.
...
Phase I safety testing works. It screens out immediately toxic compounds. But blocking patient access during the 8.2-year post-safety efficacy delay makes drugs more dangerous. It forces small artificial trials and excludes real-world evidence collection. Meanwhile, people die through delayed access to life-saving treatments.
