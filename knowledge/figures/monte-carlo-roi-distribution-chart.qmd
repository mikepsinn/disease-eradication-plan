...
base_roi = float(DFDA_ROI_RD_ONLY)
...
fig.subplots_adjust(top=0.88, bottom=0.12)  # More space for title, room for watermark
...
ax1.set_title('Conditional ROI\n(If Implementation Succeeds)', fontsize=22, weight='bold', pad=15)
ax1.set_xlabel('ROI (ratio)', fontsize=18)
ax1.set_ylabel('Frequency', fontsize=18)
# Move legend to upper left to avoid overlap

ax1.legend(fontsize=13, loc='upper left', framealpha=1.0, 
           bbox_to_anchor=(0.02, 0.98), frameon=True, 
           edgecolor=COLOR_BLACK, facecolor=COLOR_WHITE)
# Move CI box to bottom left

ax1.text(0.02, 0.05, f"95% CI: [{cp['p5']:.1f}:1, {cp['p95']:.1f}:1]", 
         transform=ax1.transAxes, fontsize=15, weight='bold',
...
ax2.hist(expected_samples, bins=50, color=COLOR_BLACK, edgecolor=COLOR_BLACK, linewidth=1.5)
ax2.axvline(ep['p50'], color=COLOR_WHITE, linestyle='--', linewidth=2, label=f"Median: {ep['p50']:.1f}:1")
ax2.axvline(ep['p5'], color=COLOR_WHITE, linestyle=':', linewidth=1.5, alpha=0.9, label=f"5th: {ep['p5']:.1f}:1")
ax2.axvline(ep['p95'], color=COLOR_WHITE, linestyle=':', linewidth=1.5, alpha=0.9, label=f"95th: {ep['p95']:.1f}:1")
ax2.set_title('Expected ROI\n(Accounting for Political Risk)', fontsize=22, weight='bold', pad=15)
ax2.set_xlabel('ROI (ratio)', fontsize=18)
ax2.set_ylabel('Frequency', fontsize=18)
# Move legend to upper left to avoid overlap

ax2.legend(fontsize=13, loc='upper left', framealpha=1.0,
           bbox_to_anchor=(0.02, 0.98), frameon=True,
           edgecolor=COLOR_WHITE, facecolor=COLOR_BLACK)
# Move CI box to bottom left with white text on black background

ax2.text(0.02, 0.05, f"95% CI: [{ep['p5']:.1f}:1, {ep['p95']:.1f}:1]", 
         transform=ax2.transAxes, fontsize=15, weight='bold', color=COLOR_WHITE,
         verticalalignment='bottom',
         bbox=dict(boxstyle='round,pad=0.5', facecolor=COLOR_BLACK, edgecolor=COLOR_WHITE, linewidth=1.5))
clean_spines(ax2)

# Add overall title - moved down to avoid overlap

fig.suptitle('Monte Carlo Simulation: ROI Uncertainty Quantification', 
             fontsize=26, weight='bold', y=0.95)
