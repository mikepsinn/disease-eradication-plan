...
from dih_models.plotting.chart_style import (
    ...
    get_project_root, COLOR_BLACK, COLOR_WHITE, get_figure_output_path
)
...
# Import parameters from dih_models/parameters.py

from dih_models.parameters import *

# Conversion constant - parameters store RAW USD values

BILLION = 1_000_000_000

# NPV Model Parameters (from dih_models/parameters.py)

C0 = float(DFDA_NPV_UPFRONT_COST_TOTAL) / BILLION  # $0.26975B upfront
Cop = float(DFDA_NPV_ANNUAL_OPEX_TOTAL) / BILLION  # $0.04005B annual ops
Rd = float(GLOBAL_CLINICAL_TRIALS_SPENDING_ANNUAL) / BILLION  # $100B annual R&D spend
...
cumulative_costs_npv = np.zeros(int(T) + 1)
cumulative_savings_npv = np.zeros(int(T) + 1)
cumulative_net_benefit_npv = np.zeros(int(T) + 1)
...
ax.set_title('Net Present Value: A Decentralized Drug Assessment Framework\n8% Discount, 5-Yr Adoption Ramp',
...
output_path = get_figure_output_path('dfda-npv-analysis-over-time-line-chart.png')
...
add_png_metadata(
    output_path,
    title="Net Present Value of a Decentralized Drug Assessment Framework",
