...
```{python}
#| fig-cap: "VICTORY Incentive Alignment Bonds: 270% Perpetual Returns"
from pathlib import Path

import matplotlib.pyplot as plt
from dih_models.plotting.chart_style import (
    setup_chart_style, add_watermark, clean_spines,
    add_png_metadata, COLOR_BLACK, COLOR_WHITE, get_figure_output_path, get_figure_output_path)
...
# Venture Capital (industry avg), Medallion Fund (../references.qmd),
# Cocaine (UN estimates), VICTORY [Incentive Alignment Bonds](../appendix/incentive-alignment-bonds-paper.qmd) (../economics/victory-bonds.qmd)

investments = ['S&P 500', 'Real Estate', 'Venture\nCapital', 'Medallion\nFund', 'Cocaine\nTrafficking', 'VICTORY\nBonds']
returns = [
    AVERAGE_MARKET_RETURN_PCT * 100,  # S&P 500
    15,  # Real Estate (no parameter exists)
    25,  # Venture Capital (no parameter exists)
    39,  # Medallion Fund (no parameter exists)
    90,  # Cocaine (no parameter exists)
    VICTORY_BOND_ANNUAL_RETURN_PCT * 100  # VICTORY Incentive Alignment Bonds
]
...
# White bars with black outline for comparison items, solid black for VICTORY [Incentive Alignment Bonds](../appendix/incentive-alignment-bonds-paper.qmd) (emphasis)

bars = []
for i, ret in enumerate(returns):
    if i == len(returns) - 1:  # Last item (VICTORY Incentive Alignment Bonds) - solid black for emphasis
...
for i, (bar, ret) in enumerate(zip(bars, returns)):
    ...
    # White text on black bar (VICTORY Incentive Alignment Bonds), black text on white bars (others)
    if i == len(returns) - 1:  # VICTORY [Incentive Alignment Bonds](../appendix/incentive-alignment-bonds-paper.qmd) - white text on black bar
...
output_path = get_figure_output_path('victory-bonds-roi-projection-bar-chart.png')
...
add_png_metadata(
    ...
    description="Comparison of ROI: S&P 500, real estate, venture capital, hedge funds, and VICTORY Incentive Alignment Bonds"
)
