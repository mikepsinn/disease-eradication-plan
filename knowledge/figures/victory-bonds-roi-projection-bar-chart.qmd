...
```{python}
#| output: false
#| fig-cap: "VICTORY Social Impact Bonds: 270% Perpetual Returns"
...
# Cocaine (UN estimates), VICTORY Social Impact Bonds (../economics/victory-bonds.qmd)
...
returns = [
    ...
    VICTORY_BOND_ANNUAL_RETURN_PCT * 100  # VICTORY Bonds
]
...
# White bars with black outline for comparison items, solid black for VICTORY Social Impact Bonds (emphasis)

bars = []
for i, ret in enumerate(returns):
    if i == len(returns) - 1:  # Last item (VICTORY Social Impact Bonds) - solid black for emphasis
...
for i, (bar, ret) in enumerate(zip(bars, returns)):
    ...
    # White text on black bar (VICTORY Social Impact Bonds), black text on white bars (others)
    if i == len(returns) - 1:  # VICTORY Social Impact Bonds - white text on black bar
...
add_png_metadata(
    ...
    description="Comparison of ROI: S&P 500, real estate, venture capital, hedge funds, and VICTORY Social Impact Bonds"
)
