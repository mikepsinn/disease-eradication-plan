...
## Treaty Peace Dividend Calculations
...
print(f"Military budget available for a 1% Treaty Fund: ${peace_dividend['military_budget_redirect']/1e9:.1f}B")
```

## ROI for a Global Pragmatic Trial Framework
...
def calculate_dfda_roi(
    ...
    """
    Calculate ROI for a decentralized framework for drug assessment (dFDA).
...
print(f"Framework ROI: {dfda_roi['roi_ratio']:.0f}:1")
print(f"Annual savings: ${dfda_roi['annual_savings']/1e9:.1f}B")
print(f"10-year NPV: ${dfda_roi['npv_10_year']/1e9:.1f}B")
```

## QALY Value Calculations

```{python}
#| echo: true
def calculate_qaly_value(
    qalys_generated: int = int(DFDA_EFFICACY_LAG_ELIMINATION_DALYS),
    value_per_qaly: float = QALY_VALUE
) -> Dict[str, float]:
    """
    Calculate economic value of DALYs averted from one-time timeline shift.
    Based on regulatory delay elimination methodology (efficacy lag elimination).
    """
    total_value = qalys_generated * value_per_qaly

    # Cost per DALY (for cost-dominant intervention)
    cost_per_daly = float(TREATY_COST_PER_DALY_TRIAL_CAPACITY_PLUS_EFFICACY_LAG)

    return {
        'total_dalys': qalys_generated,
        'total_value': total_value,
        'value_per_daly': value_per_qaly,
        'cost_per_daly': cost_per_daly,
        'total_timeline_shift_years': float(EFFICACY_LAG_YEARS)
    }
...
def calculate_market_efficiency(
    ...
    dih_budget: float = 27_000_000_000,  # e.g., an allocation from a 1% Treaty Fund
...
print(f"Budget for pragmatic clinical trials: ${market_efficiency['dih_budget']/1e9:.1f}B")
...
## Accelerated Innovation Impact

```{python}
#| echo: true
def calculate_acceleration_impact(
    current_approvals: int = 50,  # FDA approvals per year
    acceleration_factor: float = 22.8,  # 22.8x more trial capacity
...
formulas_df = pd.DataFrame({
    ...
    'Calculation': [
        ...
        f"{qaly_value['total_qalys']/1e6:.1f}M QALYs × $150K",
        'Allocated Budget × 50% gain = $13.5B',
