...
roi_data = pd.DataFrame({
    ...
    'ROI Ratio': [
        float(OPTIMISTIC_SCENARIO_ROI),
        float(DFDA_ROI_RD_ONLY),
        float(CONSERVATIVE_SCENARIO_ROI)
    ]
})
...
```{python}
#| echo: false
# QALY sources breakdown - using current parameters from parameters.py

faster_access = float(QALYS_FROM_FASTER_ACCESS)
prevention = float(QALYS_FROM_PREVENTION)
new_therapies = float(QALYS_FROM_NEW_THERAPIES)
total_qalys = faster_access + prevention + new_therapies

qaly_sources = pd.DataFrame({
    'Source': ['Faster Drug Access', 'Enhanced Prevention', 'New Therapies for Rare/Untreatable Diseases'],
    'Annual QALYs': [faster_access, prevention, new_therapies],
    'Percentage': [
        (faster_access / total_qalys) * 100,
        (prevention / total_qalys) * 100,
        (new_therapies / total_qalys) * 100
    ]
})
...
fig.update_layout(
    title=f"Annual QALYs Generated by Source (Total: {total_qalys/1000:.0f}K)",
    annotations=[dict(text=f'{total_qalys/1000:.0f}K<br>QALYs/Year', x=0.5, y=0.5,
...
- **ROI Calculation**: `{python} roi_dfda_savings_only_formatted` based on `{python} dfda_annual_opex_formatted` annual cost generating `{python} dfda_rd_gross_savings_annual_formatted` savings
