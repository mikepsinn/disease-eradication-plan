...
conservative_deaths_annual = float(HISTORICAL_PROGRESS_DEATHS_ANNUAL) / 1e6  # Convert to millions
conservative_deaths_total = float(HISTORICAL_PROGRESS_DEATHS_TOTAL) / 1e6
primary_deaths_annual = float(DISEASE_ERADICATION_DELAY_DEATHS_ANNUAL) / 1e6
primary_deaths_total = float(DISEASE_ERADICATION_DELAY_DEATHS_TOTAL) / 1e6
optimistic_deaths_annual = float(DISEASE_ERADICATION_PLUS_ACCELERATION_DEATHS_ANNUAL) / 1e6
