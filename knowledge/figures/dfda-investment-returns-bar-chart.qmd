...
# Core parameters for a model of a decentralized framework for drug assessment (dFDA) (in billions of USD)
# WORST CASE SCENARIO (Conservative) from ../appendix/dfda-impact-paper.qmd

BILLION = 1_000_000_000

global_clinical_trial_market = float(GLOBAL_CLINICAL_TRIALS_SPENDING_ANNUAL) / BILLION  # Convert raw USD to billions
dfda_rd_cost_reduction_pct = DFDA_TRIAL_COST_REDUCTION_PCT
...
health_gains_conservative = float(EXISTING_DRUGS_EFFICACY_LAG_ECONOMIC_LOSS) / BILLION
health_gains_primary = float(DFDA_EFFICACY_LAG_ELIMINATION_ECONOMIC_VALUE) / BILLION
health_gains_optimistic = float(DFDA_TRIAL_CAPACITY_PLUS_EFFICACY_LAG_ECONOMIC_VALUE) / BILLION

# Component values for equations

conservative_deaths_total = float(EXISTING_DRUGS_EFFICACY_LAG_DEATHS_TOTAL) / 1e6
primary_deaths_total = float(DFDA_EFFICACY_LAG_ELIMINATION_DEATHS_AVERTED) / 1e6
optimistic_deaths_total = float(DFDA_TRIAL_CAPACITY_PLUS_EFFICACY_LAG_LIVES_SAVED) / 1e6
...
# Helper function to format values in trillions (used for both labels and legend)

def format_as_trillions(value_usd):
    """Format USD values in trillions with T suffix, regardless of magnitude."""
    trillions = value_usd / 1e12
    if abs(trillions) >= 100:
        result = "${:.0f}T".format(trillions)
    elif abs(trillions) >= 10:
        result = "${:.1f}T".format(trillions)
    else:
        result = "${:.2f}T".format(trillions)
    # Clean up trailing zeros
    result = result.replace(".0T", "T")
    return result

conservative_value = format_as_trillions(EXISTING_DRUGS_EFFICACY_LAG_ECONOMIC_LOSS)
primary_value = format_as_trillions(DFDA_EFFICACY_LAG_ELIMINATION_ECONOMIC_VALUE)
optimistic_value = format_as_trillions(DFDA_TRIAL_CAPACITY_PLUS_EFFICACY_LAG_ECONOMIC_VALUE)
...
# Health gains - label each tier in trillions with "T" suffix (TOTAL values, not annual)
# Note: format_as_trillions() function defined earlier in the code

# Conservative tier (bottom)

conservative_label = format_as_trillions(EXISTING_DRUGS_EFFICACY_LAG_ECONOMIC_LOSS)
...
primary_label = format_as_trillions(DFDA_EFFICACY_LAG_ELIMINATION_ECONOMIC_VALUE)
...
optimistic_label = format_as_trillions(DFDA_TRIAL_CAPACITY_PLUS_EFFICACY_LAG_ECONOMIC_VALUE)
