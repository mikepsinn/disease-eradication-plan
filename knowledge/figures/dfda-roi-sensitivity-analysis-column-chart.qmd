...
# Three-tier ROI based on different health benefit scenarios (all use TREATY_CAMPAIGN_TOTAL_COST campaign cost)

scenarios = ['Historical Rate\n(Existing Drugs Only)', 'Lag Elimination\n(PRIMARY)', 'Full Timeline Shift\n(Trial Capacity + Lag)']
roi_parameters = [
    TREATY_ROI_EXISTING_DRUGS_ONLY,                      # Conservative: Existing drugs only
    DFDA_EFFICACY_LAG_ELIMINATION_ECONOMIC_VALUE / TREATY_CAMPAIGN_TOTAL_COST,  # PRIMARY: Efficacy lag elimination
    TREATY_ROI_TRIAL_CAPACITY_PLUS_EFFICACY_LAG          # Full: Trial capacity + efficacy lag (~220 years)
]
roi_values = [float(roi) for roi in roi_parameters]

# Format benefit labels with parameter formatter

benefit_historical = format_parameter_value(EXISTING_DRUGS_EFFICACY_LAG_ECONOMIC_LOSS)
benefit_primary = format_parameter_value(DFDA_EFFICACY_LAG_ELIMINATION_ECONOMIC_VALUE)
benefit_optimistic = format_parameter_value(DFDA_TRIAL_CAPACITY_PLUS_EFFICACY_LAG_ECONOMIC_VALUE)
...
ax.set_ylabel(
    f'Return on Investment\n(Total Benefit ÷ {TREATY_CAMPAIGN_TOTAL_COST} Campaign Cost)',
    fontsize=18,
...
add_png_metadata(
    ...
    description=f"ROI analysis across three health benefit tiers based on {TREATY_CAMPAIGN_TOTAL_COST} campaign cost. Historical Rate (existing drugs only): {roi_values[0]:,.0f}:1 ({benefit_historical}). Lag Elimination PRIMARY: {roi_values[1]:,.0f}:1 ({benefit_primary}). Full Timeline Shift: {roi_values[2]:,.0f}:1 ({benefit_optimistic}). All scenarios show total one-time benefits divided by campaign investment."
)
