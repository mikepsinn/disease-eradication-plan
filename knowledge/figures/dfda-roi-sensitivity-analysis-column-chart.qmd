...
roi_values = [
    float(TREATY_ROI_HISTORICAL_RATE),        # Conservative: $251T / $1B = 250,920:1
    float(TREATY_ROI_LAG_ELIMINATION),        # PRIMARY: $1,286T / $1B = 1,286,242:1
    float(TREATY_ROI_INNOVATION_ACCELERATION) # Optimistic: $2,572T / $1B = 2,572,484:1
]
...
for i, (roi, benefit) in enumerate(zip(roi_values, benefits)):
    ...
    # Use white text for middle bar (PRIMARY, which is black), black text for others
    ...
    ax.text(
        ...
        color=text_color,
        bbox=dict(boxstyle='round,pad=0.6', facecolor=COLOR_WHITE if i != 1 else COLOR_BLACK,
...
ax.set_xticklabels(scenarios, fontsize=20, fontweight='bold')
