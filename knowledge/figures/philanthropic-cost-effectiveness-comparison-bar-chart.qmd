...
# For a 1% treaty bar, we'll stack it to show peace dividend vs dFDA savings
# Total QALYs from parameters:
#   - Peace: TREATY_QALYS_GAINED_ANNUAL_GLOBAL (85,610 annual recurring)
#   - dFDA: DISEASE_ERADICATION_DELAY_DALYS (8.57B total from one-time timeline shift)
...
dfda_qalys = float(DISEASE_ERADICATION_DELAY_DALYS)
total_qalys = peace_qalys + dfda_qalys

peace_dividend_per_life = -5870000 * (peace_qalys / total_qalys)  # Proportional to QALYs
dfda_savings_per_life = -5870000 * (dfda_qalys / total_qalys)   # Proportional to QALYs
...
# For the 5th bar (1% treaty), create stacked bar showing peace + dFDA
...
bar_dfda = ax.bar(x_pos[4], dfda_savings_per_life, width=0.6, bottom=peace_dividend_per_life,
                  color=COLOR_WHITE, edgecolor=COLOR_BLACK, linewidth=2,
...
ax.text(x_pos[4] - 0.35, peace_dividend_per_life + dfda_savings_per_life/2,
        'dFDA\nR&D\nSavings',
        ha='right', va='center', fontsize=9, color=COLOR_BLACK, style='italic')
...
#   - dFDA: 74.15M QALYs (regulatory delay elimination methodology)
